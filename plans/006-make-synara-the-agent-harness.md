# Plan 006: Make Cortex the Authoritative Agent Harness
...
An explicit request to start two Cortex threads produced six threads. The
...
`cortex_create_thread`, tried several invalid provider/model combinations, and
...
1. **Model awareness** — every provider is told, through its strongest native
   instruction channel, that Cortex is the hosting harness and that Cortex
   resources are controlled through `cortex_*` tools.
2. **Capability truth** — agents can ask Cortex which providers, models,
...
```text
user asks for exactly two Cortex threads
  -> parent reads cortex_context / cortex_capabilities
  -> parent calls cortex_create_threads once with exactly two entries
  ...
  -> parent calls cortex_wait_for_threads until both runs are terminal
...
- Cortex is the harness; provider-native subagents are implementation details
  inside a provider turn and never substitute for user-requested Cortex
  threads.
- All model-facing host text comes from one versioned
  `CortexHarnessPolicy`. Provider adapters only choose the delivery mechanism.
- Every state-changing gateway call is bound to a live Cortex provider session
...
- Unsupported provider/model/options fail before `thread.create`, git branch,
  worktree, or provider side effects. Cortex never interprets `Low` as part of
...
- `apps/server/src/agentGateway/Layers/AgentGateway.ts:73-81` owns the only full
  “Cortex is hosting this session / use cortex\_\*” policy, but
...
- `apps/server/src/provider/Layers/ClaudeAdapter.ts:880-885` tells Claude it is
  running inside Cortex, but does not define Cortex-thread versus native
  subagent precedence.
- `apps/server/src/codexAppServerManager.ts:475-486,620-648` sends Codex
  ...
  include the Cortex harness contract.
- Cursor, Grok, and Droid receive MCP transport configuration and a
  Cortex `clientInfo`, but no shared model-facing host policy. See
...
- `apps/server/src/provider/runtimeLayer.ts:54-70` explicitly excludes OpenCode
  ...
  and a clear “Cortex control unavailable” capability instead of pretending
  parity.
...
- A background native agent that retains the same MCP connection can therefore
  mutate Cortex after the visible parent turn has completed.
...
- predecessor naming — Cortex identity must remain consistent;
...
### `cortex_context` (read-only, idempotent)

```ts
type CortexContextResult = {
  harness: { name: "Cortex"; policyVersion: string };
  caller: {
    threadId: ThreadId;
    turnId: TurnId | null;
...
### `cortex_capabilities` (read-only, idempotent)
...
### `cortex_create_threads` (destructive, idempotent with request id)

```ts
type CortexCreateThreadsInput = {
...
Keep `cortex_create_thread` as a backwards-compatible single-entry wrapper
...
### `cortex_wait_for_threads` (read-only, idempotent)

```ts
type CortexWaitForThreadsInput = {
...
- Rebranding or renaming `cortex_*` tools.
- Disabling provider-native subagents globally. They remain available for
  ...
  explicit Cortex-resource requests.
...
- UI redesign. Existing Cortex MCP logo/labels may consume tool annotations,
...
- Branch from the current feature branch with `codex/cortex-harness-control-plane`.
...
### Step 2: Centralize and deliver the Cortex harness policy
...
1. “You are running inside Cortex. Cortex is the host/harness for this
   session.”
2. Use `cortex_*` for Cortex threads, projects, automations, and coordination.
3. Provider-native `spawn_subagent`/Task/collaboration tools do not create
   Cortex threads and must not substitute for an explicit request for Cortex
   threads.
4. Use one `cortex_create_threads` call for plural requests; the array count is
   exact.
5. Resolve canonical models/options through `cortex_capabilities`; do not guess
   slugs or silently change provider/model.
6. Wait with `cortex_wait_for_threads`, then synthesize all results.
...
- **Claude**: append it to the existing Claude system-prompt append. Add MCP
  ...
  list.
- **Cursor/Grok/Droid ACP paths**: include consistent Cortex clientInfo
  ...
  user-visible Cortex message projection.
- **OpenCode/Kilo**: deliver truthful host identity, but advertise Cortex MCP
...
- **Pi**: deliver host identity through its supported prompt/extension path and
  explicitly advertise that Cortex MCP control is unavailable. Do not simulate
  successful Cortex actions.

Keep MCP initialize `instructions` generated from the same policy. Add tool
annotations and make every description begin with its user-facing Cortex
...
- caller thread id;
- a Cortex-generated provider-session key (not an upstream provider id);
- provider kind;
...
- return the same catalog from `cortex_capabilities` that validation uses;
...
Implement `cortex_wait_for_threads` as a read-only long poll over projections:
...
This cap protects Cortex projections from provider fan-out, but it does not
disable native subagent execution. Keep native-child budgeting separate from
the ordinary top-level `cortex_create_threads` operation.
...
- the active parent path uses one two-entry Cortex batch;
...
- [x] Every provider receives the same versioned Cortex host identity through
      its strongest available instruction channel.
- [x] Every provider truthfully reports whether it can call Cortex MCP; no
      provider simulates unsupported host actions.
- [x] MCP initialize identity, tool descriptions, and native host instructions
      all say Cortex, never a predecessor or provider-branded harness.
- [x] Every destructive call requires a live scoped provider session and active
      caller turn.
- [x] A detached background agent cannot create or mutate Cortex threads after
...
- `CortexHarnessPolicy` is the single source of truth. Any new provider must add
