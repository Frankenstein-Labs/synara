...
files:
  - url: Cortex-9.9.9-smoke.0-arm64.zip
    sha512: arm64zip
    size: 125621344
path: Cortex-9.9.9-smoke.0-arm64.zip
sha512: arm64zip
...
`,
  ...
  writeFileSync(
    ...
    `version: 9.9.9-smoke.0
files:
  - url: Cortex-9.9.9-smoke.0-x64.zip
    sha512: x64zip
    size: 132000112
path: Cortex-9.9.9-smoke.0-x64.zip
sha512: x64zip
...
function verifyCanonicalIdentity(): void {
  ...
  if (serverPackage.name !== "@cortex/cli") {
    throw new Error(`Expected CLI package @cortex/cli, got ${serverPackage.name ?? "<missing>"}.`);
  }
  const expectedBinaries = {
    cortex: "dist/index.mjs",
    "cortex-restore-migration-backup": "dist/restoreMigrationBackup.mjs",
  };
  if (JSON.stringify(serverPackage.bin ?? {}) !== JSON.stringify(expectedBinaries)) {
    throw new Error(
      "Expected the CLI to expose only the Cortex entry point and migration recovery binary.",
    );
  }
  if (CORTEX_PRODUCTION_BUNDLE_ID !== "com.emanueledipietro.cortex") {
    throw new Error(`Unexpected production bundle ID: ${CORTEX_PRODUCTION_BUNDLE_ID}.`);
  }
  if (CORTEX_DESKTOP_UPDATE_CHANNEL !== "cortex") {
    throw new Error(`Unexpected desktop update channel: ${CORTEX_DESKTOP_UPDATE_CHANNEL}.`);
  }
  ...
  if (
    ...
    throw new Error("Expected stable clean Cortex releases to publish on GitHub Latest.");
  }
}
...
function verifyReleaseWorkflowSafety(): void {
  ...
  assertContains(
    workflow,
    "needs.preflight.outputs.publish_release == 'true' && vars.CORTEX_PUBLISH_CLI == '1'",
    "Expected CLI publication to require explicit publication mode.",
  );
  assertContains(
    workflow,
    "needs.preflight.outputs.publish_release == 'true' && vars.CORTEX_FINALIZE_RELEASE == '1'",
    "Expected release finalization to require explicit publication mode.",
  );
  assertContains(
    workflow,
    "CORTEX_PUBLISH_RELEASE: ${{ needs.preflight.outputs.publish_release }}",
  ...
  assertContains(
    desktopBuildConfig,
    "__CORTEX_WINDOWS_UPDATER_PUBLISHER__",
...
const tempRoot = mkdtempSync(join(tmpdir(), "cortex-release-smoke-"));
...
try {
  ...
  assertContains(
    mergedManifest,
    "Cortex-9.9.9-smoke.0-arm64.zip",
    "Merged manifest is missing the arm64 asset.",
  );
  assertContains(
    mergedManifest,
    "Cortex-9.9.9-smoke.0-x64.zip",
