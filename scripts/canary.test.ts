...
describe("canary tooling", () => {
  it("keeps managed source and Canary data separate from Stable", () => {
    expect(resolveCanaryPaths({}, "/Users/tester")).toEqual({
      home: "/Users/tester/.cortex-canary",
      source: "/Users/tester/.cache/cortex-canary/source",
      state: "/Users/tester/.cortex-canary/canary-state.json",
      pid: "/Users/tester/.cortex-canary/canary.pid",
      log: "/Users/tester/.cortex-canary/canary.log",
    });
  });
  ...
  it("supports explicit path overrides", () => {
    expect(
      resolveCanaryPaths(
        {
          CORTEX_CANARY_HOME: "/tmp/canary-data",
          CORTEX_CANARY_SOURCE: "/tmp/canary-source",
        },
        "/Users/tester",
      ),
    ).toEqual({
  ...
  it("tracks main by default and accepts a stacked PR ref", () => {
    ...
    expect(parseCanaryArgs(["setup", "--ref", "codex/cortex-canary"])).toEqual({
      command: "setup",
      ref: "codex/cortex-canary",
    });
  });

  it("checks out the managed source during clone so the cleanliness guard starts clean", () => {
    expect(canaryCloneArgs("git@example.com:cortex.git", "/tmp/canary-source")).toEqual([
      "clone",
      "--",
      "git@example.com:cortex.git",
      "/tmp/canary-source",
    ]);
  });
  ...
  it("keeps updating the selected stacked ref until explicitly moved to main", () => {
    ...
    expect(resolveCanaryRef(parseCanaryArgs(["update"]), "codex/cortex-canary")).toBe(
      "codex/cortex-canary",
    );
