...
`,
  ...
  it("builds Electron framework symlink paths for the top-level app bundle", () => {
    assert.deepStrictEqual(buildMacUpdateZipSymlinkEntries("Cortex.app"), [
      "Cortex.app/Contents/Frameworks/Electron Framework.framework/Electron Framework",
      "Cortex.app/Contents/Frameworks/Electron Framework.framework/Helpers",
      "Cortex.app/Contents/Frameworks/Electron Framework.framework/Libraries",
      "Cortex.app/Contents/Frameworks/Electron Framework.framework/Resources",
      "Cortex.app/Contents/Frameworks/Electron Framework.framework/Versions/Current",
    ]);
  });
  ...
  it("resolves exactly one top-level .app from update zip entries", () => {
    assert.equal(
      resolveSingleTopLevelMacAppBundle([
        "__MACOSX/Cortex.app/Contents/Info.plist",
        "Cortex.app/Contents/Info.plist",
        "Cortex.app/Contents/MacOS/Cortex",
      ]),
      "Cortex.app",
    );
  ...
  it("resolves exactly one macOS update zip artifact", () => {
    assert.equal(
      resolveSingleMacUpdateZipFileName([
        "Cortex-0.1.5-arm64.dmg",
        "Cortex-0.1.5-arm64.zip",
        "latest-mac.yml",
      ]),
      "Cortex-0.1.5-arm64.zip",
    );
    ...
    assert.throws(
      () => resolveSingleMacUpdateZipFileName(["Cortex-0.1.5-arm64.zip", "Cortex-0.1.5-x64.zip"]),
  ...
  it("requires at least one macOS update manifest", () => {
    ...
    assert.throws(
      () => resolveMacUpdateManifestFileNames(["Cortex-0.1.5-arm64.dmg"]),
...
files:
  - url: Cortex-0.1.4-arm64.zip
    sha512: oldzip
    size: 100
  - url: Cortex-0.1.4-arm64.dmg
    sha512: olddmg
    size: 200
path: 'Cortex-0.1.4-arm64.zip'
sha512: oldzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;

    const updated = updateMacUpdateManifestZipEntry(manifest, "Cortex-0.1.4-arm64.zip", {
      sha512: "newzip",
      size: 12345,
    });
...
releaseDate: '2026-06-07T12:00:00.000Z'
`,
          "Cortex-0.1.4-arm64.zip",
          {
            sha512: "newzip",
            size: 12345,
          },
        ),
      /Could not update Cortex-0.1.4-arm64.zip entry/,
    );
  });
...
files:
  - url: Cortex-0.1.5-arm64.zip
    sha512: newzip
    size: 12345
path: Cortex-0.1.5-arm64.zip
sha512: newzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;
    ...
    assert.deepStrictEqual(
      validateMacUpdateManifestZipMetadata(manifest, "Cortex-0.1.5-arm64.zip", metadata),
      {
    ...
    assert.deepStrictEqual(
      assertMacUpdateManifestZipMetadata(manifest, "Cortex-0.1.5-arm64.zip", metadata),
      {
        manifestHasZipPath: true,
        manifestHasZipSha: true,
        manifestHasZipSize: true,
