# Cortex Code-Quality Branch — Complete Review Handoff
...
- Effect remains in Cortex for lifecycle, cancellation, queues, errors, and application policy.
...
| Commit              | Summary                                       |                                         Size |
| ------------------- | --------------------------------------------- | -------------------------------------------: |
| `0e5f6af9c`         | Refactor Cortex orchestration and web UI flow |                  387 files, +43,323 / -9,299 |
...
AcpSessionRuntime
  ...
  └─ Cortex: lifecycle, bounds, logging, policy, normalized events
        │
        ▼
...
| Workstream                              | Why it was needed                                                                   | Main change                                                             | Result to verify                                                       |
| --------------------------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `P2-ACP-01` ACP foundation              | Cortex maintained a custom wire stack beside the official SDK                       | Official SDK owns all production ACP wire behavior; custom wire deleted | Grok, Droid, and Cursor have one wire implementation and no fallback   |
...
Use the official TypeScript SDK for the protocol wire, while keeping Effect for Cortex's application
lifecycle.
...
| Provider process supervision                  | Cortex/Effect          |
| Queue and resource limits                     | Cortex/Effect          |
| Session/product policy                        | Cortex/Effect          |
| Normalized Cortex events                      | Cortex/Effect          |
| Error translation into Cortex domain errors   | Thin local adapter     |
...
3. Provider adapters should depend on a stable Cortex runtime interface, not choose a protocol
   implementation.
...
The package currently retains only generated/schema/error adapters still imported by Cortex. A
...
SQLite persistence, or the full Cortex app.
...
```text
Review the Cortex branch `codex/audit-code-quality` as a senior architecture, correctness,
