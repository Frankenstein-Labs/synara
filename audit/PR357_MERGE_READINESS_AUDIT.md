# Cortex PR #357 — Final code-quality, ACP, and provider audit

**Audit date:** 2026-07-15
**PR:** [#357 — Audit and harden code quality across desktop and server paths](https://github.com/Emanuele-web04/cortex/pull/357)
...
- after all tests, direct children of the Cortex server totalled about 1.84 GiB RSS, dominated by
  OpenCode;
...
as the sole protocol/wire authority and keep Effect above it as Cortex's lifecycle and application
...
- `AcpSessionRuntime.ts` constructs `OfficialAcp.client({ name: "cortex" })`, registers client
...
client/agent builders, handlers, streams, validation, and connection lifecycle that Cortex would
...
| Protocol upgrades              | Cortex had to regenerate and reconcile behavior | Follow the official SDK and negotiate protocol capabilities         |
| Process lifecycle              | Effect                                          | Effect, unchanged                                                   |
| Backpressure and queue budgets | Effect/custom transport                         | Cortex-owned bounded bridge around the official SDK                 |
| Canonical provider events      | Cortex adapters                                 | Cortex adapters, unchanged                                          |
...
- Closing the browser reduced the retained process from roughly 651 MiB to 135 MiB; stopping Cortex
...
- [ ] Do not recreate official SDK types in a new Cortex package.
