# SYN-47 — Cortex Studio (head-to-toe implementation plan)
...
Add a **"Studio"** experience to Cortex that is a near-exact clone of the existing **Chats / Threads**
...
- Studio chats are **normal AI threads** (full chat surface, any provider) — but their working directory is
  rooted at **`~/Documents/Cortex/Studio`**, so anything the agent writes lands there, e.g.
  `~/Documents/Cortex/Studio/Outbox/Content/2026-06-09_cortex_local_dev_server_x_posts.md`.
...
2. **Folders:** Mirror `~/Documents/Claude/*` structure under `~/Documents/Cortex/Studio`. Single shared
...
- A hidden project with `kind: "chat"` rooted at `chatWorkspaceRoot` (= `~/Documents/Cortex`) acts as the
...
- A hidden **Studio container** project (`kind: "studio"`) rooted at **`studioWorkspaceRoot`**
  (= `~/Documents/Cortex/Studio`). Lazily ensured by a new `ensureStudioProject`.
...
of `~/Documents/Cortex/Studio` with **no extra change** to this resolver. (Codex CLI/Claude SDK then run with
...
- No required change: `resolveThreadWorkspaceCwd` only special-cases `"chat"`. Confirm
  `@cortex/shared/threadWorkspace` helpers (`isWorkspaceRootWithin`, `workspaceRootsEqual`) are exported for
...
1. **`workspaceStore.ts`** — add `studioWorkspaceRoot: string | null` to state + a setter; extend
   `setServerWorkspacePaths` to accept/store it. **Bump the persist key** (`cortex:workspace-pages:v2` → `v3`)
...
- **Manual smoke (for the UI-refinement pass / verify skill):**
  ...
  3. Start a studio chat, ask it to write a file → it appears under `~/Documents/Cortex/Studio/Outbox/Content/…`.
...
- **cwd correctness:** verify a studio thread's resolved cwd is `~/Documents/Cortex/Studio` (not scratch/tmp).
...
3. A new studio chat is a real AI thread whose cwd is `~/Documents/Cortex/Studio`; agent-written files land
