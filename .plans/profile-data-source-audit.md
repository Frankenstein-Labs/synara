...
You are GPT-5.5 (xhigh) in the Cortex monorepo at `/Users/emanueledipietro/Developer/cortex`.
...
- `config.homeDir` = `OS.homedir()` ALWAYS (`apps/server/src/main.ts:172,217`) — it is NOT affected by
  `--home-dir`.
- `config.baseDir` = `--home-dir` / `CORTEX_HOME` (dev: `./.cortex/electron-dev`; prod: `~/.cortex` or
  `~/Documents/Cortex`). `config.dbPath` = `baseDir/{dev|userdata}/state.sqlite`.
...
- The dev runner sets `CORTEX_HOME/CORTEX_HOME/CORTEX_HOME=baseDir` but does NOT set `CODEX_HOME` or
...
5. **Run the checks** to back every claim:
   - `sqlite3` on `./.cortex/electron-dev/dev/state.sqlite` (dev) and `~/.cortex/userdata/state.sqlite`
     (prod) where relevant.
