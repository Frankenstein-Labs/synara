...
You are GPT-5.5 (xhigh) working in the Cortex monorepo at `/Users/emanueledipietro/Developer/cortex`.
...
- BRANCH dev DB (what the dev instance uses): `./.cortex/electron-dev/dev/state.sqlite`  (~40 turns)
- REAL app DB: `~/.cortex/userdata/state.sqlite`  (~437 turns, 192 MB) — the important one
...
2. `npx turbo run typecheck --filter=@cortex/contracts --filter=@cortex/cli` must pass. DO NOT run the web
   (`@cortex/web`) typecheck — its UI is being rewritten in parallel and will be red until Opus finishes.
