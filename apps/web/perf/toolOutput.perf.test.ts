...
// CORTEX_PERF=1 CORTEX_PERF_OUT=/tmp/tool-output.json bun run --cwd apps/web test perf/toolOutput.perf.test.ts
import { createHash } from "node:crypto";
import { writeFileSync } from "node:fs";
import os from "node:os";
import { MessageId } from "@cortex/contracts";
import { summarizeToolRawOutput } from "@cortex/shared/toolOutputSummary";
...
it.skipIf(process.env.CORTEX_PERF !== "1")(
  ...
  () => {
    ...
    writeFileSync(
      process.env.CORTEX_PERF_OUT ?? "/tmp/cortex-tool-output.json",
