...
//   CORTEX_PERF=1 bunx vitest run perf/clientHotPath.perf.test.ts
// Snapshot fixtures come from apps/server/perf/threadDetailSnapshot.perf.test.ts
// (/tmp/cortex-perf/snapshot-{p50,p90,max}.json).
...
const FIXTURE_DIR = process.env.CORTEX_PERF_DIR ?? "/tmp/cortex-perf";
const THREAD_COUNTS = [1, 4, 8];
const FLUSHES = Number(process.env.CORTEX_PERF_FLUSHES ?? 40);
...
describe.skipIf(!ENABLED)("client hot path perf", () => {
  it("measures snapshot decode/merge and per-flush reducer cost", () => {
    ...
    writeFileSync(
      process.env.CORTEX_PERF_OUT ?? `${FIXTURE_DIR}/client-report.json`,
