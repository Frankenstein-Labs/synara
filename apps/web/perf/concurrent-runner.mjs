    ...
    for (const mode of ["hidden", "visible"])
      for (const threads of paired ? [1, 5, 10] : [10]) {
        for (const variant of repeat % 2 ? [...variants].reverse() : variants) {
          ...
          await page.waitForFunction(() => window.__cortexConcurrentPerf);
          await page.waitForTimeout(2000);
          await page.evaluate(() => window.__cortexConcurrentPerf.run(10));
          ...
          const report = await page.evaluate(() => window.__cortexConcurrentPerf.run(60));
