...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.cortex/worktrees/cortex-mvp/cortex-4e609bb8",
    );
    expect(result).toBe("cortex-4e609bb8");
  });

  it("keeps legacy .cortex worktree paths readable after migration", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.cortex/worktrees/cortex-mvp/cortex-legacy123",
    );
    expect(result).toBe("cortex-legacy123");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.cortex\\worktrees\\cortex-mvp\\cortex-4e609bb8",
    );
    expect(result).toBe("cortex-4e609bb8");
  });

  it("uses the final segment even when outside ~/.cortex/worktrees", () => {
