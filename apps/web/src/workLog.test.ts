import { MessageId, TurnId, type OrchestrationThreadActivity } from "@cortex/contracts";
...
describe("deriveWorkLogEntries", () => {
  ...
  it("keeps created-automation milestones and exposes their card fields despite a null turn id", () => {
    const activities: OrchestrationThreadActivity[] = [
      ...
      makeActivity({
        ...
        summary: "Created automation: Watch Cortex PR 231 - Every 5m",
        tone: "info",
        payload: {
          source: "chat-composer",
          automationId: "automation-7",
          automationName: "Watch Cortex PR 231",
  ...
  it("exposes a provider-independent Cortex thread creation recap", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "cortex-created-threads",
        createdAt: "2026-02-23T00:00:05.000Z",
        turnId: "turn-1",
        kind: "cortex.threads.created",
        summary: "Created 2 Cortex threads",
        tone: "info",
        payload: {
    ...
    expect(entry?.cortexThreadCreation).toEqual({
      operationId: "gateway:create:two-workers",
      requestedCount: 2,
      createdCount: 2,
      threads: [
  ...
  it("hides repeated non-adjacent recovery rows for the same provider turn", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        summary: "Cortex recovered a stale running state",
        turnId: "turn-stale",
              ...
              commandActions: [
                {
                  ...
                  name: "README.md",
                  path: "/Users/emanueledipietro/Developer/Testing/cortex/README.md",
                },
              ],
            },
          },
        ...
        payload: {
          ...
          data: {
            item: {
              ...
              command: "/bin/zsh -lc 'git status --short'",
              cwd: "/Users/emanueledipietro/Developer/Testing/cortex",
              status: "completed",
              commandActions: [{ type: "unknown", command: "git status --short" }],
  ...
  it("preserves cancellation when an owning turn aborts", () => {
    const turnId = TurnId.makeUnsafe("turn-with-cancelled-cortex-tool");
    const entries = deriveWorkLogEntries(
      [
        makeActivity({
          id: "cancelled-cortex-start",
          createdAt: "2026-02-23T00:00:01.000Z",
          kind: "tool.started",
          summary: "Cortex create thread",
          turnId,
          payload: {
            itemType: "mcp_tool_call",
            title: "Cortex create thread",
            data: {
              toolCallId: "cancelled-cortex-call",
      ...
      [
        makeActivity({
          ...
          payload: {
            ...
            status: "interrupted",
            data: {
              toolCallId: "interrupted-cortex-call",
              toolName: "mcp__cortex__cortex_create_thread",
            },
          },
        }),
  ...
  it("presents Cortex MCP activity consistently across provider item shapes", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "cortex-mcp-create-thread-progress",
        kind: "tool.updated",
        ...
        payload: {
          ...
          title: "MCP tool call",
          data: {
            toolCallId: "cortex-mcp-create",
            toolName: "mcp__cortex__cortex_create_thread",
          },
        },
      }),
      makeActivity({
        id: "cortex-dynamic-send-message-progress",
        kind: "tool.updated",
        summary: "Tool call",
        payload: {
          itemType: "dynamic_tool_call",
          title: "Cortex__cortex_send_message",
          data: {
            toolCallId: "cortex-dynamic-send",
          },
        },
      }),
      makeActivity({
        id: "cortex-file-change-list-threads-progress",
        kind: "tool.updated",
        summary: "File change",
        payload: {
          itemType: "file_change",
          title: "mcp__Cortex__cortex_list_threads",
          data: {
            toolCallId: "cortex-file-change-list",
          },
        },
      }),
    ];
    ...
    expect(entries.map((entry) => [entry.itemType, entry.toolTitle])).toEqual(
      expect.arrayContaining([
        ["mcp_tool_call", "Cortex is creating a thread"],
        ["dynamic_tool_call", "Cortex is sending a message"],
        ["file_change", "Cortex is listing threads"],
      ]),
    );
    expect(entries).toHaveLength(3);
  });

  it("preserves a failed Cortex MCP result as a failed activity sentence", () => {
    const [entry] = deriveWorkLogEntries(
      [
        makeActivity({
          id: "cortex-create-threads-failed",
          kind: "tool.completed",
          summary: "cortex__cortex_create_threads",
          payload: {
            itemType: "mcp_tool_call",
            status: "failed",
            data: {
              toolCallId: "cortex-create-failed",
    ...
    expect(entry).toMatchObject({
      toolStatus: "failed",
      toolTitle: "Cortex couldn't create threads",
