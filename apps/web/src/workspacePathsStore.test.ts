...
describe("workspacePathsStore", () => {
  ...
  it("keeps chat workspace root while server config is still loading", async () => {
    ...
    useWorkspacePathsStore.getState().setChatWorkspaceRoot("/Users/tester/Documents/Cortex");
  ...
  it("keeps studio workspace root while server config is still loading", async () => {
    ...
    useWorkspacePathsStore
      .getState()
      .setStudioWorkspaceRoot("/Users/tester/Documents/Cortex/Studio");
    ...
    expect(useWorkspacePathsStore.getState().studioWorkspaceRoot).toBe(
      "/Users/tester/Documents/Cortex/Studio",
    );
  });
  ...
  it("migrates cached paths without retaining legacy workspace pages", async () => {
    ...
    localStorage.setItem(
      "cortex:workspace-pages:v2",
      JSON.stringify({
        state: {
          homeDir: "/Users/legacy",
          chatWorkspaceRoot: "/Users/legacy/Documents/Cortex",
    ...
    expect(localStorage.getItem("cortex:workspace-pages:v2")).toBeNull();
    expect(localStorage.getItem("cortex:workspace-paths:v1")).not.toBeNull();
  });
});
