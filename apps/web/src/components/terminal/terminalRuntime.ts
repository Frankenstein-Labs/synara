...
import { describeErrorMessage } from "@cortex/shared/errorMessages";
import {
  TERMINAL_MAX_COLS,
  TERMINAL_MAX_ROWS,
  TERMINAL_MIN_COLS,
...
import type { TerminalSessionSnapshot } from "@cortex/contracts";
...
const TERMINAL_PARKING_CONTAINER_ID = "cortex-terminal-parking";

type CortexTerminalOptions = NonNullable<ConstructorParameters<typeof Terminal>[0]> & {
...
const TERMINAL_CURSOR_STYLE: NonNullable<CortexTerminalOptions["cursorStyle"]> = "bar";
const TERMINAL_INACTIVE_CURSOR_STYLE: NonNullable<CortexTerminalOptions["cursorInactiveStyle"]> =
  "bar";
...
function syncTheme(entry: TerminalRuntimeEntry): void {
  ...
  const terminalOptions = entry.terminal.options as CortexTerminalOptions;
...
export function createRuntimeEntry(config: TerminalRuntimeConfig): TerminalRuntimeEntry {
  ...
  const terminalOptions: CortexTerminalOptions = {
    cursorBlink: true,
