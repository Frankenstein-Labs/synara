// FILE: CortexThreadCreationCard.tsx
// Purpose: End-of-turn recap for threads created through the Cortex MCP harness.
// Layer: Chat transcript UI

import { PROVIDER_DISPLAY_NAMES } from "@cortex/contracts";
import { formatModelDisplayName } from "@cortex/shared/model";
import { memo } from "react";

import type { WorkLogCortexThreadCreation } from "../../session-logic";
import { ProviderIcon } from "../ProviderIcon";
import { CortexLogo } from "../CortexLogo";
import { Button } from "../ui/button";

function threadMeta(thread: WorkLogCortexThreadCreation["threads"][number]): string {
...
export const CortexThreadCreationCard = memo(function CortexThreadCreationCard({
  creation,
  onOpenThread,
}: {
  readonly creation: WorkLogCortexThreadCreation;
  ...
  return (
    <div
      ...
      data-cortex-thread-creation-card="true"
    >
      <div className="flex min-w-0 items-center gap-3 px-3 py-2.5">
        <span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-[var(--color-background-elevated-secondary)] text-foreground">
          <CortexLogo className="h-[22px] w-auto" aria-label="Cortex" />
        </span>
