...
import { type MessageId, type TurnId } from "@cortex/contracts";
...
function workLogCortexThreadCreationsEqual(
  a: WorkLogEntry["cortexThreadCreation"],
  b: WorkLogEntry["cortexThreadCreation"],
) {
  if (a === b) return true;
  if (!a || !b) return false;
  if (
...
function workLogEntryContentEqual(a: WorkLogEntry, b: WorkLogEntry): boolean {
  return (
    ...
    workLogCortexThreadCreationsEqual(a.cortexThreadCreation, b.cortexThreadCreation) &&
