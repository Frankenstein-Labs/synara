...
import { CheckpointRef, MessageId, ThreadId, TurnId } from "@cortex/contracts";
...
describe("MessagesTimeline", () => {
  ...
  it("labels only the first message when another task created the conversation", async () => {
    ...
    expect(markup).toContain("Sent by Cortex from another thread");
    expect(markup).toContain('aria-label="Open source thread"');
    expect(markup.indexOf("Sent by Cortex from another thread")).toBeLessThan(
  ...
  it("shows the Cortex mark for every provider-specific tool row shape", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    ...
    const claudeMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          {
            id: "entry-inline-cortex-claude",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-cortex-claude",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "dynamic_tool_call",
              toolTitle: "Cortex__cortex_create_thread",
              toolName: "Cortex__cortex_create_thread",
              detail: "Cortex__cortex_create_thread",
    ...
    expect(claudeMarkup).toContain('data-tool-icon="cortex"');
    expect(claudeMarkup).not.toContain('data-tool-icon="mcp"');
    expect(claudeMarkup).toContain("Cortex is creating a thread");
    expect(claudeMarkup).not.toContain("Cortex__cortex_create_thread");
    ...
    const codexMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          {
            id: "entry-inline-cortex-codex",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-cortex-codex",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "file_change",
              toolTitle: "mcp__Cortex__cortex_list_threads",
              detail: "mcp__Cortex__cortex_list_threads",
            },
          },
        ]}
      />,
    );
    expect(codexMarkup).toContain('data-tool-icon="cortex"');
    expect(codexMarkup).toContain("Cortex listed threads");
    expect(codexMarkup).not.toContain("mcp__Cortex__cortex_list_threads");
    ...
    const failedMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          {
            id: "entry-inline-cortex-failed",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-cortex-failed",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "mcp_tool_call",
              toolName: "mcp__cortex__cortex_create_threads",
              toolStatus: "failed",
    ...
    expect(failedMarkup).toContain("Cortex couldn&#x27;t create threads");
    expect(failedMarkup).toContain("Claude rejected reasoningEffort");
  });

  // Browser calls get the globe rather than the generic Cortex mark: a browsing
  // row is about a page, and the surface it acted on is the first thing to read.
  it("uses the browser icon and action name for Cortex browser calls", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...makeTimelineBaseProps()}
        timelineEntries={[
          {
            id: "entry-inline-cortex-browser",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-cortex-browser",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "mcp_tool_call",
    ...
    expect(markup).not.toContain("Cortex: Browser Open");
  ...
  it("hides raw `ToolName: {json}` argument details behind the humanized heading", async () => {
    ...
    const readThreadMarkup = renderSingleToolRow({
      id: "work-cortex-read-thread-args",
      createdAt: "2026-03-17T19:12:28.000Z",
      label: "MCP tool call",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolName: "mcp__cortex__cortex_read_thread",
      detail: 'mcp__cortex__cortex_read_thread: {"threadId":"c357d8c5-b4c1-47d0"}',
      activityKind: "tool.completed",
    });
    expect(readThreadMarkup).toContain("Cortex read a thread");
    expect(readThreadMarkup).not.toContain("mcp__cortex__cortex_read_thread:");
    expect(readThreadMarkup).not.toContain("threadId");

    const diagnoseMarkup = renderSingleToolRow({
      id: "work-cortex-diagnose-args",
      createdAt: "2026-03-17T19:12:28.000Z",
      label: "MCP tool call",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolName: "mcp__cortex__cortex_diagnose_thread",
      detail: 'mcp__cortex__cortex_diagnose_thread: {"threadId":"09a1615d-084f-40b9"}',
      activityKind: "tool.completed",
    });
    expect(diagnoseMarkup).toContain("Cortex diagnosed a thread");
    expect(diagnoseMarkup).not.toContain("mcp__cortex__cortex_diagnose_thread:");
    ...
    const dynamicToolMarkup = renderSingleToolRow({
      ...
      detail: 'ToolSearch: {"query":"select:mcp__cortex__cortex_read_thread_events"}',
      activityKind: "tool.completed",
    });
    ...
    expect(failedArgsMarkup).toContain("Cortex couldn&#x27;t create threads");
    expect(failedArgsMarkup).toContain("Invalid params");
  });

  it("keeps Cortex tool calls and adds a thread creation recap at the end of the turn", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const assistantMessageId = MessageId.makeUnsafe("message-cortex-recap");
    const workEntries = [
      {
        id: "entry-cortex-create-tool",
        kind: "work",
        createdAt: "2026-03-17T19:12:28.000Z",
        entry: {
          id: "work-cortex-create-tool",
          createdAt: "2026-03-17T19:12:28.000Z",
          label: "MCP tool call",
          tone: "tool",
          itemType: "mcp_tool_call",
          toolName: "mcp__cortex__cortex_create_threads",
          toolTitle: "Cortex created threads",
          activityKind: "tool.completed",
        },
      },
      {
        id: "entry-cortex-create-recap",
        kind: "work",
        createdAt: "2026-03-17T19:12:29.000Z",
        entry: {
          id: "work-cortex-create-recap",
          createdAt: "2026-03-17T19:12:29.000Z",
          label: "Created 2 Cortex threads",
          tone: "info",
          activityKind: "cortex.threads.created",
          cortexThreadCreation: {
    ...
    expect(liveMarkup).toContain("Cortex created threads");
    expect(liveMarkup).not.toContain('data-cortex-thread-creation-card="true"');
    ...
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          ...workEntries,
          {
            id: "entry-cortex-recap-assistant",
            kind: "message",
            createdAt: "2026-03-17T19:12:30.000Z",
            message: {
              id: assistantMessageId,
    ...
    expect(markup).toContain("Worked for");
    expect(markup).toContain('data-cortex-thread-creation-card="true"');
    ...
    expect(markup.indexOf("Both threads are running.")).toBeLessThan(
      markup.indexOf('data-cortex-thread-creation-card="true"'),
    );
  });
          ...
          new Map([
            [
              assistantMessageId,
              {
                ...
                checkpointRef: CheckpointRef.makeUnsafe("refs/cortex/checkpoints/thread/turn/1"),
                status: "ready",
                completedAt: "2026-03-17T19:12:30.000Z",
                assistantMessageId,
                files: [
