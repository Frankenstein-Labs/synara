import { CheckpointRef, MessageId, OrchestrationProposedPlanId, TurnId } from "@cortex/contracts";
...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("preserves Cortex tool calls when a separate creation recap is present", () => {
    const createTool = workEntry(
      "cortex-create-tool",
      "2026-01-01T00:00:01Z",
      "Cortex created threads",
    );
    ...
    expect(collapsedSignature(messageRow(rows, "a1")!)).toEqual([
      "work:cortex-create-tool",
      "work:cortex-create-recap",
    ]);
  });
