...
describe("external MCP guided setup", () => {
  it("builds copyable Codex and Claude Code commands without embedding a credential", () => {
    ...
    expect(codex.value).toBe(
      "codex mcp add cortex --env ELECTRON_RUN_AS_NODE=1 -- /Applications/Cortex.app/Contents/MacOS/Cortex server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Cortex home'",
    );
    expect(claude.value).toBe(
      "claude mcp add --scope user cortex -e ELECTRON_RUN_AS_NODE=1 -- /Applications/Cortex.app/Contents/MacOS/Cortex server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Cortex home'",
    );
  ...
  it("builds standard JSON configuration for desktop and other clients", () => {
    ...
    const parsed = JSON.parse(desktop.value) as {
      mcpServers: { cortex: { command: string; args: ReadonlyArray<string> } };
    };

    expect(desktop.format).toBe("json");
    expect(parsed.mcpServers.cortex).toEqual(stdio);
  });
  ...
  it("builds a project-specific prompt without exposing implementation identifiers", () => {
    const prompt = buildExternalMcpExamplePrompt("Cortex app");

    expect(prompt).toContain('project named "Cortex app"');
  ...
  it("builds one agent-facing setup prompt covering pairing, registration, and verification", () => {
    const prompt = buildExternalMcpSetupPrompt({
      setupCommand: "cortex mcp pair --code syn_pair_v1_example --home-dir /tmp/home",
      stdio,
    });

    expect(prompt).toContain("syn_pair_v1_example");
    expect(prompt).toContain("codex mcp add cortex");
    expect(prompt).toContain("claude mcp add --scope user cortex");
    expect(prompt).toContain('"mcpServers"');
    expect(prompt).toContain("cortex_overview");
