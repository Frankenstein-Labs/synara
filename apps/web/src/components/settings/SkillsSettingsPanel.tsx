...
// catalog (~/.cortex/skills plus each provider's skills folder), shows which provider
...
import type { ProviderKind, ServerSettings } from "@cortex/contracts";
...
export function SkillsSettingsPanel() {
  ...
  const cortexSkillsDir = catalogQuery.data?.cortexSkillsDir;

  return (
    <div className="space-y-8">
      <SettingsSection title="Portable skills">
        <SettingsRow
          title="Cortex skills folder"
          description="Skills placed here are available on every provider. When a provider already ships its own copy of a skill, that copy is used; otherwise Cortex's copy is the fallback."
          status={
            cortexSkillsDir ? (
              <code className="break-all text-[11px] text-muted-foreground">{cortexSkillsDir}</code>
            ) : null
          }
          control={
      ...
      {catalogQuery.isError ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Cortex could not scan the skill folders. Retry after checking that the server is running."
          />
        </SettingsSection>
      ) : null}
      ...
      {!catalogQuery.isLoading && !catalogQuery.isError && totalSkills === 0 ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Add a skill folder containing a SKILL.md to the Cortex skills folder above, or install skills for any supported provider."
          />
        </SettingsSection>
      ) : null}
