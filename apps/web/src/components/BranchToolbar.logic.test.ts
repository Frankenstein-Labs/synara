import type { GitBranch } from "@cortex/contracts";
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.cortex/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a branch already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.cortex/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("keeps legacy .cortex worktree paths working for migrated threads", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.cortex/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("preserves associated worktree metadata during local branch-only syncs", () => {
    expect(
      resolveAssociatedWorktreeMetadataAfterWorkspacePatch({
        branch: "main",
        worktreePath: null,
        existingAssociatedWorktreePath: "/repo/.worktrees/cortex-pi",
        existingAssociatedWorktreeBranch: "cortex/pi",
        existingAssociatedWorktreeRef: "cortex/pi",
      }),
    ).toEqual({
      associatedWorktreePath: "/repo/.worktrees/cortex-pi",
      associatedWorktreeBranch: "cortex/pi",
      associatedWorktreeRef: "cortex/pi",
    });
  });
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        activeWorktreePath: "/repo/.cortex/worktrees/feature-a",
        branch: {
          isDefault: false,
          worktreePath: "/repo/.cortex/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.cortex/worktrees/feature-b",
      nextWorktreePath: "/repo/.cortex/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default branches", () => {
    expect(
      ...
      checkoutCwd: "/repo/.cortex/worktrees/feature-a",
      nextWorktreePath: "/repo/.cortex/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
