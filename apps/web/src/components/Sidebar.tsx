...
import { isGenericChatThreadTitle } from "@cortex/shared/chatThreads";
import { getDefaultModel } from "@cortex/shared/model";
import { pluralize } from "@cortex/shared/text";
import { resolveThreadWorkspaceCwd } from "@cortex/shared/threadEnvironment";
...
type DebugFeatureFlagsWindow = Window & {
  cortexShowFeatureFlags?: () => void;
  cortexHideFeatureFlags?: () => void;
};
...
const CORTEX_DOCS_URL = "https://trycortex.com/docs";
      ...
      <Menu>
        ...
        <ComposerPickerMenuPopup align="end" side="top" className="w-64 min-w-64">
          ...
          <MenuGroup>
            ...
            <MenuItem
              className={SIDEBAR_CONTEXT_MENU_ITEM_CLASS_NAME}
              onClick={() => openExternalLink(CORTEX_DOCS_URL)}
            >
 ...
 */
const ACTIVITY_ONBOARDING_STORAGE_KEY = "cortex:activity-onboarding:v1";
...
const SIDEBAR_SURFACE_PICKER_COPY: Record<SidebarView, { title: string; description: string }> = {
  threads: { title: "Cortex", description: "Build, debug, and ship" },
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    debugWindow.cortexShowFeatureFlags = showFeatureFlags;
    debugWindow.cortexHideFeatureFlags = hideFeatureFlags;
    window.addEventListener("storage", updateVisibility);
    updateVisibility();

    return () => {
      window.removeEventListener("storage", updateVisibility);
      if (debugWindow.cortexShowFeatureFlags === showFeatureFlags) {
        delete debugWindow.cortexShowFeatureFlags;
      }
      if (debugWindow.cortexHideFeatureFlags === hideFeatureFlags) {
        delete debugWindow.cortexHideFeatureFlags;
      }
    };
  }, []);
  ...
  // "Cortex" for project-less chats) with the hover cards and Activity rows.
  ...
  function renderProjectItem(
    ...
    // A project reads as "running" when Cortex tracks a run for it or when a
    // local server (possibly started outside Cortex) is attributed by cwd.
  ...
  const searchPaletteActions = useMemo<SidebarSearchAction[]>(
    () => [
      ...
      {
        id: "feedback",
        label: "Feedback Cortex",
        description: "Send feedback or report an issue to the Cortex team.",
        keywords: ["feedback", "bug", "issue", "problem", "report", "support", "cortex"],
      },
      {
        id: "settings",
        label: "Settings",
        ...
        .then((nextState) => {
          ...
          if (nextState.status === "available") {
            toastManager.add({
              ...
              description: `Cortex is preparing version ${nextState.availableVersion ?? "available"} in the background.`,
            });
            return;
          }
          ...
          if (nextState.status === "downloading") {
            toastManager.add({
              type: "info",
              title: "Preparing update",
              description: "Cortex is downloading the update in the background.",
            });
            return;
          }
          ...
          if (nextState.status === "up-to-date") {
            toastManager.add({
              ...
              description: `Cortex ${nextState.currentVersion} is already the newest version.`,
            });
            return;
          }
