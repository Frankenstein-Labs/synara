import type { GitStatusResult } from "@cortex/contracts";
...
describe("resolveDefaultCreateBranchName", () => {
  it("uses Cortex as the default namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "fix toast copy");
    assert.equal(branch, "cortex/fix-toast-copy");
  });

  it("normalizes an existing legacy cortex namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "cortex/refine-toolbar-actions");
    assert.equal(branch, "cortex/refine-toolbar-actions");
  });

  it("preserves nested namespaces under Cortex", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "feature/refine-toolbar-actions");
    assert.equal(branch, "cortex/feature/refine-toolbar-actions");
  });

  it("increments suffix when the Cortex branch already exists", () => {
    const branch = resolveDefaultCreateBranchName(
      ["main", "cortex/fix-toast-copy", "cortex/fix-toast-copy-2"],
      "fix toast copy",
    );
    assert.equal(branch, "cortex/fix-toast-copy-3");
  });

  it("falls back to cortex/update when no preferred name is provided", () => {
    const branch = resolveDefaultCreateBranchName(["main"]);
    assert.equal(branch, "cortex/update");
  });
});
...
describe("resolveLiveThreadBranchUpdate", () => {
  it("does not regress a semantic thread branch back to a temporary worktree branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      ...
      gitStatus: status({ branch: "cortex/deadbeef" }),
    });
...
describe("shouldOfferCreateBranchPrompt", () => {
  const temporaryBranch = "cortex/deadbeef";
