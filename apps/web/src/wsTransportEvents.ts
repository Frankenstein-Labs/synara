...
import type { WsCompatibilityError } from "@cortex/contracts";

export type WsTransportState = "connecting" | "open" | "closed" | "incompatible" | "disposed";

export const CORTEX_WS_TRANSPORT_STATE_EVENT = "cortex:ws-transport-state";
export const CORTEX_WS_COMPATIBILITY_ISSUE_EVENT = "cortex:ws-compatibility-issue";
...
export function emitWsTransportState(state: WsTransportState): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsTransportStateEventDetail>(CORTEX_WS_TRANSPORT_STATE_EVENT, {
      detail: { state },
    }),
  );
}
...
export function addWsTransportStateListener(
  ...
  window.addEventListener(CORTEX_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  return () => {
    window.removeEventListener(CORTEX_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  };
}
...
export function emitWsCompatibilityIssue(issue: WsCompatibilityError | null): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsCompatibilityIssueEventDetail>(CORTEX_WS_COMPATIBILITY_ISSUE_EVENT, {
      detail: { issue },
    }),
  );
}
...
export function addWsCompatibilityIssueListener(
  ...
  window.addEventListener(CORTEX_WS_COMPATIBILITY_ISSUE_EVENT, handleIssue);
  return () => {
    window.removeEventListener(CORTEX_WS_COMPATIBILITY_ISSUE_EVENT, handleIssue);
  };
}
