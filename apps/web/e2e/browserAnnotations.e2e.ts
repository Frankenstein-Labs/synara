...
import type { BrowserAnnotationEvent, BrowserAnnotationTheme } from "@cortex/contracts";
...
test("a real Electron guest commits and reprojects a continuous annotation session", async () => {
  const mainPath = process.env.CORTEX_E2E_ELECTRON_MAIN;
  ...
  const home = mkdtempSync(join(tmpdir(), "cortex-browser-annotations-e2e-"));
  ...
  const electronApp = await electron.launch({
    ...
    env: {
      ...
      HOME: home,
      CORTEX_HOME: home,
      CORTEX_BROWSER_HOST_PIPE_PATH: pipePath,
      CORTEX_BROWSER_HOST_CAPABILITY: capability,
      CORTEX_E2E_SHELL_PATH: shellPath,
      CORTEX_E2E_THREAD_ID: threadId,
      CORTEX_E2E_BROWSER_ANNOTATION_PRELOAD: annotationPreloadPath,
    },
  });
  ...
  try {
    ...
    const sendNativeInput = (event: Record<string, unknown>) =>
      electronApp.evaluate(
        (_electron, input) => {
          ...
          ).__cortexVisibleBrowserE2E.browserManager;
    ...
    const annotationEvents = async (): Promise<BrowserAnnotationEvent[]> =>
      electronApp.evaluate(() => {
        const fixture = (
          globalThis as typeof globalThis & {
            __cortexVisibleBrowserE2E: { annotationEvents: BrowserAnnotationEvent[] };
          }
        ).__cortexVisibleBrowserE2E;
