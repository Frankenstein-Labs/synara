...
import type { BrowserAnnotationEvent, ThreadBrowserState, ThreadId } from "@cortex/contracts";
...
const pipePath = process.env.CORTEX_BROWSER_HOST_PIPE_PATH;
const capability = process.env.CORTEX_BROWSER_HOST_CAPABILITY;
const shellPath = process.env.CORTEX_E2E_SHELL_PATH;
const threadId = process.env.CORTEX_E2E_THREAD_ID as ThreadId | undefined;
const cortexHome = process.env.CORTEX_HOME;
const annotationPreloadPath = process.env.CORTEX_E2E_BROWSER_ANNOTATION_PRELOAD;

if (!pipePath || !capability || !shellPath || !threadId || !cortexHome || !annotationPreloadPath) {
  throw new Error("The visible-browser Electron fixture requires its isolated E2E environment.");
}

app.setPath("userData", path.join(cortexHome, "electron-userdata"));
...
function setPanelVisible(visible: boolean): void {
  ...
  pushState();
  mainWindow?.webContents.send("cortex-e2e:open-panel");
}
function pushState(): void {
  if (shellReady && latestState && mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send("cortex-e2e:browser-state", latestState);
  }
}
...
ipcMain.on("cortex-e2e:shell-ready", () => {
  shellReady = true;
  pushState();
});

ipcMain.handle(
  "cortex-e2e:attach-webview",
