...
import { BROWSER_TOOL_NAMES } from "@cortex/contracts";
...
test("production MCP controls one persistent Electron page across visibility changes", async () => {
  ...
  const home = mkdtempSync(join(tmpdir(), "cortex-visible-browser-e2e-"));
  ...
  try {
    ...
    await electronApp.evaluate(() => {
      (
        globalThis as typeof globalThis & {
          __cortexVisibleBrowserE2E: { setPanelRevealEnabled(enabled: boolean): void };
        }
      ).__cortexVisibleBrowserE2E.setPanelRevealEnabled(false);
    });
    ...
    await electronApp.evaluate(() => {
      ...
      ).__cortexVisibleBrowserE2E.setPanelRevealEnabled(true);
    });
    ...
    const runtimeCount = await electronApp.evaluate(() => {
      const manager = (
        globalThis as typeof globalThis & {
          __cortexVisibleBrowserE2E: { browserManager: { runtimes: Map<string, unknown> } };
        }
