...
it("does not expose WebMCP on insecure or permissions-policy-blocked documents", () => {
  const root = globalThis as typeof globalThis & { readonly __cortexWebMcpBridgeV1?: unknown };
  ...
  expect(root.__cortexWebMcpBridgeV1).toBeUndefined();
...
it("uses the host-validated response policy when Electron does not recognize tools", () => {
  ...
  expect(root.__cortexWebMcpBridgeV1).toBeDefined();
