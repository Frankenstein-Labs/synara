...
/**
 * Install the page-facing WebMCP compatibility API and Cortex's private bridge
...
export function installWebMcpBridgeInMainWorld(hostAllowsCompatibility = false): void {
  ...
  const BRIDGE_PROPERTY = "__cortexWebMcpBridgeV1";
  ...
  const safeError = (error: unknown): { readonly name: string; readonly message: string } => {
    ...
    try {
      ...
      // let that hostile error value escape Cortex's bounded error envelope.
    }
    return {
  ...
  const declarativeForm = Symbol("cortex.webmcp.declarative-form");
  ...
  const normalizeTool = async (
    ...
    const descriptor = {
      ...
      annotations: {
        readOnlyHint: tool.annotations?.readOnlyHint === true,
        // All page-provided metadata and results are untrusted to Cortex even
  ...
  const bridge = Object.freeze({
    ...
    async invoke(index: number, signature: string, inputJson: string, invocationId: string) {
      ...
      try {
        ...
        // so Cortex remains compatible with early-preview Chromium builds.
