...
import { MIGRATION_RECOVERY_MAX_RESUME_ATTEMPTS } from "@cortex/shared/migrationRecovery";
...
describe("desktop migration recovery", () => {
  it("offers safe updater escape hatches when structured recovery details are invalid", () => {
    expect(
      ...
      { label: "Update Cortex and restart", decision: "install-update" },
  ...
  it("targets the same production database and bundled restore authority as the server", () => {
    expect(
      resolveDesktopMigrationRecoveryPaths({
        baseDir: Path.join(Path.sep, "home", "cortex"),
        appRoot: Path.join(Path.sep, "app"),
        isDevelopment: false,
      }),
    ).toEqual({
      dbPath: Path.join(Path.sep, "home", "cortex", "userdata", "state.sqlite"),
  ...
  it("uses the isolated development database when the desktop backend receives a dev URL", () => {
    ...
    expect(paths.dbPath).toBe(Path.join(Path.sep, "home", "cortex", "dev", "state.sqlite"));
  });

  it("continues only when the server-owned command clears the durable marker", async () => {
    const directory = await FS.mkdtemp(Path.join(OS.tmpdir(), "cortex-desktop-recovery-"));
  ...
  it("verifies that completed provenance records the exact restored backup", async () => {
    const directory = await FS.mkdtemp(Path.join(OS.tmpdir(), "cortex-desktop-provenance-"));
    const paths = resolveDesktopMigrationRecoveryPaths({
      baseDir: directory,
      appRoot: directory,
      isDevelopment: false,
  ...
  it("relaunches only after the exact completed restore is verified", async () => {
    const directory = await FS.mkdtemp(Path.join(OS.tmpdir(), "cortex-desktop-relaunch-"));
    const paths = resolveDesktopMigrationRecoveryPaths({
      baseDir: directory,
      appRoot: directory,
      isDevelopment: false,
...
describe("requiresDesktopMigrationRecovery", () => {
  async function withMarker(
    ...
    const directory = await FS.mkdtemp(Path.join(OS.tmpdir(), "cortex-recovery-gate-"));
    try {
      const paths = resolveDesktopMigrationRecoveryPaths({
        baseDir: directory,
        appRoot: directory,
