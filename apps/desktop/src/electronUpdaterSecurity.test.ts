...
describe("electronUpdaterSecurity", () => {
  ...
  it("parses distinguished names the same way as builder-util-runtime", () => {
    const parsed = parseDistinguishedName('CN=Cortex, O="Acme, Inc.", OU=Tools\\2C Desktop');

    expect(parsed.get("CN")).toBe("Cortex");
  ...
  it("uses only embedded full publisher DNs and never feed-controlled names", () => {
    expect(
      resolveWindowsUpdatePublisherNames(
        ...
        [" CN=Cortex, O=Acme Tools ", "CN=Only", ""],
      ),
    ).toEqual(["CN=Cortex, O=Acme Tools"]);
  ...
  it("validates a matching full distinguished name through the injected execFile seam", async () => {
    const execFile = vi.fn((file, args, options, callback) => {
      callback(
        ...
        JSON.stringify({
          Status: 0,
          Path: "C:\\Users\\test\\AppData\\Local\\Temp\\CortexSetup.exe",
          SignerCertificate: {
            Subject: "CN=Cortex, O=Acme Tools",
          },
        }),
        "",
      );
    ...
    const result = await verifyWindowsUpdateCodeSignature(
      ["CN=Cortex, O=Acme Tools"],
      "C:\\Users\\test\\AppData\\Local\\Temp\\CortexSetup.exe",
  ...
  it("rejects a CN-only publisher allowlist", async () => {
      ...
      logger,
      {
        ...
        execFile: vi.fn((_file, _args, _options, callback) => {
          callback(
            ...
            JSON.stringify({
              Status: 0,
              Path: "C:\\Temp\\CortexSetup.exe",
              SignerCertificate: { Subject: "CN=Cortex, O=Acme Tools" },
            }),
            "",
          );
        }),
    ...
    expect(result).toContain("publisherNames: CN=Cortex");
  ...
  it("returns a mismatch summary for an unexpected publisher", async () => {
    ...
    expect(result).toContain("publisherNames: CN=Cortex, O=Acme Tools");
  ...
  it("patches electron-updater BaseUpdater spawnSyncLog only on Windows", () => {
    ...
    expect(prototype.__cortexSpawnSyncLogPatched).toBe(true);
  });
