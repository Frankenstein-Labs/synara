...
function makeIsolatedServerEnvironment(input: {
  ...
  return {
    ...environment,
    CORTEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "0",
    CORTEX_CLAUDE_KEEPALIVE: "0",
    CORTEX_DESKTOP_SHUTDOWN_TOKEN: input.shutdownToken,
    CORTEX_HOME: input.homeDir,
    CORTEX_HOST: "127.0.0.1",
    CORTEX_MODE: "desktop",
    CORTEX_NO_BROWSER: "1",
    CORTEX_PORT: String(input.port),
  };
}
...
describeWindows("Windows desktop backend shutdown integration", () => {
  ...
  it("authenticates a real server, deduplicates shutdown, drains its runtime, and clears its finalizer artifact", async () => {
    ...
    try {
      const homeDir = await resources.makeTempDir("cortex-windows-shutdown-");
