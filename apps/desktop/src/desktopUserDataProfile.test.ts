...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "cortex-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  it("resolves the canonical Cortex profile names", () => {
    const appDataBase = "/Users/tester/Library/Application Support";
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "cortex-dev" })).toBe(
      "/Users/tester/Library/Application Support/cortex-dev",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "cortex" })).toBe(
      "/Users/tester/Library/Application Support/cortex",
    );
    expect(
      resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "cortex-canary" }),
    ).toBe("/Users/tester/Library/Application Support/cortex-canary");
  });
  ...
  it("uses an explicit smoke profile instead of the development profile", () => {
    expect(
      resolveDesktopUserDataPath({
        ...
        testOverridePath: "/tmp/cortex-desktop-smoke/electron-user-data",
      }),
    ).toBe("/tmp/cortex-desktop-smoke/electron-user-data");
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPath = Path.join(appDataBase, "cortex");
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "cortex-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "cortex-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the Cortex profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing Cortex database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "cortex");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".cortex-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "cortex");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "cortex-profile-seed.json"), "{");
