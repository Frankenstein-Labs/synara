...
function cleanupStaleDevApps() {
  ...
  const commandPattern = `^${executable}[[:space:]]+--cortex-dev-root=${devRoot}([[:space:]]|$)`;
  spawnSync("pkill", ["-f", "--", commandPattern], { stdio: "ignore" });
}

function listStaleComputerUsePids() {
  // Only macOS exposes a verifiable Cortex (Dev) executable path for these
  ...
  return candidatePids.filter((pid) => {
    ...
    if (!/Cortex \(Dev\)\.app\/Contents\/MacOS\/Electron/.test(command)) {
      return false;
    }
...
function cleanupStaleComputerUseApps() {
  ...
  console.error(
    `[desktop-dev] Cleaning up ${stalePids.length} stale Cortex (Dev) Computer Use helper process${stalePids.length === 1 ? "" : "es"} from other worktrees.`,
  );
...
function warnIfAlphaAppRunning() {
  ...
  const pids = listPidsByExactProcessName("Cortex").filter((pid) =>
    readProcessCommand(pid).startsWith("/Applications/Cortex.app/Contents/MacOS/Cortex"),
  );
  if (pids.length === 0) {
    return;
  }
  ...
  console.error(
    "[desktop-dev] Cortex is still running. Close it before testing voice in Cortex (Dev), or you may be looking at the wrong app/runtime.",
  );
  console.error(`[desktop-dev] Running Cortex process IDs: ${pids.join(", ")}`);
}
...
function startApp() {
  ...
  const app = spawn(
    resolveElectronPath(),
    [`--cortex-dev-root=${desktopDir}`, "dist-electron/main.js"],
    {
      cwd: desktopDir,
      env: {
        ...childEnv,
