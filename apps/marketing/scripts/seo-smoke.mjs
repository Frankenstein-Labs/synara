...
try {
  ...
  assert.match(
    homepage,
    /<title>Cortex — AI Coding Workspace for Claude Code, Codex &amp; Cursor<\/title>/,
  );
  assert.match(
    homepage,
    /<link rel="canonical" href="https:\/\/www\.trycortex\.com"\/>/,
  ...
  assert.ok(install.includes("Download Cortex"));
  assert.ok(install.includes("coding-agent runtime already authenticated"));
  assert.match(install, /<title>Download Cortex — Coding Agent Workspace<\/title>/);

  const docs = await readRoute("/docs", /text\/html/i);
  assert.match(
    docs,
    /<link rel="canonical" href="https:\/\/www\.trycortex\.com\/docs"/,
  ...
  assert.ok(docsIndexMarkdown.includes("# Cortex documentation"));
  assert.ok(docsIndexMarkdown.includes("## What is Cortex?"));
  ...
  assert.equal(
    ...
    '<https://www.trycortex.com/docs/providers/claude-code>; rel="canonical"',
  );
  for (const marker of [
    "# Claude Code",
    "Canonical URL: https://www.trycortex.com/docs/providers/claude-code",
    "## Install",
    "claude --version",
  ]) {
  ...
  for (const marker of [
    ...
    "User-Agent: GPTBot",
    "Disallow: /api/",
    "Sitemap: https://www.trycortex.com/sitemap-index.xml",
  ]) {
  ...
  assert.ok(sitemapIndex.includes("https://www.trycortex.com/sitemap.xml"));
  assert.ok(sitemapIndex.includes("https://www.trycortex.com/changelog/sitemap.xml"));
  ...
  for (const route of [
    ...
    assert.ok(
      sitemap.includes(`https://www.trycortex.com${route}`),
      `sitemap.xml is missing ${route}`,
    );
  }
  for (const utilityPath of ["/llms.txt", "/llms-full.txt", "/ai.txt"]) {
    assert.equal(
      sitemap.includes(`https://www.trycortex.com${utilityPath}`),
      false,
  ...
  for (const marker of [
    ...
    "## Documentation index",
    "https://www.trycortex.com/docs/providers.md",
    "https://www.trycortex.com/docs/workflows.md",
    "https://www.trycortex.com/docs/troubleshooting.md",
  ...
  assert.ok(
    llmsFull.includes(
      "Canonical URL: https://www.trycortex.com/docs/troubleshooting/report-a-problem",
    ),
  );
  ...
  assert.ok(ai.includes("Markdown documentation: https://www.trycortex.com/docs.md"));
  assert.ok(
