...
export async function generateMetadata({ params }: { params: Promise<{ version: string }> }) {
  ...
  return pageMetadata({
    title: `Cortex ${entry.version} — Changelog`,
    description: `What's new in Cortex ${entry.version} (${entry.date}): ${highlights}.`,
...
export default async function ChangelogVersionPage({
  ...
  return (
    <>
      ...
      <ChangelogContent
        releases={[entry]}
        title={`Cortex ${entry.version} release notes.`}
        description={`What changed in Cortex ${entry.version} (${entry.date}), including ${entry.features
