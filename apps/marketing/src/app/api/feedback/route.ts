...
// Purpose: Receives explicit Cortex feedback and delivers it to the maintainer through Resend.
...
const DEFAULT_FROM_EMAIL = "Cortex Feedback <feedback@trycortex.com>";
...
function corsOrigin(request: Request): string | null {
  ...
  if (origin === "cortex://app") return origin;
  ...
  return {
    ...
    ...(origin
      ? {
          ...
          "access-control-allow-headers": "content-type, x-cortex-feedback",
        }
      : {}),
  };
}
...
async function sendEmail(feedback: ParsedFeedback): Promise<string> {
  ...
  const toEmail = process.env.CORTEX_FEEDBACK_TO_EMAIL?.trim();
  ...
  try {
    const response = await fetch(RESEND_API_URL, {
      ...
      body: JSON.stringify({
        from: process.env.CORTEX_FEEDBACK_FROM_EMAIL?.trim() || DEFAULT_FROM_EMAIL,
        to: [toEmail],
        subject: `[Cortex Feedback] ${categoryLabel} · v${String(feedback.diagnostics.appVersion)}`,
        text,
        html,
      }),
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (request.headers.get("x-cortex-feedback") !== "1") {
