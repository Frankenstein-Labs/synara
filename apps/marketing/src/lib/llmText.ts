...
const PRIMARY_PAGES = [
  ["Homepage", `${SITE_URL}/`],
  ["Download Cortex", `${SITE_URL}/install`],
  ["Cortex documentation", `${SITE_URL}/docs`],
  ["Changelog", `${SITE_URL}/changelog`],
  ["Sponsor Cortex", `${SITE_URL}/sponsor`],
  ["Cortex sponsors", `${SITE_URL}/sponsors`],
...
export function buildLlmsTxt() {
  ...
  return [
    ...
    "- How do I install Cortex and connect a coding-agent runtime?",
    ...
    "- What evidence should I collect before reporting a Cortex problem?",
    "",
    "## Recent releases",
    ...releases.map(
      (entry) =>
        `- [Cortex ${entry.version}](${SITE_URL}/changelog/${toVersionSlug(entry.version)}): ${entry.features
...
export async function buildLlmsFullTxt() {
  ...
  return [
    ...
    ...releases.flatMap((entry) => [
      `### Cortex ${entry.version} (${entry.date})`,
...
export function buildAiTxt() {
  return [
    ...
    AI_DISCOVERY_NOTICE,
    "",
    "Purpose: help search, answer, and browser agents identify canonical public Cortex pages.",
    "",
    ...
    "- Cortex is local-first and does not require a Cortex cloud account.",
    ...
    "- Cortex does not proxy or store normal provider traffic on a Cortex server.",
