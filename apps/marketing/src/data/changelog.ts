...
// Purpose: Curated Cortex release notes powering the public /changelog page.
// Layer: static data (server-importable). Mirrors the in-app "What's new"
//        changelog from the Cortex desktop app, newest release first.
...
export const CHANGELOG_ENTRIES: readonly ChangelogEntry[] = [
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Use /rename with a title, or let Cortex generate a title from the conversation.",
        details:
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        details:
          "A current in-progress tool call now receives its own one-hour idle budget instead of sharing the ordinary 30-minute turn watchdog, with stale events prevented from refreshing the active clock. Cortex also normalizes Devin's unexpected boolean `get_output.block` field without weakening other ACP messages, and project creation waits for a real task before navigation so superseded routes cannot overwrite newer work.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        details:
          "Disabling a provider now prevents server lifecycle execution instead of only hiding it in the interface. Usage is available for every enabled provider Cortex can verify, provider-driven context changes appear in the conversation, routine lifecycle hooks stay out of the transcript, and update notices recover when availability checks overlap or temporarily fail.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Cortex now shows every chat still working before the desktop app closes and can continue eligible work on the next launch.",
        details:
          "The quit dialog lists live chats and remembers whether automatic resume is enabled. When you confirm, Cortex records the exact in-flight turns before interrupting them, then starts one guarded continuation per unchanged chat after restart. Completed, archived, deleted, replaced, or otherwise advanced work is skipped, and a bounded fallback still lets the app quit if the local server cannot acknowledge the resume record.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Provider usage is no longer limited to Codex and Claude; Settings and in-context meters now cover every authenticated runtime Cortex can verify.",
        details:
          "Cortex adds account or quota views for Antigravity, Cursor, Grok, OpenCode, and locally authenticated providers, while keeping Codex and Claude's detailed windows. Provider-specific adapters use local credentials or documented account endpoints, share cached snapshots, retain useful stale data through transient failures, apply bounded cooldowns after rate limiting, and explain when a provider exposes sign-in state but no machine-readable quota.",
      },
      {
      ...
      },
      {
        ...
        details:
          "On Windows, a project opened through \\\\wsl.localhost or \\\\wsl$ launches provider commands inside the selected distribution with a Linux cwd, while local-folder mentions and the project browser recognize UNC and Windows home paths. Runtime icon changes now refresh the shell-visible taskbar icon. Windows and Linux users can also switch between Cortex's custom title bar and the system title bar from Appearance, with an explicit restart to apply the frame change.",
      },
      {
        id: "headless-server-distribution",
        title: "Run and inspect Cortex without the desktop shell",
        description:
        ...
        details:
          "The release pipeline builds cortex-server-<version>.tar.gz from the same source and web client as the desktop release. The new `cortex server status` command discovers the persisted local runtime or accepts an explicit HTTP(S) URL, verifies the runtime identity, probes `/health`, reports projection readiness, supports JSON output, and exits non-zero when the server is unreachable or not ready.",
      },
      {
      ...
      },
      {
        ...
        details:
          "Duplicate approval responses are rejected durably at the serialized decider. Permanent deletion reclaims only Cortex-owned managed worktrees and preserves unowned paths. Malformed cached feature flags reset instead of leaving stale values, route restoration ignores snapshots from superseded refreshes, and large projection repair stops thrashing when the state is already being repaired.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "The new iOS Simulator pane gives you and supported agents one live, interactive device surface inside Cortex.",
        details:
          "On macOS, Cortex can boot and attach simulators, stream their display, install and launch apps, tap, swipe, type, press hardware controls, save screenshots, record the view, and inspect accessibility elements. The source-shipped helper compiles with your selected Xcode, runs in a constrained sandbox, drops slow frames instead of blocking RPC traffic, and reclaims Cortex-owned devices after crashes.",
      },
      {
      ...
      },
      {
        ...
        details:
          "Chat width is persisted as a visual preference, while Markdown file links can copy their path, open in Cortex, or reveal supported workspace references. File and snippet search use the same direct-to-file workflow for a more consistent navigation path.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Cortex now keeps waiting for a healthy backend and recovers late connections instead of giving up while a large history is still loading.",
        details:
      ...
      },
      {
        ...
        description:
          "Workspace previews are now useful for small code and text edits without leaving Cortex.",
        details:
    ...
    features: [
      {
        ...
        title:
          "A review of the Cortex codebase found an analytics configuration that came from the original T3 Code codebase when Cortex was created as a clone in March.",
        description:
          "We did not add it, and we have no access to the PostHog project receiving the events.",
        details:
          "The configuration has been removed. Cortex no longer sends remote product analytics. The events did not include prompts, source code, filenames, or file contents. We're sorry this wasn't caught earlier.",
      },
    ],
  },
  {
    ...
    features: [
      {
        ...
        description:
          "Create a project from a GitHub URL or repository name and let Cortex prepare the local checkout for you.",
        details:
      ...
      },
      {
        ...
        description:
          "Completion alerts stay quiet while Cortex is in front of you, and provider update notices refresh more reliably when you return.",
        details:
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        details:
          "Human interactions are now fenced to the owning provider request and turn across Claude and OpenCode. Resolved guards remain resolved, stale cross-turn requests are ignored, retryable interactions stay visible, and OpenCode permission replies wait for a real acknowledgement before Cortex clears them.",
      },
      {
        id: "provider-skills-and-models",
        title: "Installed skills and favourite models stay discoverable",
        description:
          "Cortex now finds installed Claude plugin skills and keeps similarly named favourite models tied to the correct provider.",
        details:
      ...
      },
      {
        ...
        description:
          "Cortex no longer treats interrupted or errored work as a successful completion or repeats an alert as timestamps change.",
        details:
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Cortex negotiates one authenticated connection, compresses large traffic, and resumes thread detail from a cursor when possible.",
        details:
    ...
    features: [
      {
        ...
        description:
          "Interrupt and stop actions now take priority over new work, even when a busy Cortex server has filled its ordinary command queue.",
        details:
      ...
      },
      {
        ...
        details:
          "Cortex retains enough turn identity to settle late Claude results, fences stale lifecycle generations, reconciles durable provider commands and runtime events, and aligns Codex, Claude, Cursor, and ACP session ownership through start, stop, reconnect, and restart boundaries.",
      },
      {
    ...
    features: [
      {
        ...
        details:
          "Cortex now normalizes live and settled tool activity into one presentation model, preserves expandable tool details and interactions, and reconciles terminal states without leaving duplicate or permanently running work rows behind.",
      },
      {
      ...
      },
      {
        ...
        details:
          "Cortex abandons ambiguous blockers oldest-first, then replays only the skipped turn starts. This restores the conversation without risking a duplicate resend of the command whose delivery could not be proven.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Cortex loads expensive provider and diff machinery only when needed and computes working-tree statistics without transferring full patches.",
        details:
      ...
      },
      {
        ...
        details:
          "Database access now proves exclusive ownership, migration backups and resumable artifacts receive broader retention and reclamation coverage, and orphan cleanup stays bounded to verified Cortex-owned paths.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Cortex now detects and repairs the database state that could leave some 0.6.0 installations stuck during startup.",
        details:
  ...
  },
  {
    ...
    features: [
      {
        id: "external-cortex-mcp",
        title: "Bring Cortex to any MCP-capable agent",
        description:
          "Connect Codex, Claude Code, Claude Desktop, or another local MCP app, then let it discover your Cortex workspace, launch isolated tasks, wait for results, and bring the answer back.",
        details:
      ...
      {
        id: "built-in-cortex-mcp",
        title: "Cortex's agents can now operate Cortex",
        description:
          "Every supported agent running inside Cortex receives built-in tools to understand the app, delegate work, coordinate parallel tasks, inspect failures, and manage automations.",
        details:
          "The new Cortex Agent Gateway can list and read projects and tasks, create one task or an exact multi-agent batch across providers and models, wait for every result, continue or interrupt work, rename or archive tasks, inspect runtime diagnostics, and manage automation lifecycles. Thread-bound authority, privilege caps, idempotent creation, isolated worktrees, and restart recovery keep delegated work visible and contained.",
      },
      {
        id: "project-spaces",
      ...
      },
      {
        ...
        description:
          "Follow Claude's native subagents and dynamic workflows as real Cortex work, with live status, phases, tools, usage, steering, and background controls.",
        details:
      ...
      },
      {
        ...
        description:
          "Mention an existing Cortex task from the composer to give the current agent the right recent context without copying a transcript by hand.",
        details:
      ...
      },
      {
        ...
        details:
          "Cortex prefetches provider models before the composer opens, avoids a redundant first-turn Claude permission wait, prepares Codex overlays without blocking, parallelizes independent turn-start I/O, reduces streaming SQL work, and expands React Compiler coverage across the web app.",
      },
      {
        ...
        details:
          "Highlights include Fable 5 and Opus 4.8 in Pi, namespaced Cursor and Grok model support, accurate Claude context windows, official ACP SDK handling, app-owned OpenCode review commands, isolated Codex child events, safer provider updates, preserved blank PATH defaults, and an Antigravity hook that no longer launches Cortex unexpectedly.",
      },
      {
    ...
    features: [
      {
        ...
        title: "Antigravity joins Cortex",
        description:
        ...
        details:
          "The new provider adapter covers installation and authentication guidance, model and effort discovery, session creation and resume, tool and plan events, permission requests, usage reporting, cancellation, and restart recovery. Cortex also includes dedicated Antigravity branding throughout provider setup and selection.",
      },
      {
      ...
      },
      {
        ...
        details:
          "Model-catalog queries retain successful and cached data when one Cursor discovery path fails, while pull-request data is coalesced through shared list logic and picker popups use a unified interaction model. Diff headers now use Cortex's own visual chrome for a more consistent workspace.",
      },
    ],
  },
  {
    ...
    features: [
      {
        ...
        title: "Review pull requests without leaving Cortex",
        description:
      ...
      },
      {
        ...
        description:
          "Cortex restores the previous desktop window size, position, and maximized state while keeping reopened windows on a visible display.",
        details:
    ...
    features: [
      {
        ...
        description:
          "Press both Option keys to capture the window you are using and attach it to the current Cortex task.",
        details:
    ...
    features: [
      {
        ...
        description:
          "Droid is now available alongside Cortex's other agents, with runtime model discovery, session import, token multipliers, and resilient resume and recovery.",
        details:
          "Cortex now connects to Factory Droid through ACP, discovers models and their switching capabilities at runtime, imports existing Droid sessions, carries context across forks and restarts, and keeps bootstrap, configuration, and turn teardown state coherent. The release also adds the Factory logo and richer Droid token reporting.",
      },
      {
        id: "large-history-startup",
        title: "Large conversation histories start reliably",
        description:
          "Cortex now upgrades large local histories without leaving the project list stuck on its loading screen.",
        details:
  ...
  },
  {
    ...
    features: [
      {
        id: "cortex-identity",
        title: "Cortex, all the way through",
        description:
        ...
        details:
          "The desktop bundle is now com.emanueledipietro.cortex, the CLI is @cortex/cli with the cortex command, and every first-party runtime identifier uses the Cortex namespace. The 0.4.2 bridge preserves renderer state during the origin change.",
      },
      {
  ...
  },
  {
    ...
    features: [
      {
        id: "cortex-identity-bridge",
        title: "Cortex is preparing a seamless identity upgrade",
        description:
          "Launch this version at least once before installing the next Cortex release so your drafts, pins, theme, browser state, and other local interface preferences move with you.",
        details:
          "This bridge writes a validated Cortex-only snapshot of renderer state, keeps existing project and thread data intact, and prepares database and checkpoint metadata for the final identity cutover. Earlier command and environment aliases are accepted by this bridge release only and will be removed next.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Cortex checks that an update actually installed after restart, detects failed installs, and recovers instead of silently staying on the old version.",
        details:
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Cortex is more careful about restoring the right destination after reloads, segment switches, reconnects, and partially completed project creation.",
        details:
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "When Claude reports usage or rate-limit trouble, Cortex now presents the condition more calmly instead of turning it into a generic provider failure.",
        details:
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Cortex is better at shutting down stubborn terminal process trees without returning early while child processes are still alive.",
        details:
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "When a worktree setup step fails, Cortex now shows the failed setup state, keeps the timeline from looking stuck, and resets the local dispatch when you send again.",
        details:
      ...
      },
      {
        ...
        description:
          "Cortex avoids unnecessary provider refresh work during startup and gates Claude keepalive behavior more carefully, so opening the app is calmer and less likely to fight credential checks.",
        details:
    ...
    features: [
      {
        ...
        description:
          "When Cortex is downloading a desktop update, the sidebar update button now shows a live percent badge so you can tell whether it is moving or nearly ready.",
        details:
      ...
      },
      {
        ...
        description:
          "Cortex preserves the first pre-turn snapshot more carefully, so transcript diffs and restore points better match what was on disk when you pressed send.",
        details:
    ...
    features: [
      {
        ...
        description:
          "Cortex now surfaces Sonnet 5 variants in the model picker, so Claude workflows can jump to the newest Sonnet generation right from the app.",
        details:
      ...
      },
      {
        ...
        description:
          "Cortex now finds Cursor agent commands across more install layouts, including bundled sibling shims and legacy shim locations, while rejecting unsafe fallbacks.",
        details:
      ...
      {
        id: "muxy-open-in",
        title: "Muxy can open from Cortex",
        description:
      ...
      },
      {
        ...
        description:
          "Cortex refreshes Claude credential freshness on macOS so long sessions are less likely to hit the familiar stale-token sign-in failure.",
        details:
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        details:
          "Cortex now reuses the central robot glyph for agent chips, improves file-change and tool-call labels, refines chat card contrast, and tightens shared switch sizing, thumb travel, and animation.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Cortex can now launch VS Code Store installs through the right Windows app identity and URI fallback, so editor buttons work even when the normal `code` command is unavailable.",
        details:
      ...
      },
      {
        ...
        description:
          "A new settings toggle lets you disable provider update checks when you want Cortex to stay quieter about external CLI versions.",
        details:
      ...
      },
      {
        ...
        title: "The app icon and Cortex mark look cleaner",
        description:
          "The desktop, web, marketing, and release assets were refreshed so the Cortex icon renders more consistently across macOS, Windows, browser favicons, and update artifacts.",
        details:
          "This release refreshes the inline Cortex logo path, replaces generated icon assets from the full source image, corrects macOS bundle icon handling after the rounded-icon Ventura pass, and removes a literal Dock-icon workaround that was not the final direction.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Cortex polls local server state less aggressively while idle, reducing background work without changing the active-session refresh path.",
        details:
    ...
    features: [
      {
        ...
        description:
          "The branch toolbar can now show and change the active project, so project, branch, and worktree context are easier to keep aligned while you move around Cortex.",
        details:
      ...
      },
      {
        ...
        details:
          "Cortex now builds file diff trees, renders a collapsible review panel with shared disclosure motion, and reuses file-row styling so review navigation feels closer to the rest of the workspace.",
      },
      {
    ...
    features: [
      {
        ...
        details:
          "Cortex now formats tool command transcripts, normalizes patch/change output, labels more tool kinds consistently, preserves structured work metadata through the timeline, and adds focused coverage for tool-call labels and formatting.",
      },
      {
        id: "transcript-flow",
      ...
      },
      {
        ...
        details:
          "Cortex now ships a curated central-icons set, improves provider/model picker presentation, refines composer picker and automation banners, adds better project/thread hover details, and keeps repeated UI surfaces closer to the same visual language.",
      },
    ],
  },
  {
    ...
    features: [
      {
        ...
        description:
          "Cortex now has first-class Automations for scheduled agent work, with sidebar navigation, list/detail pages, run history, triage actions, and inline editing.",
        details:
      ...
      },
      {
        ...
        details:
          "File attachments now flow through contracts, upload storage, composer paste/drop, provider prompts, Kanban dispatch, recap/bootstrap surfaces, optimistic timeline rendering, caps/rollback, attachment-bearing plan follow-ups, explicit unsupported-file rejection, worktree-backed file preview roots, and blob-based download handling that keeps failed local image downloads inside Cortex.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Cortex now waits for one fresh server snapshot before giving up on a remembered chat route, so app restarts are less likely to dump you onto an empty fallback screen.",
        details:
    ...
    features: [
      {
        ...
        description:
          "Cortex now tracks richer local profile stats, including your most worked project, skill and agent usage, active hours, provider/model mix, and prompt activity.",
        details:
          "Profile stats now derive more signal from Cortex's local projection database: most-worked project, prompt/thread activity, skill and agent usage, provider/model usage, reasoning patterns, active-hour windows, and token heatmap data are all represented in the profile contract and settings panel.",
      },
      {
    ...
    features: [
      {
        ...
        title: "Your Cortex profile has more personality",
        description:
          "Profile settings now include richer identity details, activity stats, and a cleaner editing flow so Cortex feels more like your own workspace.",
        details:
      ...
      },
      {
        ...
        details:
          "Cortex now tracks thread retention state explicitly, covers soft-delete cleanup behavior with server tests, and keeps deletion/recovery semantics more predictable for early WIP data.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "After provider restarts, reconnects, or quiet ACP sessions, Cortex does a better job of reconciling active turns and finishing idle work instead of getting stuck.",
        details:
    ...
    features: [
      {
        ...
        title: "PDFs open safely inside Cortex",
        description:
      ...
      },
      {
        ...
        details:
          "Cortex now routes file preview through `WorkspaceFilePreview`, `PdfFilePreview`, shared preview headers, markdown/source selection references, workspace file openers, dock pane activation metadata, local preview URL helpers, and tighter file reference context-menu behavior.",
      },
      {
        id: "pi-plugin-routing",
        title: "Pi plugin sessions start in the right place",
        description:
          "Pi-backed plugin flows now route through Cortex more reliably, discover model support better, and keep startup prompts attached to the correct provider session.",
        details:
      ...
      },
      {
        ...
        details:
          "Cortex now preserves Cursor message identity more reliably, removes the older turn diff tree path, refines changed-file file-list rendering, fixes duplicate plan-mode icons and stale plan sidebar state, and hides inline project actions from the chat header where they created noise.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "There is now a playful soccer-ball physics view for experimenting with motion and interaction inside Cortex.",
        details:
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        details:
          "Cortex now discovers native editor apps and icons, caches icon assets server-side, exposes authenticated icon routes, and tightens macOS/Linux/Windows launcher handling, including Ghostty working-directory behavior and Linux desktop-entry matching.",
      },
      {
        id: "portable-skills",
      ...
      },
      {
        ...
        details:
          "Cortex now reconciles legacy migration bookkeeping before running migrations, expands migration coverage, validates workspace real-path containment, and carries terminal environment updates through shared server and web contracts.",
      },
    ],
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        details:
          "Cortex now reads provider credentials and usage data through shared server parsers, normalizes snapshots, stores cached values in SQLite, and renders reusable usage rows, progress tracks, line lists, and settings panels in the web app.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        details:
          "Cortex now caches site favicons server-side, serves authenticated favicon image URLs, recognizes bare domains in composer text, and keeps markdown link text aligned with the same medium-weight chip styling used while composing.",
      },
      {
    ...
    features: [
      {
        ...
        details:
          "Release tooling now validates Mac update artifacts, parses boolean environment flags consistently, and tests the resumable update downloader without the older update-feed cache layer. The README and release docs were refreshed around the current Cortex desktop flow too.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        details:
          "Claude SIGTERM from outside Cortex is treated as a benign suspend path, terminal process cleanup has stronger tests, and websocket RPC failure handling is less likely to leave the UI believing a request is still in flight.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Cortex can now generate and cache thread recaps, show current-state context in the chat environment, and reuse provider-backed recap generation without making the transcript harder to follow.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "When Claude reports a missing conversation id, Cortex clears the stale resume cursor, recreates the provider session, and retries with transcript context instead of leaving the turn failed.",
      },
      {
        id: "desktop-update-manual-fallback",
        title: "Desktop updates now have a manual escape hatch",
        description:
          "If an in-app install silently fails, Cortex restarts the backend, resumes update polling, deduplicates error toasts, and points you at the exact GitHub release page for a manual download.",
      },
      {
      ...
      },
      {
        ...
        description:
          "OpenCode task child sessions and newer shell-step events now flow into Cortex's activity timeline, while generic agent task rows keep their useful prompt and result text instead of disappearing or showing wrapper noise.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "OpenCode startup, model discovery, command discovery, server connection options, and experimental WebSocket mode now flow through the same settings and runtime paths as the rest of Cortex.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Fresh repair migrations reconcile older imported migration trackers that skipped Cortex's sidechat-source or pinned-thread columns, preventing startup crashes in those upgraded histories.",
      },
      {
  ...
  },
  {
    ...
    features: [
      {
        id: "cortex-home-migration",
        title: "Cortex is now the default home",
        description:
          "The app now starts from `~/.cortex`, carries the Cortex environment variables through the desktop and server runtime, and safely imports existing legacy data on first launch.",
      },
      {
        id: "desktop-platform-polish",
    ...
    features: [
      {
        ...
        description: "Cortex now includes Claude Opus 4.8 in the Claude model picker.",
      },
    ],
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "The provider picker no longer treats unknown provider status as usable. Providers stay in a checking state until Cortex has confirmed that the local runtime is available and authenticated.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Cortex can now launch and monitor Kilo Code sessions alongside Codex, Claude, Cursor, and OpenCode, with health checks, settings, mentions, handoff, and model compatibility wired through the app.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Cortex now tracks the newer OpenCode SDK session events, keeps titles fresher, and has much deeper coverage around OpenCode startup, output, and recovery flows.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Cortex now serves generated files through a dedicated local-image endpoint with MIME checks, workspace-aware path resolution, and Codex generated_images allowlists for both the normal home and desktop overlay home.",
      },
      {
  ...
  },
  {
    ...
    features: [
      {
        id: "cursor-provider",
        title: "Cursor is now a first-class Cortex provider",
        description:
          "Run Cursor CLI sessions directly from Cortex with ACP-backed startup, model discovery, existing-chat resume, handoff, and provider health checks alongside Codex and OpenCode.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "The Git actions menu can now commit current work and push it from Cortex, keeping the common release and handoff flow closer to the chat.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "The Git menu now includes branch creation with Cortex-style names, and PR creation can recover from GitHub duplicate-PR responses by reusing the existing open pull request.",
      },
      {
        id: "legacy-import-recovery",
        title: "Legacy imports heal themselves",
        description:
          "A new migration reconciles older imported legacy databases whose migration history skipped Cortex schema changes, preventing missing-column crashes after import.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "OpenCode todo events now flow into Cortex as active task updates, with a compact banner option for keeping current work visible without taking over the chat.",
      },
      {
      ...
      },
      {
        ...
        description:
          "OpenCode sessions now report context usage more consistently, giving Cortex better runtime visibility as conversations grow.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Cursor is now available as a provider, with ACP sessions, model discovery, existing chats, handoff, shortcuts, and git text generation wired into Cortex.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Toast notifications picked up feature-flag wiring, giving Cortex a safer way to roll notification changes forward or back.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Cortex now handles messy branch switches with clearer recovery actions, recreated stashes, unpublished branch publishing, and stronger checks around conflicts and local work.",
      },
      {
      ...
      },
      {
        ...
        description:
          "The desktop app now has app-level back and forward navigation controls, making it easier to move around Cortex without losing your place.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "GPT-5.5 is now in the model picker with the right default reasoning behavior, so you can move new Codex sessions onto the latest model directly from Cortex.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Cortex now shows a proper splash screen while the desktop backend spins up, so launch feels intentional instead of looking briefly stalled.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "The new theme pack editor lets you tune UI colors directly in Cortex, with shared theme tokens keeping the sidebar, composer, transcript, and controls in sync.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "If the updater never answers, Cortex now times out and recovers instead of hanging on a permanent Checking status.",
      },
    ],
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Point Cortex at your own Codex or Claude binary when your setup lives outside the default install path.",
      },
      {
