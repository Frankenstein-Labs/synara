...
const pillars = [
  {
    ...
    description:
      "Projects, task history, settings, and local application state are stored by the desktop app on your machine rather than in a Cortex-hosted workspace account.",
  },
  {
    Icon: LuPlug,
    title: "Provider traffic goes to the selected provider",
    description:
      "The coding-agent runtime receives the prompts, files, diffs, command output, and tool results required for that provider session. Cortex does not proxy normal provider traffic through its own model service.",
  },
  {
    Icon: LuShieldCheck,
    title: "No Cortex account is required",
    description:
  ...
  return (
    <section className="border-t border-[var(--divide)] py-14 sm:py-20">
      <div className={container}>
        ...
        <p className={body}>
          Cortex keeps its workspace layer local and connects to the provider runtime you select.
