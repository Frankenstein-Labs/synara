...
const themeInit = `(function(){try{var d=document.documentElement;var K=${JSON.stringify(THEME_KEY)};function stored(){try{return localStorage.getItem(K)}catch(e){return null}}function apply(){var t=stored();var dark=t==='dark'||(t!=='light'&&window.matchMedia('(prefers-color-scheme: dark)').matches);if(d.classList.contains('dark')!==dark)d.classList.toggle('dark',dark)}apply();window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change',function(){if(!stored())apply()});if(!window.__cortexThemeObserver){window.__cortexThemeObserver=new MutationObserver(function(){setTimeout(apply,0)});window.__cortexThemeObserver.observe(d,{attributes:true,attributeFilter:['class']})}window.addEventListener('load',function(){setTimeout(apply,0)});setTimeout(apply,300)}catch(e){}})()`;

/** Injects theme sync into the HTML stream so it runs before paint without React 19 script-tag warnings. */
export function ThemeScript() {
  useServerInsertedHTML(() => (
    <script id="cortex-theme-init" dangerouslySetInnerHTML={{ __html: themeInit }} />
  ));
  return null;
}
