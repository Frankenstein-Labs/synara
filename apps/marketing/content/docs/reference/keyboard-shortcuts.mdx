...
description: The full default keymap, verified against the Cortex source, plus JSON customization.
---
...
<Callout type="info" title="Spaces (preview on main)">
  Cortex `main` adds project spaces with `mod+alt+←` / `mod+alt+→` to switch spaces and
  `mod+alt+1`…`9` to jump. See [Features on main](/docs/reference/main-preview).
</Callout>

## Custom keybindings

A settings panel lets you explore and remap shortcuts. For full control, Cortex reads user overrides from:
