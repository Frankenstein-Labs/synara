---
title: iOS Simulator
description: Run, watch, and control an iOS Simulator beside a Cortex task on macOS.
---
...
The Cortex server must run on macOS with:
...
The pane is hidden when Cortex is connected to a non-macOS server. Building the app remains the responsibility of the task and its terminal; the device integration installs and launches build products but does not replace `xcodebuild`, Tuist, or the project's build system.

## Open and attach the pane

Open the right-dock add menu and choose **iOS Simulator**, then select a device. Cortex boots the selected simulator when needed and attaches its live stream to the current task.

The first attachment compiles Cortex's source-shipped device helper with the selected Xcode toolchain. The compiled helper is cached by source and toolchain identity, so later attachments normally reuse it. If setup is incomplete, the pane shows a checklist and updates it as Xcode, its license, and a simulator runtime become available.

An agent launch through Cortex's device tools can request that the pane open automatically for the owning task.
...
**Rotate view** changes how Cortex presents the captured screen; it does not force the guest app to change its simulator orientation.
...
Cortex distinguishes simulators it booted from devices that were already running. It can reclaim Cortex-owned devices after task cleanup or a crash; it does not automatically shut down a user-owned simulator simply because the pane detaches.
