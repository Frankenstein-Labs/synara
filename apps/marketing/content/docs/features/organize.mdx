...
Press `mod+f` to open the floating transcript search. Cortex highlights matching message text, shows the active result and total count, and supports previous/next navigation without treating tool-only activity as conversation text.
...
The primary sidebar destinations can also be dragged into your preferred order. Cortex keeps that order across windows and restarts while adding newly introduced destinations to a sensible default position.
...
Choose **Split diff** or **Stacked diff** in Diff options. Cortex remembers this choice when you reopen the panel and after an app restart. Split is the default until you save a preference.
