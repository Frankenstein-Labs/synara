---
title: Debug mode
description: Diagnose defects through Cortex's evidence-first observe, reproduce, investigate, fix, and verify loop.
---
...
Keep the requested application or state open until the provider says the evidence has been captured. Cortex cannot observe an external action unless the relevant browser, terminal, device, logs, or other signal is actually available to the task.
