---
title: Pull requests
description: Discover, inspect, fix, verify, and manage GitHub pull requests from the Cortex workspace.
---

Cortex’s pull-request workspace brings repository context, review discussion, checks, diffs, and agent tasks into one workflow. Use it to understand and repair a pull request, but keep Git and GitHub as the authoritative state for commits, branches, checks, and remote actions.

## Requirements

The repository must have:

- A local Git checkout available to Cortex
...
Cortex can gather visible unresolved review comments into a numbered composer prompt. Merge conflicts can be supplied through a separate Fix flow.
...
When GitHub exposes a dependent pull-request stack, Cortex adds a position badge to list and detail views. Open it to see the ordered stack from its base branch upward, each PR's state and branch, the current PR, and an assessment such as **Ready to merge**, **Merge status pending**, or **Stack needs attention**.
...
**Merge stack** affects the open stack prefix through the selected PR, not every PR above it. Cortex asks GitHub to merge that prefix atomically when the preview endpoint is available, polls the asynchronous result, and falls back to ordinary GitHub CLI merge behavior when the preview operation is unavailable. PRs above the selected one can remain open and be retargeted by GitHub.
...
Cortex can expose GitHub actions such as commenting, merging, closing, and reopening. These mutate shared remote state.
