...
Cortex gives you parallel tasks, worktrees, subagents, handoffs, browser tools, automations, and orchestration. The safest workflow is not the one using the most features—it is the smallest workflow that keeps ownership clear and produces a result you can verify.
...
Cortex checkpoints and managed environments help with recovery, but committed Git history is the strongest portable record of what you chose to keep.
