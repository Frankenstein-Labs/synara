...
| Agent Gateway orchestration | One Cortex task coordinates other Cortex tasks   | Structured multi-task execution and monitoring |
...
Create separate Cortex tasks when work needs its own:
...
Some providers can create child agents or tasks inside one provider session. Cortex can project supported child activity into the task UI, including identity, status, and tool activity.
...
The parent task remains the owner. Provider-native children are not automatically separate Cortex tasks, branches, or worktrees.
...
The [Agent Gateway](/docs/workflows/agent-gateway) lets supported Cortex provider sessions create, read, wait for, steer, interrupt, title, and archive ordinary Cortex tasks through app-owned MCP tools.
