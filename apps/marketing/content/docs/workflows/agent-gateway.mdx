---
title: Agent Gateway
description: Let a supported Cortex provider session inspect, create, coordinate, and monitor ordinary Cortex tasks through app-owned MCP tools.
---

The Cortex Agent Gateway is an internal, thread-scoped MCP control surface injected into supported provider sessions. It lets one Cortex task coordinate other Cortex resources while Cortex continues to enforce project, worktree, runtime-mode, and task-lifecycle rules.
...
- Create one or more ordinary Cortex tasks
...
The gateway does not create a separate orchestration database. Its tools delegate to Cortex’s existing task, provider, Git, automation, diagnostics, and browser services.

## Start with context and capabilities

A coordinating agent should orient itself before writing anything:

1. Call `cortex_context` to confirm the caller thread, active turn, provider, project, and granted coordination capabilities.
2. Call `cortex_capabilities` to discover provider/model targets, option shapes, availability, and operation limits.
3. Use `cortex_list_projects` when the work may target another project.
4. Use `cortex_list_threads` and `cortex_read_thread` to avoid duplicating existing work.
...
Use `cortex_create_thread` when the plan contains exactly one new task.
...
Use `cortex_create_threads` for an exact batch of 1–20 standalone tasks.
...
Use `cortex_wait_for_threads` to wait for authorized child tasks rather than repeatedly listing state or sending status messages.
...
Use `cortex_send_message` for a specific correction or follow-up. Do not flood a running task with repeated status requests.
...
- Project and thread access remain constrained by Cortex’s authorization model.
...
- A Cortex top-level task
- A Cortex worktree
- A separately resumable task
- An independent Git owner
- A resource managed through the Agent Gateway

Use provider-native subagents for bounded work inside one owner task. Use the Agent Gateway when you need durable, independently visible Cortex tasks.

## Browser tools

Supported provider sessions can receive the shared `browser_*` tools through the gateway. These control the task-scoped Cortex browser runtime, not the desktop or another chat.
...
- [ ] Native subagents are not confused with Cortex tasks
