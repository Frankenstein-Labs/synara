---
title: Worktrees
description: Isolate concurrent Cortex tasks with task-owned working directories and Git branches.
---

A Git worktree gives a task a separate working directory and checked-out branch while sharing the repository’s object database and history. In Cortex, it is the standard isolation boundary for concurrent tasks that may edit the same repository.
...
A worktree-backed Cortex task owns:
...
Cortex-managed worktrees remain associated with their tasks. Active environments are protected from routine cleanup, while archived environments can be pruned according to Cortex’s managed-worktree lifecycle.
...
<Callout type="info" title="Git is the preservation boundary">
  Cortex manages the environment, but commits and pushed refs are what make work portable and
  durable.
</Callout>
