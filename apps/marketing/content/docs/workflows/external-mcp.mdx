---
title: External MCP
description: Pair another local application with a restricted, user-approved Cortex task surface.
---

External MCP lets another local application—such as Codex, Claude Code, or an MCP-capable desktop client—create and inspect restricted Cortex tasks through a user-approved integration.

It is separate from the internal [Agent Gateway](/docs/workflows/agent-gateway) injected into supported provider sessions already running inside Cortex.
...
Use External MCP when work begins outside Cortex but should execute through Cortex’s durable task and worktree pipeline.
...
Start Cortex and open **Settings → Integrations**.
...
Choose **Create integration**. For an agentic MCP client, copy the generated setup prompt into that client. For a client that cannot complete the guided prompt, use the copy-ready configuration after completing pairing in Cortex.
...
The integration moves through waiting, paired, and connected states. The setup flow verifies the connection with `cortex_overview`.

</Step>
</Steps>

The generated setup is preferred because it uses the exact executable and data directory of the running Cortex installation. It does not require you to guess a global Cortex command, project ID, model slug, request ID, or credential path.
...
- `cortex_overview` — allowed projects, paths, provider availability, scopes, defaults, limits, activity, and suggested next steps
- `cortex_capabilities` — valid provider/model construction and safety limits for an allowed project
- `cortex_list_allowed_projects` — only projects granted to the integration
- `cortex_create_task` — one durable task for one stable `requestId`
- `cortex_wait_for_task` — wait for an authorized task without changing it
- `cortex_read_task` — read tasks owned by the integration, with broader project reads requiring a separate scope
...
Use `cortex_overview` and `cortex_capabilities` first. Do not guess IDs, model slugs, or option shapes.
...
- The external endpoint is available only while Cortex is loopback-only.
...
- Cortex stores credential hashes in its server database rather than raw server-side credentials.
- The local credential file is stored under the Cortex home directory with restrictive POSIX permissions where supported.
- The bridge verifies the running local instance with a fresh challenge before forwarding credentials or pairing codes.

On Windows, the credential remains under the current user profile but does not receive the same POSIX mode guarantee. Protect the Windows account and Cortex data directory accordingly.
...
- The exact Cortex server entry point
- `mcp serve`
- The integration identifier
- The actual Cortex home directory
- `ELECTRON_RUN_AS_NODE=1` where desktop packaging requires it

Do not copy a configuration from another Cortex installation without updating its executable and home directory. The bridge intentionally fails rather than silently connecting to an ambiguous or unsafe instance.

## Failure handling

### Waiting for pairing never completes

Use **Resume pairing**, confirm Cortex is still running, and restart the guided setup. Do not manually paste raw credentials into the client configuration.

### Paired but not connected

The client has a credential but has not successfully called the bridge. Verify the generated command, executable paths, Cortex home directory, and local client logs.

### The wrong projects appear

Review the integration’s selected projects and scopes. `cortex_list_allowed_projects` should never return projects outside that grant.

### Task creation is rejected

Read `cortex_overview` and `cortex_capabilities`. Check project access, provider availability, model selection, active-task limits, and whether the requested environment or runtime mode requires an ungranted scope.
...
Close stale Cortex instances or use the generated configuration for the intended installation. The bridge will not guess between ambiguous local runtimes.
...
- [ ] Generated setup uses the intended Cortex installation
- [ ] `cortex_overview` verifies the connection
...
<Callout type="warn" title="External MCP is local, scoped authority">
  Pairing another client grants it real ability to create Cortex work. Keep project access and
