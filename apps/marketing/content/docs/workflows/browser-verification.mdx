...
description: Verify visible web behavior with Cortex’s shared in-app browser, semantic snapshots, screenshots, logs, and deliberate retries.
---

Cortex’s browser is a shared Chromium page surface attached to the task. Supported agents can inspect and operate the same page, cookies, and session that you can reveal in the app. Use it to verify behavior—not as a substitute for understanding the implementation or reviewing the diff.
...
These are two presentations of one task-scoped browser session, not separate browsers. Tabs, cookies, navigation, and the page an agent operates remain attached to the task when you move between them. When the floating panel owns the live native page, the hidden sidebar stays preview-only so two surfaces cannot send input to the same guest.
...
Use the task’s assigned browser tab. Browser tabs are scoped to the Cortex thread; do not reuse a tab identifier from another task.
