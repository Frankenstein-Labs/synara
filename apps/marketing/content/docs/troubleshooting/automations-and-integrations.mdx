...
7. Cortex was running when the automation depended on the local desktop runtime.
...
- The caller is a provider session running inside Cortex
...
Read-only tools can remain available when a write tool is blocked. Use `cortex_context` and `cortex_capabilities` before assuming the gateway is broken.

## Agent Gateway creation was rejected

For `cortex_create_thread` or `cortex_create_threads`, verify:

- The `requestId` is stable for the same exact plan
- A changed plan uses a new `requestId`
- The provider and model come from `cortex_capabilities`
...
Use `cortex_wait_for_threads` rather than tight polling.
...
- Confirm the client reaches `cortex_overview`
- Verify the setup targets the currently running Cortex installation and data directory
...
- The explicit Cortex data directory is correct
- The intended integration ID is selected when more than one credential exists
- Cortex is running and loopback-only
...
Review the integration’s project scope in Cortex.
...
When several Cortex installations or credentials exist:
...
- [ ] The explicit Cortex installation and data directory were verified
