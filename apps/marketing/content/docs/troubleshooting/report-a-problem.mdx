...
description: Create a minimal, reproducible Cortex bug report with useful evidence and no exposed secrets.
---
...
1. Search existing [Cortex issues](https://github.com/Emanuele-web04/cortex/issues).
...
```text
Starting state:
- Cortex packaged release 0.x.y on macOS
...
- Client type
- Whether `cortex_overview` succeeds
...
Start with a redacted `cortex_diagnose_thread` result when available. Add one narrower source only when it answers a specific question.
...
- Do not upload the complete Cortex data directory
...
Open a new report in [Cortex issues](https://github.com/Emanuele-web04/cortex/issues/new) only after this checklist is complete.
