...
| ----------------------------------- | ------------------------------------------------ | -------------------------------------------------------- |
| `cortex_diagnose_thread`            | Bounded forensic snapshot across several sources | First-pass triage of a stuck or inconsistent task        |
| `cortex_read_thread_activity`       | Projected task activity                          | What Cortex presented as user-facing task activity       |
| `cortex_read_thread_events`         | Durable orchestration event journal              | Authoritative ordering of persisted orchestration events |
| `cortex_read_thread_runtime_events` | Retained provider-runtime events                 | What the provider adapter emitted before projection      |
...
```text
cortex_diagnose_thread({ threadId })
```
...
Use `cortex_read_thread_activity` when the question is:
...
Use `cortex_read_thread_events` when ordering and durable persistence matter.
...
Use `cortex_read_thread_runtime_events` when the question is:
...
1. Cortex version and build type
2. Operating system
...
8. A redacted `cortex_diagnose_thread` result
