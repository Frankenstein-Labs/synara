...
This guide takes you through Cortex's basic loop: open a repository, give one coding agent a concrete task, supervise the work, and review the resulting diff.

<Callout type="info" title="Before you begin">
  Install Cortex and authenticate at least one supported provider. In shortcuts, `mod` means Command
...
For this first task, use the local checkout and run only one agent against the repository. Use a [Git worktree](/docs/workflows/worktrees) when you begin running multiple tasks or want stronger isolation.
...
Cortex uses the provider runtime and account configured on your machine. It does not add a separate Cortex model plan.
...
<Cards>
  ...
  <Card
    ...
    description="Use the patterns Cortex maintainers rely on for real development work."
