...
import type { ProviderRuntimeEvent } from "@cortex/contracts";
...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.CORTEX_HOME = "/cortex-test/runtime";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/cortex-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.cortex/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.cortex/runtime/codex-home-overlay";
