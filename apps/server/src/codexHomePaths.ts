...
export const CORTEX_CODEX_HOME_OVERLAY_DIR = "codex-home-overlay";
...
export function resolveCortexCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
  sourceHomePath: string,
): string {
  const runtimeHome = env.CORTEX_HOME?.trim();
  const overlayRoot = runtimeHome || path.join(path.dirname(sourceHomePath), ".cortex", "runtime");
  return path.join(overlayRoot, CORTEX_CODEX_HOME_OVERLAY_DIR);
}

/**
 * Returns the home directory that the codex app-server child process actually
 * writes under. Cortex keeps its generated config isolated from the user's
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  const overlay = resolveCortexCodexHomeOverlayPath(env, source);
