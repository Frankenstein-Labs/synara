...
import { prewarmChatGptVoiceTranscriptionConnection } from "@cortex/shared/chatGptVoiceTranscription";
import { getModelSelectionBooleanOptionValue, normalizeModelSlug } from "@cortex/shared/model";
import { decodeSubagentReceiverThreadIds } from "@cortex/shared/subagents";
...
import { CORTEX_GATEWAY_HARNESS_POLICY } from "./agentGateway/harnessPolicy.ts";
import {
...
In code mode, call browser MCP methods directly inside \`functions.exec\` with the exact \`tools.mcp__cortex__browser_*\` prefix (for example, \`await tools.mcp__cortex__browser_open({ url })\` and \`await tools.mcp__cortex__browser_snapshot({})\`). The available suffixes are ${BROWSER_TOOL_NAMES.map((name) => `\`${name.slice("browser_".length)}\``).join(", ")}.
...
// Maps Cortex's simple runtime toggle to Codex thread-level permission overrides.
...
// Cortex re-sends turn-level Codex permission overrides, so keep "always allow"
...
export class CodexAppServerManager extends EventEmitter<CodexAppServerManagerEvents> {
  ...
  private readonly cortexSkillsDir: string | undefined;
  ...
  constructor(
    ...
    options?: {
      readonly cortexSkillsDir?: string;
    ...
    this.cortexSkillsDir = options?.cortexSkillsDir;
  ...
  // The Cortex MCP server rides on the shared overlay config (no secrets),
  ...
  private async buildSessionProcessEnv(
    ...
    if (gatewayBearerToken) {
      env[CORTEX_AGENT_GATEWAY_TOKEN_ENV] = gatewayBearerToken;
    }
    return env;
  }

  // Registers `~/.cortex/skills` as a codex skill root so portable skills are
  ...
  private async registerCortexSkillsRoot(context: CodexSessionContext): Promise<void> {
    if (!this.cortexSkillsDir) {
      return;
    }
    try {
      await this.sendRequest(context, "skills/extraRoots/set", {
        extraRoots: [this.cortexSkillsDir],
      });
    } catch (error) {
      if (!this.isContextRoutable(context)) throw error;
      // Older codex builds (< extra-roots support) keep working; Cortex-only
  ...
  async startSession(input: CodexAppServerStartSessionInput): Promise<ProviderSession> {
    ...
    try {
      ...
      await this.registerCortexSkillsRoot(context);
      ...
      if (
        ...
        this.emitLifecycleEvent(
          ...
          "Starting a new Codex thread for a Cortex thread that previously had a provider binding.",
        );
        await Effect.logWarning(
          "codex app-server starting a fresh thread for a previously bound Cortex thread",
          {
            threadId,
  ...
  private async handleServerRequest(
    ...
    if (isUserInputRequest) {
      ...
      const detail = "Codex asked a question Cortex could not render, so it was declined.";
