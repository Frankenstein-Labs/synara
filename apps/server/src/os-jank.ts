...
import { createCachedLoginShellPathReader } from "@cortex/shared/loginShellEnvironment";
import {
  expandHomePath as expandHomePathSync,
  resolveCortexHomeDirectory,
} from "@cortex/shared/cortexHome";
...
export function fixPath(
  ...
  try {
    ...
    // Cached by default: the probe result is persisted under the Cortex home and reused
...
export const resolveBaseDir = (raw: string | undefined): Effect.Effect<string> =>
  Effect.succeed(resolveCortexHomeDirectory({ configuredHome: raw }));
