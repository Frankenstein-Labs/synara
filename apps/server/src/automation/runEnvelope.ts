...
import type { AutomationDefinition, AutomationRun } from "@cortex/contracts";
import { automationContinuesThread, automationOwnsItsThread } from "@cortex/shared/automationMode";
...
// cortex_cancel_automation covers standalone runs, whose per-run thread owns nothing else.
const SELF_CANCEL_INSTRUCTION =
  "You may call cortex_cancel_automation on this automation when it is no longer needed.";
...
function reportingInstructions(mode: AutomationDefinition["mode"]): string {
  ...
  if (automationContinuesThread(mode)) {
    return [
      "Before finishing, call cortex_report_automation_result.",
      'Use decision "silent" when nothing needs the user\'s attention; otherwise use "notify".',
      SELF_CANCEL_INSTRUCTION,
    ].join(" ");
  }
  return [
    "Before finishing, call cortex_report_automation_result with a concise title and summary.",
...
export function buildAutomationRunEnvelope(input: {
  ...
  return [
    ...
    'Memory (persistent across runs — replace it via cortex_update_automation_memory {"memory": "..."} before finishing):',
