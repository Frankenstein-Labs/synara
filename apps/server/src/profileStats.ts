...
// Purpose: Compute Profile-page stats from Cortex's local projection DB only.
...
import { isBuiltInComposerSlashCommandName } from "@cortex/shared/composerSlashCommands";
...
// Builds profile skill rows from every stored Cortex user message, plus the
...
>()("cortex/profileStats/ProfileStatsQuery") {}
...
const makeProfileStatsQuery = Effect.gen(function* () {
  ...
  // Activity = days/hours the user actually sent a Cortex prompt. One day-hour
  ...
  // Token usage for EVERY provider, straight from Cortex's own DB (no external
  ...
  const getProfileStats = (
    ...
    Effect.gen(function* () {
      ...
      // ── Identity ──
      const homeDirBasename = nodePath.basename(config.homeDir) || "cortex";
