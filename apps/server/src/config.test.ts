...
const originalCortexStaticDir = process.env.CORTEX_STATIC_DIR;

function makeTempDir(prefix = "cortex-config-test-"): string {
...
afterEach(() => {
  ...
  tempDirs.clear();
  if (originalCortexStaticDir === undefined) {
    delete process.env.CORTEX_STATIC_DIR;
  } else {
    process.env.CORTEX_STATIC_DIR = originalCortexStaticDir;
  }
});

describe("resolveStaticDir", () => {
  it("uses the desktop static snapshot exposed through the Cortex environment", async () => {
    const snapshotDir = makeTempDir("cortex-static-snapshot-test-");
    fs.writeFileSync(path.join(snapshotDir, "index.html"), "<main>Cortex</main>");
    process.env.CORTEX_STATIC_DIR = snapshotDir;
...
describe("resolveDefaultChatWorkspaceRoot", () => {
  it("places the managed chat workspace under Documents/Cortex on macOS and Linux", () => {
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Documents/Cortex");
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/home/tester",
        platform: "linux",
      }),
    ).toBe("/home/tester/Documents/Cortex");
  });
  ...
  it("uses Windows separators when deriving the managed chat workspace on Windows", () => {
    ...
    ).toBe("C:\\Users\\tester\\Documents\\Cortex");
  });
  ...
  it("defaults to the current process platform when no platform is supplied", () => {
    ...
    try {
      expect(resolveDefaultChatWorkspaceRoot({ homeDir: "C:\\Users\\tester" })).toBe(
        "C:\\Users\\tester\\Documents\\Cortex",
      );
    } finally {
...
describe("resolveDefaultStudioWorkspaceRoot", () => {
  it("places the Studio workspace under Documents/Cortex/Studio on macOS and Linux", () => {
    expect(
      resolveDefaultStudioWorkspaceRoot({
        homeDir: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Documents/Cortex/Studio");
    expect(
      resolveDefaultStudioWorkspaceRoot({
        homeDir: "/home/tester",
        platform: "linux",
      }),
    ).toBe("/home/tester/Documents/Cortex/Studio");
  });
  ...
  it("uses Windows separators when deriving the Studio workspace on Windows", () => {
    ...
    ).toBe("C:\\Users\\tester\\Documents\\Cortex\\Studio");
  });
});
...
describe("resolveCanonicalWorkspaceRoots", () => {
  it("canonicalizes a symlinked home directory to match project row realpaths", async () => {
    ...
    expect(result.chatWorkspaceRoot).toBe(path.join(expectedHomeDir, "Documents", "Cortex"));
    expect(result.studioWorkspaceRoot).toBe(
      path.join(expectedHomeDir, "Documents", "Cortex", "Studio"),
    );
  });
  ...
  it("canonicalizes the nearest existing ancestor when the workspace root itself does not exist yet", async () => {
    ...
    // Cortex/ nor Cortex/Studio exist yet underneath it.
    ...
    expect(result.chatWorkspaceRoot).toBe(path.join(expectedDocuments, "Cortex"));
    expect(result.studioWorkspaceRoot).toBe(path.join(expectedDocuments, "Cortex", "Studio"));
