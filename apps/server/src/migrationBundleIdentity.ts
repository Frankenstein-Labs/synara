...
declare const __CORTEX_MIGRATION_RUNTIME_SOURCE_DIGEST__: string;
    ...
    const [relationship, recovery] =
      mismatch.kind === "launcher-bundle"
        ? [
            ...
            "Rebuild with bun run build:desktop before starting Cortex.",
          ]
        : [
            "the server bundle was built from a different migration source than this checkout",
            "Rebuild the server with bun run build before starting Cortex.",
          ];
    super(
...
export function embeddedMigrationRuntimeSourceDigest(): string | null {
  return typeof __CORTEX_MIGRATION_RUNTIME_SOURCE_DIGEST__ === "string"
    ? __CORTEX_MIGRATION_RUNTIME_SOURCE_DIGEST__
    : null;
}
...
function readMigrationSourceIfPresent(cwd: string): string | undefined {
  const checkoutRoot = findCortexSourceCheckoutRoot(cwd);
...
function findCortexSourceCheckoutRoot(cwd: string): string | null {
  ...
  for (;;) {
    try {
      ...
      if (packageJson.name === "@cortex/monorepo") return candidate;
    } catch (cause) {
