...
import { workspaceRootsEqual } from "@cortex/shared/threadWorkspace";
import {
  isThreadDetailEventFor,
  THREAD_DETAIL_EVENT_TYPES,
} from "@cortex/shared/threadDetailEvents";
...
import { discoverSkillsCatalog, cortexSkillsDir } from "./provider/skillsCatalog";
...
class WsRequestAdmissionMiddleware extends RpcMiddleware.Service<WsRequestAdmissionMiddleware>()(
  "cortex/WsRequestAdmissionMiddleware",
              ...
              Effect.gen(function* () {
                ...
                const registerCheckout = Effect.gen(function* () {
                  yield* Queue.offer(queue, {
                    ...
                    kind: "phase",
                    phase: "registering",
                    message: "Adding project to Cortex",
                  });
          ...
          rpcEffect(
            Effect.tryPromise(() =>
              discoverSkillsCatalog({
                cwd: input.cwd ?? null,
                homeDir: config.homeDir,
                cortexBaseDir: config.baseDir,
                includeDuplicateOrigins: true,
              }),
            ).pipe(
              Effect.map((skills) => ({
                skills,
                cortexSkillsDir: cortexSkillsDir(config.baseDir),
              })),
            ),
    ...
    Effect.gen(function* () {
      ...
      yield* router.add(
        ...
        Effect.gen(function* () {
          ...
          // The desktop app fetches cross-origin (cortex://app); reflect only
