...
} from "@cortex/shared/deviceHelperCache";
...
export class IosSimulatorBackend implements DeviceBackend {
  ...
  async availability(): Promise<DeviceAvailability> {
    ...
    steps.push({
      ...
      label: "Build the Cortex device helper",
      done: helperBuilt,
  ...
  async screenshot(
    ...
    const directory = await mkdtemp(path.join(tmpdir(), "cortex-device-"));
   ...
   * Cortex, SimulatorKit dropped the connection) the injection silently
  ...
  private async attachedHelper(
    ...
    try {
      ...
      // A device can also be rebooted outside Cortex (Simulator.app, or simctl
