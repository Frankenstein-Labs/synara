 ...
 * directory so both land in the same place under a custom CORTEX_HOME.
 *
...
function defaultBootOwnershipPath(): string {
  const baseDir = process.env.CORTEX_HOME?.trim() || path.join(homedir(), ".cortex");
          ...
          if (reclaimed.length > 0) {
            console.info(
              `[device] shut down ${reclaimed.length} simulator(s) left booted by a previous ` +
                `Cortex run: ${reclaimed.join(", ")}`,
            );
          }
        });
      }

      // App quit shuts down every simulator Cortex booted and leaves the
