 ...
 * - The Cortex boot cap (`DEVICE_CORTEX_BOOT_LIMIT`). Boot past the cap is
...
/** How long a Cortex-booted device stays up with no thread attached. */
...
export interface DeviceManagerOptions {
  ...
  /** Remembers Cortex's boots across a crash; defaults to remembering nothing. */
...
export class DeviceManager {
  ...
  private readonly cortexBooted = new Set<string>();
  ...
  constructor(options: DeviceManagerOptions) {
    ...
    this.bootLimit = options.bootLimit ?? DEVICE_CORTEX_BOOT_LIMIT;
  ...
  private async recordBootOwnership(): Promise<void> {
    await this.bootOwnership.write([...this.cortexBooted]).catch(() => undefined);
  }
  ...
  async cortexBootedDevices(): Promise<readonly DeviceDescriptor[]> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    this.reconcileCortexBooted(devices);
    return devices
      .filter((device) => this.cortexBooted.has(device.udid))
   ...
   *
   * The set is Cortex's own bookkeeping, but the simulators are not Cortex's to
   ...
   */
  private reconcileCortexBooted(devices: readonly DeviceDescriptor[]): void {
    const running = new Set(
      devices
        .filter((device) => device.state === "booted" || device.state === "booting")
        .map((device) => device.udid),
    );
    for (const udid of this.cortexBooted) {
      if (running.has(udid)) continue;
      this.cortexBooted.delete(udid);
  ...
  async boot(udid: string): Promise<DeviceBootResult> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    // Devices that stopped without Cortex doing it still held their slots, so
    ...
    // Cortex from accumulating simulators, not to limit what the user watches.
    if (known?.state === "booted") {
      return { kind: "booted", device: this.describe(known) };
    }
    if (this.cortexBooted.size >= this.bootLimit) {
      return {
        kind: "boot-limit-reached",
        limit: this.bootLimit,
        cortexBooted: await this.cortexBootedDevices(),
      };
    }
    ...
    // exists to stop Cortex accumulating multi-gigabyte simulators would be
    // exceeded by however many requests arrived inside that window.
    this.cortexBooted.add(udid);
    ...
    return { kind: "booted", device: { ...device, bootSource: "cortex" } };
  }
  ...
  /**
   * App quit: shut down everything Cortex booted, leave the user's devices
  ...
  async dispose(): Promise<void> {
    ...
    const booted = Array.from(this.cortexBooted);
  ...
  private describe(device: DeviceDescriptor): DeviceDescriptor {
    const bootSource = this.cortexBooted.has(device.udid) ? "cortex" : device.bootSource;
   ...
   * the device down or start the idle countdown if Cortex booted it.
   *
   * A switch shuts down immediately rather than waiting out the idle window.
   * The cap is three Cortex-booted devices, and each one costs a couple of GB
  ...
  private async releaseDevice(
    ...
    if (!this.cortexBooted.has(udid)) return;
    if (reason === "switched") {
      this.clearIdleTimer(udid);
  ...
  private async shutdownIfStillIdle(udid: string): Promise<void> {
    ...
    if (this.isAttachedAnywhere(udid) || !this.cortexBooted.has(udid)) return;
    await this.shutdown(udid).catch(() => undefined);
  }
