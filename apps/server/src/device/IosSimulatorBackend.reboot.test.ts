...
function makeBackend() {
  ...
  const backend = new IosSimulatorBackend({
    platform: "darwin",
    helperCacheRoot: "/tmp/cortex-device-test-cache",
  ...
  Object.defineProperty(backend, "compileHelperIfNeeded", {
    value: async () => "/tmp/cortex-device-test-cache/cortex-device-helper",
  });
...
describe("simulator reboot", () => {
  it("re-attaches after a Cortex-driven shutdown and reboot", async () => {
  ...
  it("recovers when the device was rebooted outside Cortex", async () => {
    const { backend, helper } = makeBackend();
    await backend.tap(DEVICE, 10, 10);
