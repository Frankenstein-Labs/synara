...
describe("helper cache path agreement", () => {
  it("builds into the directory the shared key names", async () => {
    ...
    const backend = new IosSimulatorBackend({
      platform: "darwin",
      helperSourceDir: "/tmp/cortex-helper-src",
      helperCacheRoot: "/tmp/cortex-helper-cache",
    ...
    expect(build?.args[1]).toBe(join("/tmp/cortex-helper-cache", expectedKey));
  });
  ...
  it("names the binary the build script produces", () => {
    expect(DEVICE_HELPER_BINARY_NAME).toBe("cortex-device-helper");
  });
});
