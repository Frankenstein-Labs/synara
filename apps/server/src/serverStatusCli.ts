...
export interface CortexServerHealthSnapshot {
...
export type CortexServerStatusResult =
  | {
      readonly reachable: true;
      readonly ready: boolean;
      readonly url: string;
      readonly health: CortexServerHealthSnapshot;
    }
  | {
...
export interface FetchCortexServerStatusOptions {
  readonly url?: string;
...
function decodeHealthSnapshot(value: unknown): CortexServerHealthSnapshot | null {
  ...
  return snapshot as unknown as CortexServerHealthSnapshot;
}

export async function fetchCortexServerStatus(
  options: FetchCortexServerStatusOptions = {},
): Promise<CortexServerStatusResult> {
...
export function formatCortexServerStatus(result: CortexServerStatusResult): string {
  if (!result.reachable) {
    return `Cortex server: unreachable\nURL: ${result.url}\nError: ${result.error}`;
  }
