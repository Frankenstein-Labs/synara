...
function withFakeAcpAgent<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const tempDir = mkdtempSync(path.join(os.tmpdir(), "cortex-droid-text-acp-"));
      return {
        tempDir,
...
const cases: TestCase[] = [
  {
    ...
    env: (tempDir) => ({
      CORTEX_ACP_REQUEST_LOG_PATH: path.join(tempDir, "requests.ndjson"),
  ...
  },
  {
    ...
    env: () => ({ CORTEX_ACP_PROMPT_RESPONSE_TEXT: "Sidebar Thread Row Spacing" }),
    run: (agentPath) =>
      Effect.gen(function* () {
        const textGeneration = yield* TextGeneration;
  ...
  },
  {
    ...
    env: (tempDir) => ({
      CORTEX_ACP_EXIT_LOG_PATH: path.join(tempDir, "exit.log"),
      CORTEX_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
