...
const testScratchParent = mkdtempSync(path.join(tmpdir(), "cortex-scratch-test-"));
...
describe("ensureIsolatedScratchWorkspace", () => {
  it("keeps the default scratch root in a private user cache outside the checkout", () => {
    ...
    try {
      ...
      expect(path.dirname(root)).toMatch(/\.cortex-[a-f0-9]{16}$/);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      const redirectedRoot = mkdtempSync(path.join(tmpdir(), "cortex-legacy-redirect-"));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      const workspace = ensureTestScratchWorkspace(threadId);
      const redirected = mkdtempSync(path.join(tmpdir(), "cortex-scratch-redirect-"));
      rmSync(workspace, { recursive: true, force: true });
