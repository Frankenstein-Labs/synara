...
// Purpose: Build a ZIP archive that exports a single Cortex thread so a user
...
// The export-eligibility guard lives in @cortex/shared/threadExport so the
// web composer and the HTTP route share one predicate.

import zlib from "node:zlib";

import type { OrchestrationMessage, OrchestrationThread } from "@cortex/contracts";
...
export function threadArchiveFileName(input: {
  ...
  return `cortex-thread-${slugifyTitle(input.title)}-${dateBucket}.zip`;
}
