...
describe("trustedOrigins", () => {
  ...
  it("trusts only the HTTPS public origin for browser traffic in proxy-backed remote mode", () => {
    ...
    expect(
      isTrustedAppOrigin({
        ...
        requestOrigin: "http://cortex.example.test",
        config: remoteConfig,
      }),
    ).toBe(true);
    expect(
  ...
  it("normalizes desktop origins with trailing slashes", () => {
    expect(normalizeCorsOrigin("cortex://app/")).toBe("cortex://app");
    expect(normalizeCorsOrigin("cortex-canary://app/")).toBe("cortex-canary://app");
  });
