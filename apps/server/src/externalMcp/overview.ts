import type { ExternalMcpCapability } from "@cortex/contracts";
...
export function buildExternalMcpOverviewNextSteps(
  ...
  return [
    ...
    ...(capabilities.has("tasks:create") ? ["Create work with cortex_create_task."] : []),
    ...(capabilities.has("tasks:wait") ? ["Follow permitted work with cortex_wait_for_task."] : []),
    ...(capabilities.has("tasks:read")
      ? ["Read permitted task results with cortex_read_task."]
      : []),
  ];
}
