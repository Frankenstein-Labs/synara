...
describe("migration backup recovery CLI", () => {
  it("ships the recovery command from the bundled server package", async () => {
    ...
    expect(packageJson.bin?.["cortex-restore-migration-backup"]).toBe(
      "dist/restoreMigrationBackup.mjs",
    );
  });

  it("rejects relative database paths and warns operators to stop Cortex", async () => {
    ...
    expect(capture.errors.join("\n")).toContain("cortex-restore-migration-backup");
    expect(capture.warnings.join("\n")).toContain("Stop every Cortex process");
