...
import { AuthSessionId } from "@cortex/contracts";
import {
...
function makeServerAuth(sideEffects: { count: number }): ServerAuthShape {
  ...
  return {
    ...
    authenticateHttpRequest: (request) => {
      ...
      const cookie = request.cookies.cortex_session === "cookie-token";
      if (!bearer && !cookie) {
    ...
    issueStartupPairingUrl: () =>
      Effect.succeed("https://cortex.example.test/pair#token=PAIRINGTOKEN"),
  ...
  return {
    ...
    headers: {
      ...
      ...(input.credential === "bearer"
        ...
        : { Cookie: "cortex_session=cookie-token" }),
...
describe("authEffectRouteLayer", () => {
  ...
  it("rejects every cookie-authenticated mutation without a trusted origin", async () => {
    ...
    const config = {
      host: "0.0.0.0",
      publicUrl: new URL("https://cortex.example.test/"),
...
describe("binaryUploadEffectRouteLayer", () => {
  ...
  it("allows credentialed Canary attachment upload preflights", async () => {
    const config = {
      host: "127.0.0.1",
      attachmentsDir: fs.mkdtempSync(path.join(os.tmpdir(), "cortex-upload-cors-")),
    } as ServerConfigShape;
    try {
      await withAuthEffectServer(
        config,
        ...
        async (serverOrigin) => {
          ...
          expect(response.headers.get("access-control-allow-origin")).toBe("cortex-canary://app");
  ...
  it("rejects ambient cookie uploads without an origin and accepts explicit bearer auth", async () => {
    const attachmentsDir = fs.mkdtempSync(path.join(os.tmpdir(), "cortex-upload-route-"));
    const config = {
      host: "0.0.0.0",
      publicUrl: new URL("https://cortex.example.test/"),
      attachmentsDir,
      ...
      await withAuthEffectServer(
        ...
        async (serverOrigin) => {
          ...
          const cookieResponse = await fetch(url, {
            method: "POST",
            headers: { Cookie: "cortex_session=cookie-token" },
            body: Uint8Array.from([1]),
          });
