...
// Purpose: Builds provider child environments without Cortex control-plane authority.
...
const isTestHarnessKey = (key: string, env: NodeJS.ProcessEnv): boolean =>
  Boolean(env.VITEST) && (key.startsWith("CORTEX_FAKE_") || key.startsWith("CORTEX_ACP_"));

export function buildProviderChildEnvironment(input: {
  readonly provider: ProviderChildKind;
  readonly baseEnv?: NodeJS.ProcessEnv;
  readonly inheritedCortexKeys?: ReadonlyArray<string>;
  ...
  const allowedCortexKeys = new Set(input.inheritedCortexKeys ?? []);
