...
import type { ProviderSkillDescriptor } from "@cortex/contracts";
...
beforeEach(() => {
  clearSkillsCatalogCacheForTests();
  root = mkdtempSync(path.join(os.tmpdir(), "cortex-skills-catalog-"));
  homeDir = path.join(root, "home");
  cortexBaseDir = path.join(homeDir, ".cortex");
});
...
describe("discoverSkillsCatalog", () => {
  it("creates the Cortex skills folder on first discovery", async () => {
    await discoverSkillsCatalog({ homeDir, cortexBaseDir });
    await expect(access(path.join(cortexBaseDir, "skills"))).resolves.toBeUndefined();
  });

  it("aggregates skills from cortex and provider home folders with origin scopes", async () => {
    await writeSkill(path.join(cortexBaseDir, "skills", "portable"), "portable", "Cortex skill");
    ...
    const skills = await discoverSkillsCatalog({ homeDir, cortexBaseDir });
    const byName = new Map(skills.map((skill) => [skill.name, skill]));

    expect(byName.get("portable")?.scope).toBe("cortex");
  ...
  it("includes user and matching project Claude plugins but excludes other projects", async () => {
    ...
    const skills = await discoverSkillsCatalog({ cwd, homeDir, cortexBaseDir });
  ...
  it("can include duplicate skill names from different origins for settings", async () => {
    ...
    const defaultCatalog = await discoverSkillsCatalog({ homeDir, cortexBaseDir });
  ...
  it("prefers the provider-native copy and falls back to Cortex for that provider", async () => {
    await writeSkill(path.join(cortexBaseDir, "skills", "shared"), "shared", "Cortex copy");
    await writeSkill(path.join(homeDir, ".codex", "skills", "shared"), "shared", "Codex copy");
    await writeSkill(path.join(cortexBaseDir, "skills", "only-cortex"), "only-cortex", "Fallback");

    const codexView = await discoverSkillsCatalog({ homeDir, cortexBaseDir, provider: "codex" });
    ...
    expect(codexView.some((skill) => skill.name === "only-cortex")).toBe(true);

    // A provider without its own copy resolves the Cortex fallback.
    ...
    expect(claudeShared?.scope).toBe("cortex");
  });

  it("uses documented provider alias roots before Cortex fallbacks", async () => {
    await writeSkill(path.join(cortexBaseDir, "skills", "shared"), "shared", "Cortex copy");
...
`,
  ...
  it("serves cached results within the TTL and rescans on forceReload", async () => {
    await writeSkill(path.join(cortexBaseDir, "skills", "first"), "first", "First skill");

    const initial = await discoverSkillsCatalog({ homeDir, cortexBaseDir });
    expect(initial.map((skill) => skill.name)).toEqual(["first"]);

    // A skill added after the first scan is invisible to the cached entry...
    await writeSkill(path.join(cortexBaseDir, "skills", "second"), "second", "Second skill");
    const cached = await discoverSkillsCatalog({ homeDir, cortexBaseDir });
    expect(cached.map((skill) => skill.name)).toEqual(["first"]);

    // ...but forceReload bypasses the cache and refreshes it.
    const reloaded = await discoverSkillsCatalog({ homeDir, cortexBaseDir, forceReload: true });
    expect(reloaded.map((skill) => skill.name).sort()).toEqual(["first", "second"]);
  });

  it("includes project-level .cortex skills when a cwd is provided", async () => {
    const cwd = path.join(root, "repo", "packages", "web");
    await mkdir(cwd, { recursive: true });
    await writeSkill(
      path.join(root, "repo", ".cortex", "skills", "repo-skill"),
      "repo-skill",
      "Project skill",
    );
  ...
  it("keeps home origins when the cwd lives under the home dir", async () => {
    ...
    await writeSkill(path.join(cortexBaseDir, "skills", "portable"), "portable", "Cortex");
    ...
    expect(skills.find((skill) => skill.name === "portable")?.scope).toBe("cortex");
  });

  it("dedupes same-named skills within a root deterministically", async () => {
    await writeSkill(path.join(cortexBaseDir, "skills", "zeta"), "twin", "Copy in zeta");
    await writeSkill(path.join(cortexBaseDir, "skills", "alpha"), "twin", "Copy in alpha");
...
describe("mergeSkillsIntoCatalog", () => {
  ...
  it("keeps provider-native entries and appends catalog-only entries", () => {
    const merged = mergeSkillsIntoCatalog({
      ...
      catalog: [descriptor("Shared", "cortex"), descriptor("extra", "cortex")],
    });
