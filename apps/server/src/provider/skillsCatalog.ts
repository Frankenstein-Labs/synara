...
//          walking) plus the unified cross-provider skills catalog backing Cortex
//          portable skills. Aggregates `~/.cortex/skills` with every provider-native
...
//          mergeSkillsIntoCatalog, filterDisabledSkills, ensureCortexSkillsDir

import * as fs from "node:fs/promises";
import * as nodePath from "node:path";

import type { ProviderKind, ProviderSkillDescriptor } from "@cortex/contracts";
...
export interface SkillsCatalogDiscoveryInput {
  ...
  /** Cortex base dir (usually `~/.cortex`); skills live in `{base}/skills`. */
  readonly cortexBaseDir: string;
...
export interface SkillsCatalogRootInput extends SkillsCatalogDiscoveryInput {
  /** Native provider scans can opt out; the catalog itself always includes Cortex. */
  readonly includeCortexRoot?: boolean;
}
...
const ensuredCortexSkillsDirs = new Set<string>();

export function clearSkillsCatalogCacheForTests(): void {
  skillsCatalogCache.clear();
  skillsCatalogInflight.clear();
  ensuredCortexSkillsDirs.clear();
}

export function cortexSkillsDir(cortexBaseDir: string): string {
  return nodePath.join(cortexBaseDir, "skills");
}

// Creates the portable skills folder on first use so users have a drop-in target.
export async function ensureCortexSkillsDir(cortexBaseDir: string): Promise<string> {
  const dir = cortexSkillsDir(cortexBaseDir);
  if (ensuredCortexSkillsDirs.has(dir)) {
    return dir;
  }
  try {
    await fs.mkdir(dir, { recursive: true });
    ensuredCortexSkillsDirs.add(dir);
  } catch {
...
const SKILL_ORIGIN_ROOTS = {
  cortex: {
    homeRoots: (input) => [cortexSkillsDir(input.cortexBaseDir)],
    projectRootNames: [".cortex"],
  },
  codex: {
    // Keep Cortex's existing Codex-local root. Official Codex discovery uses
...
// Native copies first so an agent keeps using its own skill, then Cortex as the
...
function orderedOriginsForProvider(
  provider: ProviderKind | null | undefined,
  includeCortexRoot = true,
  includeRemainingOrigins = true,
): SkillsHomeOrigin[] {
  const preferred = preferredOriginsForProvider(provider);
  const ordered = [...preferred];
  if (includeCortexRoot && !ordered.includes("cortex")) {
    ordered.push("cortex");
  }
  if (!includeRemainingOrigins) {
    return ordered.filter((origin) => includeCortexRoot || origin !== "cortex");
  }
  for (const origin of HOME_ORIGIN_ORDER) {
    if (!includeCortexRoot && origin === "cortex") {
      continue;
    }
...
export function skillsCatalogRoots(input: SkillsCatalogRootInput): SkillRoot[] {
  return rootsForOrderedOrigins(
    input,
    orderedOriginsForProvider(input.provider, input.includeCortexRoot !== false),
  );
}
...
export async function discoverSkillsCatalog(
  ...
  const scan = (async () => {
    await ensureCortexSkillsDir(input.cortexBaseDir);
    const roots = [
