...
//          that makes Cortex catalog skills usable on every provider.
...
import type { ProviderKind, ProviderSkillReference } from "@cortex/contracts";
...
export function shouldInlineSkillForProvider(provider: ProviderKind, skillPath: string): boolean {
  ...
  switch (provider) {
    ...
    case "codex":
      ...
      // folders plus `~/.cortex/skills`, which Cortex registers at session start
    ...
    case "cursor":
      ...
      // only Cortex-owned paths need inlining.
      return segments.has(".cortex");
    case "claudeAgent":
