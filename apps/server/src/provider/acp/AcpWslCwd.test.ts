...
describe("resolveAcpSessionCwd", () => {
  it("converts modern WSL UNC paths to Linux paths on Windows", () => {
    expect(
      resolveAcpSessionCwd("\\\\wsl.localhost\\Ubuntu-24.04\\home\\dev\\cortex", "win32"),
    ).toBe("/home/dev/cortex");
  });
  ...
  it("keeps ordinary Windows workspaces unchanged", () => {
    expect(resolveAcpSessionCwd("C:\\src\\cortex", "win32")).toBe("C:\\src\\cortex");
  });
