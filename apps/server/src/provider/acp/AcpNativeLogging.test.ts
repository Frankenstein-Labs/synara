...
import { CORTEX_AGENT_GATEWAY_TOKEN_ENV } from "../../agentGateway/mcpInjection.ts";
...
describe("AcpNativeLogging", () => {
  it.effect("redacts gateway credentials from request and protocol NDJSON logs", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cortex-acp-secret-log-"));
      ...
      try {
        ...
        yield* requestLogger({
          ...
          payload: {
            ...
            mcpServers: [
              ...
              {
                env: [
                  { name: CORTEX_AGENT_GATEWAY_TOKEN_ENV, value: sentinelToken },
        ...
        yield* protocolLogger({
          ...
          payload: JSON.stringify({
            ...
            env: [{ name: CORTEX_AGENT_GATEWAY_TOKEN_ENV, value: sentinelToken }],
          }),
        });
        yield* protocolLogger({
          direction: "outgoing",
...
describe("redactAcpLogSecrets", () => {
  ...
  it("redacts raw JSON protocol frames as string and bytes", () => {
    const frame = JSON.stringify({
      ...
      env: [{ name: CORTEX_AGENT_GATEWAY_TOKEN_ENV, value: "raw-frame-gateway-SENTINEL" }],
