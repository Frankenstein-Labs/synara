...
import { TurnId } from "@cortex/contracts";
...
import {
  ...
  takeGrokCortexHarnessPolicyTextPart,
} from "./GrokAdapter.ts";
...
describe("Grok Cortex harness policy", () => {
  it("delivers private scoped host context once", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    expect(takeGrokCortexHarnessPolicyTextPart(state, true)?.text).toContain(
      CORTEX_HARNESS_POLICY_MARKER,
    );
    expect(takeGrokCortexHarnessPolicyTextPart(state, true)).toBeNull();
  });
});
...
describe("Grok native plan approval", () => {
  it("adds Plan instructions without sending a pager-only slash command as model text", () => {
    ...
    ).toMatch(/^Cortex requested Grok's native plan mode\./u);
  });
  ...
  it("backs native Plan mode with a fail-closed pre-tool hook", () => {
    expect(
      resolveGrokPlanHookResponse("plan", {
        hookCallbackId: "cortex-plan-guard",
  ...
  it("keeps native plan mode gated after Cortex captures the plan", () => {
    expect(makeGrokExitPlanModeCapturedResponse()).toEqual({
      outcome: "cancelled",
      feedback:
        "Cortex captured this plan for user review. Do not revise or implement it now. End this turn and wait for the user's next message.",
    });
  });
...
describe("Grok native user questions", () => {
  ...
  it("maps Cortex answers to Grok's question-text keyed response", () => {
