...
type AntigravitySessionContext = ToolSurfaceCounters & {
   ...
   * session's own CLI inherits `CORTEX_ANTIGRAVITY_EVENTS` and writes its
...
/**
 * Hook output when capture is inactive (the session is not Cortex-managed).
 ...
 * ("Antigravity CLI exited with code 1."). Cortex-managed sessions spawn
...
export function buildAntigravityCaptureCommand(
  ...
  if (platform === "win32") {
    ...
    return `if not defined CORTEX_ANTIGRAVITY_EVENTS (more >nul 2>nul & echo ${fallback}) else (set ELECTRON_RUN_AS_NODE=1&& ${invocation})`;
  }
  const invocation = `${shellQuote(executablePath, platform)} ${shellQuote(scriptPath, platform)} ${shellQuote(event, platform)}`;
  return `if [ -z "\${CORTEX_ANTIGRAVITY_EVENTS:-}" ]; then cat >/dev/null 2>&1 || :; printf '%s\\n' '${fallback}'; else ELECTRON_RUN_AS_NODE=1 ${invocation}; fi`;
}
...
process.stdin.on("end", () => {
  const target = process.env.CORTEX_ANTIGRAVITY_EVENTS;
  if (!target) {
  ...
  if (event === "pre-tool") {
    const decision = process.env.CORTEX_ANTIGRAVITY_HOOK_DECISION === "allow" ? "allow" : "ask";
    process.stdout.write(JSON.stringify({ decision }) + "\\n");
  } else if (event === "pre-invocation") {
    // PreInvocation vetoes the upcoming LLM invocation; Cortex-managed
    ...
    `${JSON.stringify(
      {
        ...
        name: "cortex-capture",
        description: "Streams Antigravity CLI lifecycle events to Cortex when requested.",
      },
      null,
      2,
    )}\n`,
...
export function buildAntigravityTurnProcessEnvironment(input: {
  ...
  const gatewayKeys = hasGatewayBootstrap
    ? [CORTEX_AGENT_GATEWAY_URL_ENV, CORTEX_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]
    : [];
  const gatewayEnvironment = hasGatewayBootstrap
    ? {
        [CORTEX_AGENT_GATEWAY_URL_ENV]: input.gatewayConnection!.url,
        [CORTEX_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]: input.gatewayBootstrapToken!,
      }
    : {};
  return buildProviderChildEnvironment({
    ...
    inheritedCortexKeys: [
      ...
      "CORTEX_ANTIGRAVITY_HOOK_DECISION",
      ...gatewayKeys,
    ],
    overrides: {
      CORTEX_ANTIGRAVITY_EVENTS: input.eventFile,
...
export function buildAntigravityTurnPrompt(
  ...
  const harnessPolicy = takeCortexHarnessPolicyForProviderSession(state, {
    provider: PROVIDER,
...
export function parseAntigravityCliModelLabel(
  ...
  // Cortex never treats `slug\tName` as a single model id at dispatch.
...
const makeAntigravityAdapter = (dependencies: AntigravityAdapterDependencies = {}) =>
  Effect.gen(function* () {
     ...
     * subagent CLI inherits `CORTEX_ANTIGRAVITY_EVENTS` and writes its
        ...
        yield* Effect.tryPromise({
          try: () =>
            ...
            new ProviderAdapterRequestError({
              ...
              detail: messageFromCause(cause, "Failed to install the Cortex capture hook."),
              cause,
            }),
        });
    ...
    const sendTurn: AntigravityAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const runDir = yield* Effect.tryPromise({
          try: () => fs.mkdtemp(path.join(os.tmpdir(), "cortex-antigravity-")),
          catch: (cause) =>
            new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "turn/prepare",
        ...
        if (gatewaySessionLease && !gatewayBootstrapToken) {
          ...
          return yield* new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "turn/prepare",
            detail: "The Cortex gateway credential is no longer active for this provider turn.",
          });
        }
