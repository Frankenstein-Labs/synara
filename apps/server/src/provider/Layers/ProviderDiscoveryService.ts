...
const make = Effect.gen(function* () {
  ...
  const getComposerCapabilities: ProviderDiscoveryServiceShape["getComposerCapabilities"] = (
    ...
    Effect.gen(function* () {
      ...
      // The unified Cortex skills catalog backs skill discovery for every
    ...
    Effect.gen(function* () {
      ...
      const nativeResult: ProviderListSkillsResult | null = adapter.listSkills
        ? yield* adapter
            ...
            .pipe(
              Effect.catch((error) =>
                Effect.logWarning(
                  "provider-native skill discovery failed; serving the Cortex skills catalog only",
      ...
      const catalogSkills = yield* Effect.tryPromise(() =>
        discoverSkillsCatalog({
          ...
          cortexBaseDir: serverConfig.baseDir,
          provider: parsed.provider,
          ...(parsed.forceReload !== undefined ? { forceReload: parsed.forceReload } : {}),
        }),
      ).pipe(
        Effect.catchCause((cause) =>
          Effect.logWarning("cortex skills catalog discovery failed", {
            provider: parsed.provider,
            cause,
      ...
      return {
        ...
        source: nativeResult?.source ? `${nativeResult.source}+cortex.catalog` : "cortex.catalog",
    ...
    Effect.gen(function* () {
      ...
      if (!(yield* providerIsEnabled(parsed.provider))) {
        return yield* new ProviderValidationError({
          operation: "ProviderDiscoveryService.readPlugin",
          issue: `Provider '${parsed.provider}' is disabled in Cortex settings.`,
        });
      }
      const adapter = yield* registry.getByProvider(parsed.provider);
