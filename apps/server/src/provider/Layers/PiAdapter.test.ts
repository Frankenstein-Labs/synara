...
describe("Pi native Cortex gateway tools", () => {
            ...
            tools: [
              {
                ...
                description: "Create Cortex threads.",
...
describe("getPiDiscoverableModels", () => {
  ...
  it("isolates extension providers between sessions that share an agent directory", async () => {
    const agentDir = mkdtempSync(path.join(tmpdir(), "cortex-pi-runtime-isolation-"));
  ...
  it.each([
    ...
    async ({ provider, id, auth }) => {
      const agentDir = mkdtempSync(path.join(tmpdir(), "cortex-pi-bundled-models-"));
      try {
  ...
  it("includes custom-provider models authenticated through auth.json semantics", async () => {
    const agentDir = mkdtempSync(path.join(tmpdir(), "cortex-pi-models-"));
  ...
  it("restores Fable 5 and Opus 4.8 after an extension replaces the Anthropic catalog", async () => {
    const agentDir = mkdtempSync(path.join(tmpdir(), "cortex-pi-anthropic-"));
    const modelsPath = path.join(agentDir, "models.json");
    const authPath = path.join(agentDir, "auth.json");
