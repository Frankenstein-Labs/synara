...
import { ThreadId, TurnId } from "@cortex/contracts";
...
const stripHarnessPrefix = (text: string): string =>
  text.replace(/^<cortex_host_context>[\s\S]*?<\/cortex_host_context>/, "");
...
describe("resolveDevinAdapterTimeouts", () => {
  ...
  it("uses valid environment overrides", () => {
    expect(
      resolveDevinAdapterTimeouts({
        CORTEX_DEVIN_TURN_IDLE_TIMEOUT_MS: "1234",
        CORTEX_DEVIN_TOOL_IDLE_TIMEOUT_MS: "5678",
      }),
    ...
    async (settlement) => {
      ...
      await Effect.runPromise(
        Effect.gen(function* () {
          ...
          expect(settled?.payload?.stopReason).toBe("cortex.devin.wedge-recovery");
