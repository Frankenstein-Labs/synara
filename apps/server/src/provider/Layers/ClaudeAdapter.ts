 ...
 */
import { execProcessFile, spawnProcess } from "@cortex/shared/processRuntime";
import type {
  AgentInfo,
  CanUseTool,
  AgentDefinition,
...
import { buildClaudeSubagentPrompt } from "@cortex/shared/agentMentions";
import {
  Cause,
  DateTime,
  Deferred,
...
import { renderCortexHarnessPolicy } from "../../agentGateway/harnessPolicy.ts";
...
// from these hints (see extractSubagentIdentityHints in @cortex/shared/subagents).
...
export const buildEmbeddedClaudeSystemPromptAppend = (gatewayControlAvailable: boolean) =>
  [
    "You are running inside Cortex, a coding app that embeds the Claude Agent SDK.",
    ...
    renderCortexHarnessPolicy({ gatewayControlAvailable }),
  ].join("\n");
        ...
        if (Exit.isFailure(exit)) {
          if (hasPendingUserInterrupt(context) || isClaudeInterruptedCause(exit.cause)) {
            ...
            if (isClaudeMissingResumeConversationCause(exit.cause)) {
              ...
              // starts a fresh Claude session and bootstraps Cortex's retained
      ...
      Effect.gen(function* () {
        ...
        if (liveSource?.turnState !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            issue:
              "The source Claude session has a turn in flight; Cortex will rebuild the fork from its retained transcript.",
          });
        }
