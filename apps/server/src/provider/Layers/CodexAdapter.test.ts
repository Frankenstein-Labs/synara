...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("normalizes whitespace in configuration warnings at the provider boundary", () =>
    Effect.gen(function* () {
      ...
      assert.equal(firstEvent.value.payload.path, "mcp_servers.cortex");
    }),
  );
  ...
  it.effect("maps Codex hook notifications to bounded canonical lifecycle events", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(started.raw?.payload, { cortexSanitized: true });
      ...
      assert.deepEqual(completed.raw?.payload, { cortexSanitized: true });
    }),
  );
