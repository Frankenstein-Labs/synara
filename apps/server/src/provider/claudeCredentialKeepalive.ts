...
//
// Opt in:   CORTEX_CLAUDE_KEEPALIVE=1
// Tune:     CORTEX_CLAUDE_KEEPALIVE_MINUTES=<n>   (default 30)

import { execProcessFile } from "@cortex/shared/processRuntime";
...
export function isClaudeCredentialKeepaliveEnabled(
  ...
  return platform === "darwin" && envFlagEnabled(env.CORTEX_CLAUDE_KEEPALIVE);
}
...
export function resolveClaudeCredentialKeepaliveIntervalMs(env: NodeJS.ProcessEnv): number {
  const raw = env.CORTEX_CLAUDE_KEEPALIVE_MINUTES?.trim();
...
export function startClaudeCredentialKeepalive(input?: {
  ...
  // Cortex should not do it as background work merely because the app opened.
