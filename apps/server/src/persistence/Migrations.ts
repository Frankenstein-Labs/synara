...
/**
 * A tracker identity that a *released* Cortex build wrote for a migration whose
 ...
 * does the migration Cortex registers at `historicalId` *today* still need to
 ...
 * once the imported tracker's high-water mark reaches Cortex's latest ID,
 * every Cortex migration is skipped silently and startup crashes on missing
 ...
 *
 * Instead, compare the recorded (id, name) pairs against Cortex's lineage and
 ...
 *
 * Divergences caused by Cortex renumbering one of its *own* released
...
export const reconcileMigrationLineage = Effect.gen(function* () {
  ...
  // A released Cortex build may have recorded a migration under an ID it no
  ...
  if (aliasRepairs.length > 0) {
    yield* Effect.logInfo(
      "Migration tracker records a renumbered Cortex migration; repairing tracker metadata in place",
    ).pipe(
      Effect.annotateLogs({
  ...
  if (diverged === undefined) {
    ...
    // a newer Cortex build. Continuing would expose it to stale writable
  ...
  yield* Effect.logWarning(
    "Migration tracker diverges from the Cortex lineage (legacy import); re-running migrations from the divergence point",
