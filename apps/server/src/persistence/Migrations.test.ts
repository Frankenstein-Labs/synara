...
layer("reconcileMigrationLineage", (it) => {
  ...
  // mark is at or beyond Cortex's latest migration ID. The migrator's max-ID
  // gate then skips every Cortex migration — including the #032 self-heal —
  // and startup crashes on the missing env_mode column.
  it.effect("re-runs skipped migrations when an imported tracker outruns Cortex's latest ID", () =>
    Effect.gen(function* () {
      ...
      // Record a foreign lineage from 17 through past Cortex's latest ID.
      const latestCortexId = Math.max(...migrationEntries.map(([id]) => id));
      for (let id = 17; id <= latestCortexId + 3; id++) {
        yield* sql`
      ...
      // The tracker now mirrors the Cortex lineage exactly; foreign rows are gone.
  ...
  it.effect("refuses writable migration startup for a newer Cortex schema", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      ...
      yield* sql`
        INSERT INTO effect_sql_migrations (migration_id, name)
        VALUES (${futureId}, 'FutureCortexMigration')
      `;
