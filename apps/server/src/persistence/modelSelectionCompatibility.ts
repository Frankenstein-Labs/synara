...
import { MODEL_OPTIONS_BY_PROVIDER } from "@cortex/contracts";
...
// Imported instance ids may be runtime names rather than Cortex provider literals.
...
export function normalizePersistedModelSelection(input: unknown): unknown {
  ...
  // Newer Cortex writes provider-less selections as { instanceId, model } and
  // option rows as [{ id, value }]; Cortex stores canonical provider/options objects.
