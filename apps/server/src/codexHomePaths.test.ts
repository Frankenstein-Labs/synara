...
import {
  ...
  resolveCortexCodexHomeOverlayPath,
...
describe("Codex home paths", () => {
  ...
  it("anchors the overlay under CORTEX_HOME", () => {
    assert.equal(
      resolveCortexCodexHomeOverlayPath({ CORTEX_HOME: "/cortex/runtime" }, "/users/me/.codex"),
      path.join("/cortex/runtime", "codex-home-overlay"),
    );
  });

  it("derives a default overlay beside the source home", () => {
    assert.equal(
      resolveCortexCodexHomeOverlayPath({}, "/users/me/.codex"),
      path.join("/users/me", ".cortex", "runtime", "codex-home-overlay"),
    );
  });
  ...
  it("uses the isolated overlay as Codex's write home", () => {
    assert.equal(
      resolveActiveCodexHomeWritePath({
        env: { CORTEX_HOME: "/cortex/runtime" },
  ...
  it("allowlists source and overlay homes when distinct", () => {
    assert.deepEqual(
      ...
      ["/users/me/.codex", path.join("/cortex/runtime", "codex-home-overlay")],
    );
  });
});
