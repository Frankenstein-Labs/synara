  ...
  const captureCheckpointOnce: CheckpointStoreShape["captureCheckpoint"] = (input) =>
    Effect.gen(function* () {
      ...
      yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "cortex-fs-checkpoint-" }),
        (tempDir) =>
          Effect.gen(function* () {
            ...
            const commitEnv: NodeJS.ProcessEnv = {
              ...
              GIT_AUTHOR_EMAIL: "cortex@users.noreply.github.com",
              GIT_COMMITTER_NAME: "Cortex",
              GIT_COMMITTER_EMAIL: "cortex@users.noreply.github.com",
            };
            ...
            const message = `Cortex checkpoint ref=${input.checkpointRef}`;
  ...
  const reverseCheckpointDiff: CheckpointStoreShape["reverseCheckpointDiff"] = (input) =>
    Effect.gen(function* () {
      ...
      return yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "cortex-checkpoint-undo-" }),
        (tempDir) =>
          Effect.gen(function* () {
