...
describe("managed checkpoint refs", () => {
  ...
  it("creates canonical Cortex refs", () => {
    expect(checkpointRefForThreadTurn(threadId, 4)).toMatch(/^refs\/cortex\/checkpoints\//);
  });
  ...
  it("recognizes a structurally valid persisted ref for the same thread", () => {
    ...
    const historical = canonical.replace("refs/cortex/", "refs/historical/");
