...
import {
  ...
  fetchCortexServerStatus,
  formatCortexServerStatus,
...
describe("server status CLI probe", () => {
  it("reports a ready server from the unauthenticated health endpoint", async () => {
    ...
    const result = await fetchCortexServerStatus({ fetch });
    ...
    expect(formatCortexServerStatus(result)).toBe(
      "Cortex server: ready\nURL: http://127.0.0.1:3773\nProjection: healthy",
    );
  });

  it("reports a reachable server that is still starting", async () => {
    const result = await fetchCortexServerStatus({
      fetch: async () =>
        Response.json({
          status: "ok",
          startupReady: false,
    ...
    expect(formatCortexServerStatus(result)).toContain("Cortex server: starting");
  });
  ...
  it("distinguishes an unhealthy projection from a server that is still starting", async () => {
    ...
    expect(result).toMatchObject({ reachable: true, ready: false });
    expect(formatCortexServerStatus(result)).toContain("Cortex server: not ready");
  });
  ...
  it("normalizes an explicit server base URL to /health", async () => {
    ...
    await fetchCortexServerStatus({
      url: "https://cortex.example.com/some/path?ignored=1#hash",
      fetch,
    });

    expect(fetch).toHaveBeenCalledWith(
      "https://cortex.example.com/health",
  ...
  it("rejects credentials and never reports URL secrets", async () => {
    const result = await fetchCortexServerStatus({
      url: "https://user:password@cortex.example.com/path?token=secret#fragment",
    });
  ...
  it("fails closed for invalid URLs and malformed health responses", async () => {
    await expect(fetchCortexServerStatus({ url: "file:///tmp/cortex" })).resolves.toMatchObject({
      reachable: false,
      ready: false,
    ...
    await expect(
      fetchCortexServerStatus({ fetch: async () => Response.json({ status: "ok" }) }),
    ).resolves.toMatchObject({
      reachable: false,
      ready: false,
      error: "Health response did not match the Cortex health shape.",
    });
  });
  ...
  it("reports HTTP and transport failures without throwing", async () => {
    ...
    expect(formatCortexServerStatus(result)).toContain("Cortex server: unreachable");
  });
});
