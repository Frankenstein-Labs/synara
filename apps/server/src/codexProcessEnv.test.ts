...
describe("linkOrCopyCodexOverlayEntry", () => {
  it("copies auth.json when symlink creation is unavailable", async () => {
    ...
    await linkOrCopyCodexOverlayEntry(
      {
        ...
        targetPath: "C:\\Users\\test\\.cortex\\codex-home-overlay\\auth.json",
        type: "file",
      },
      { symlink, copyFile },
    );
    ...
    expect(symlink).toHaveBeenCalledWith(
      ...
      "C:\\Users\\test\\.cortex\\codex-home-overlay\\auth.json",
      "file",
    );
      ...
      linkOrCopyCodexOverlayEntry(
        {
          ...
          targetPath: "C:\\Users\\test\\.cortex\\codex-home-overlay\\sessions",
          type: "dir",
        },
...
describe("buildCodexProcessEnv", () => {
  it("registers the active custom provider env key for diagnostic redaction", async () => {
    const codexHome = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-provider-key-"));
    writeFileSync(
  ...
  it("keeps a user-provided CODEX_SQLITE_HOME for the session overlay", async () => {
    const codexHome = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-sqlite-home-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "cortex-runtime-home-"));
    const sqliteHome = mkdtempSync(path.join(os.tmpdir(), "cortex-user-sqlite-home-"));
    writeFileSync(path.join(codexHome, "config.toml"), 'model = "gpt-5.5"', "utf8");

    try {
      const env = await buildCodexProcessEnv({
        env: { CORTEX_HOME: runtimeHome, CODEX_SQLITE_HOME: sqliteHome },
        homePath: codexHome,
        platform: "win32",
      });
  ...
  it("replaces a user-defined Cortex MCP table only inside the session overlay", async () => {
    const sourceHome = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-source-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-runtime-"));
    const sourceConfig = [
      'model = "gpt-5.5"',
      "",
      "[mcp_servers.cortex]",
    ...
    const managedConfig = [
      ...
      'bearer_token_env_var = "CORTEX_AGENT_GATEWAY_TOKEN"',
      "",
      "[shell_environment_policy]",
      'exclude = ["CORTEX_AGENT_GATEWAY_TOKEN"]',
    ].join("\n");
    ...
    try {
      ...
      if (!overlayHome) {
        throw new Error("Expected a Cortex Codex home overlay.");
      }
      const overlayConfig = readFileSync(path.join(overlayHome, "config.toml"), "utf8");

      expect(overlayConfig.match(/^\[mcp_servers\.cortex\]$/gm)).toHaveLength(1);
      expect(overlayConfig).toContain('url = "http://127.0.0.1:3773/mcp"');
      expect(overlayConfig).toContain('bearer_token_env_var = "CORTEX_AGENT_GATEWAY_TOKEN"');
      ...
      expect(overlayConfig).not.toContain("[mcp_servers.cortex.headers]");
      expect(overlayConfig).not.toContain("[mcp_servers.cortex.env]");
      ...
      expect(overlayConfig).toContain('exclude = ["CORTEX_AGENT_GATEWAY_TOKEN", "USER_SECRET"]');
