...
export const MODEL_SELECTION_INPUT_SCHEMA = {
  ...
  properties: {
    ...
    model: {
      type: "string",
      description: "Exact model slug from cortex_capabilities providers[].models[].slug.",
    },
    options: {
      type: "object",
      description: AGENT_GATEWAY_TARGET_OPTIONS_DESCRIPTION,
...
export function decodeCreateThreadsInput(value: unknown) {
  try {
    return Schema.decodeUnknownSync(CortexCreateThreadsInput)(value);
  } catch (error) {
    throw new ToolInputError(`Invalid Cortex creation plan: ${errorText(error)}`);
  }
}

export function decodeWaitForThreadsInput(value: unknown) {
  try {
    return Schema.decodeUnknownSync(CortexWaitForThreadsInput)(value);
  } catch (error) {
    throw new ToolInputError(`Invalid Cortex wait request: ${errorText(error)}`);
  }
}
