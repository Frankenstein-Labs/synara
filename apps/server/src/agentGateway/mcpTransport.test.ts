...
import { ProjectId, ThreadId, TurnId, type OrchestrationThreadShell } from "@cortex/contracts";
...
describe("makeAgentGatewayMcpTransport tools/list schema sanitization", () => {
  it.effect("serves sanitized schemas while keeping stored definitions dirty", () =>
    Effect.gen(function* () {
      ...
      const listed = findToolOrThrow(response.body.result.tools, "cortex_recursive");
