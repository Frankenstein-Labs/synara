/**
 * Stdio-to-HTTP proxy script for the Cortex agent gateway.
 *
...
const STDIO_PROXY_SCRIPT = `// Cortex agent gateway stdio<->HTTP MCP proxy (generated file, do not edit).
const url = process.env.CORTEX_AGENT_GATEWAY_URL;
let token = process.env.CORTEX_AGENT_GATEWAY_TOKEN;
let bootstrapToken = process.env.CORTEX_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
...
function localInactiveResponse(message) {
  ...
  // CLI launched outside Cortex therefore sees a valid empty MCP server
  // instead of a noisy failed integration. Cortex-managed processes receive
  ...
  return [
    {
      jsonrpc: "2.0",
      id,
      error: { code: -32601, message: "Cortex is not active for this Antigravity session." },
    },
  ];
}
...
async function resolveToken() {
  ...
  if (!tokenResolution) {
    ...
    tokenResolution = (async () => {
      ...
      if (!response.ok) {
        throw new Error("Cortex gateway bootstrap failed with HTTP " + response.status);
      }
      const payload = await response.json();
      if (!isRecord(payload) || typeof payload.bearerToken !== "string") {
        throw new Error("Cortex gateway bootstrap returned an invalid response");
      }
      token = payload.bearerToken;
      bootstrapToken = undefined;
      delete process.env.CORTEX_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
      return token;
    })().finally(() => {
    ...
    return [
      {
        jsonrpc: "2.0",
        id,
        error: { code: -32603, message: "Cortex gateway request failed: " + String(error) },
      },
    ];
  }
}
