...
import { CORTEX_HARNESS_POLICY_VERSION } from "./harnessPolicy.ts";
...
export function makeThreadReadTools(input: ThreadReadToolsInput): ReadonlyArray<ToolEntry> {
  ...
  const contextTool: ToolEntry = {
    ...
    definition: {
      name: "cortex_context",
      description:
        "Inspect the current Cortex harness identity, caller thread/turn, and authorized coordination capabilities.",
      inputSchema: { type: "object", properties: {}, additionalProperties: false },
      annotations: {
        title: "Cortex context",
        readOnlyHint: true,
    ...
    handler: (_args, context) =>
      Effect.gen(function* () {
        ...
        return mcpToolResultJson({
          harness: { name: "Cortex", policyVersion: CORTEX_HARNESS_POLICY_VERSION },
          caller: {
            threadId: caller.id,
            turnId,
  ...
  const capabilitiesTool: ToolEntry = {
    ...
    definition: {
      ...
      description: `List canonical Cortex provider/model targets, exact provider option keys, examples, and gateway limits used to validate thread creation. ${AGENT_GATEWAY_TARGET_OPTIONS_DESCRIPTION}`,
      inputSchema: { type: "object", properties: {}, additionalProperties: false },
      annotations: {
    ...
    handler: (_args, context) =>
      Effect.gen(function* () {
        ...
        return mcpToolResultJson({
          ...
          providers,
          limits: {
            maxThreadsPerOperation: CORTEX_GATEWAY_MAX_THREADS_PER_OPERATION,
            maxWaitMs: 60_000,
  ...
  const listProjects: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "List Cortex projects (id, title, workspace root). System-managed containers (the Chats and Studio surfaces) are not projects and are excluded. Use before creating a thread in another project.",
      inputSchema: { type: "object", properties: {}, additionalProperties: false },
      annotations: { title: "List Cortex projects", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: () =>
  ...
  const listThreads: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "Discover Cortex threads by project, hierarchy, provider, model, status, title, creation source, or update window. Archived threads are hidden unless includeArchived is true.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "List Cortex threads", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
  ...
  const readThread: ToolEntry = {
    ...
    definition: {
      ...
      description: `Read one Cortex thread's status and recent messages (newest last). Pass nextCursor as cursor to page older messages. To read one settled long message losslessly, pass the summary's index, messageId, and messageVersion with messageOffsetChars 0, then follow messagePage.nextOffsetChars with the same identity and version.`,
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Read a Cortex thread", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
                ...
                return Effect.succeed(
                  pinned.map((pin) => {
                    ...
                    return {
                      ...
                      readThread: {
                        tool: "cortex_read_thread" as const,
