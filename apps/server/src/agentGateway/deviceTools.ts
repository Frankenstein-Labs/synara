...
/**
 * Providers whose sessions run without a per-tool approval gate. Cortex cannot
...
const DEVICE_BUILD_AGENT_GUIDANCE =
  "Cortex never builds the app; when needed, build it first with xcodebuild or the project's build tool. This tool surfaces the driven device in the pane.";
  ...
  const tools: ToolEntry[] = [
    {
      ...
      definition: {
        name: "device_list",
        description:
          "List iOS simulators Cortex can drive, with their runtime, boot state, and who booted them. Call this before any other device_* tool and use an already-booted device when one is available; booting another wastes time and can leave the user watching the wrong pane.",
        inputSchema: {
          type: "object",
          properties: {
            includeShutdown: {
    ...
    },
    {
      ...
      definition: {
        ...
        description: approvalRequiredDeviceDescription(
          'Boot a simulator only when device_list finds nothing booted or the user named a different device. Cortex caps the simulators it boots; kind "boot-limit-reached" lists devices to relay to the user so they can choose one to shut down, and must not be retried.',
        ),
        inputSchema: {
          type: "object",
    ...
    },
    {
      ...
      definition: {
        ...
        description: deviceInputDescription(
          "Scroll a labelled element into view in one call. Cortex swipes and re-reads the tree until the element reaches the tappable band, then returns its tap point. Use this instead of a device_swipe loop when reading or confirming content below the fold; labelled device_tap already scrolls on its own.",
        ),
        inputSchema: {
          type: "object",
          properties: {
