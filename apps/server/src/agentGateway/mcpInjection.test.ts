...
import {
  ...
  CORTEX_AGENT_GATEWAY_TOKEN_ENV,
...
describe("agent gateway MCP injection", () => {
  ...
  it("builds a codex config block that references the token env var, not the token", () => {
    ...
    assert.include(block, "[mcp_servers.cortex]");
    assert.include(block, `url = "${connection.url}"`);
    assert.include(block, `bearer_token_env_var = "${CORTEX_AGENT_GATEWAY_TOKEN_ENV}"`);
  ...
  it("appends the codex section once and keeps existing config intact", () => {
    ...
    assert.include(appended, "[mcp_servers.cortex]");

    const reappended = appendCodexConfigSection(appended, section);
    assert.equal(reappended.split("[mcp_servers.cortex]").length, 2);
  });
  ...
  it("merges the token exclusion into a user-defined shell environment policy", () => {
    ...
    const merged = mergeShellEnvPolicyExclude(withExclude, CORTEX_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${CORTEX_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`);

    // Idempotent: the var is not added twice.
    assert.equal(mergeShellEnvPolicyExclude(merged, CORTEX_AGENT_GATEWAY_TOKEN_ENV), merged);

    // A policy table without an exclude key gains one.
    const withoutExclude = ["[shell_environment_policy]", 'inherit = "core"'].join("\n");
    const gained = mergeShellEnvPolicyExclude(withoutExclude, CORTEX_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(gained, `exclude = ["${CORTEX_AGENT_GATEWAY_TOKEN_ENV}"]`);
    assert.include(gained, 'inherit = "core"');

    // No policy table: unchanged (the managed section appends its own).
    assert.equal(
      mergeShellEnvPolicyExclude('[model]\nname = "gpt-5.5"', CORTEX_AGENT_GATEWAY_TOKEN_ENV),
      '[model]\nname = "gpt-5.5"',
    );
  });
  ...
  it("ignores commented and unrelated token references when merging shell exclusions", () => {
    const commentedExample = [
      "[shell_environment_policy]",
      `# exclude = ["${CORTEX_AGENT_GATEWAY_TOKEN_ENV}"]`,
      'exclude = ["AWS_*"]',
    ].join("\n");
    ...
    assert.include(
      ...
      `exclude = ["${CORTEX_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`,
    );

    const unrelatedString = [
      "[shell_environment_policy]",
      `note = "keep ${CORTEX_AGENT_GATEWAY_TOKEN_ENV} private"`,
      'exclude = ["AWS_*"]',
    ].join("\n");
  ...
  it("recognizes only active exact entries in multiline shell exclusion arrays", () => {
    ...
    assert.equal(mergeShellEnvPolicyExclude(existing, CORTEX_AGENT_GATEWAY_TOKEN_ENV), existing);
    ...
    const merged = mergeShellEnvPolicyExclude(tokenOnlyInComment, CORTEX_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${CORTEX_AGENT_GATEWAY_TOKEN_ENV}",`);
  });

  it("detects real TOML table headers, ignoring comments and strings", () => {
    assert.isTrue(
      configHasTomlTableHeader('[mcp_servers.cortex]\nurl = "x"', "[mcp_servers.cortex]"),
    );
    assert.isTrue(
      configHasTomlTableHeader("  [mcp_servers.cortex]  # managed", "[mcp_servers.cortex]"),
    );
    assert.isTrue(
      configHasTomlTableHeader("  [ mcp_servers.cortex ]  # managed", "[mcp_servers.cortex]"),
    );
    ...
    assert.isFalse(configHasTomlTableHeader('["mcp_servers.cortex"]', "[mcp_servers.cortex]"));
    assert.isFalse(configHasTomlTableHeader('[mcp_servers."syn\\qara"]', "[mcp_servers.cortex]"));
    // A commented-out example block must not count as the table being present.
    assert.isFalse(configHasTomlTableHeader("# [mcp_servers.cortex]", "[mcp_servers.cortex]"));
    assert.isFalse(
      configHasTomlTableHeader('note = "see [mcp_servers.cortex] docs"', "[mcp_servers.cortex]"),
    );
  });
                ...
                tools: [
                  {
                    ...
                    description: "List Cortex threads.",
  ...
  it("uses the ACP http transport when the agent advertises support", () => {
    const servers = buildAcpCortexMcpServers({
      connection,
    ...
    assert.deepEqual(servers, [
      {
        ...
        args: stdioProxy.args,
        env: [
          { name: "CORTEX_AGENT_GATEWAY_URL", value: connection.url },
          { name: CORTEX_AGENT_GATEWAY_TOKEN_ENV, value: connection.bearerToken },
        ],
      },
    ]);
  });
