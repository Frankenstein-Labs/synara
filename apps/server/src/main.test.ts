...
import { CliConfig, makeServerStartupLogData, cortexCli, type CliConfigShape } from "./main";
...
let defaultCortexHome = "";
const tempHomes = new Set<string>();

function makeTempHome(prefix = "cortex-main-test-"): string {
...
const runCli = (args: ReadonlyArray<string>, env: Record<string, string> = {}) => {
  const program = Command.runWith(cortexCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            CORTEX_HOME: defaultCortexHome,
            CORTEX_NO_BROWSER: "true",
            ...env,
          },
        }),
      ),
...
beforeEach(() => {
  vi.clearAllMocks();
  defaultCortexHome = makeTempHome();
  resolvedConfig = null;
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("cortex-main-flag-");
  ...
  it.effect("passes the root --home-dir flag to MCP subcommands", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("cortex-main-mcp-flag-");
  ...
  it.effect("discovers the persisted runtime origin for the server status subcommand", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("cortex-main-status-flag-");
  ...
  it.effect("rejects a discovered endpoint that cannot prove the persisted runtime identity", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("cortex-main-status-proof-");
      const stateDir = path.join(flagHome, "userdata");
      fs.mkdirSync(stateDir, { recursive: true, mode: 0o700 });
      fs.chmodSync(stateDir, 0o700);
      fs.writeFileSync(
  ...
  it.effect("reports an unreachable result when no persisted runtime exists", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("cortex-main-status-missing-");
      const previousExitCode = process.exitCode;
      const output: string[] = [];
      const stdout = vi.spyOn(process.stdout, "write").mockImplementation((chunk) => {
        output.push(String(chunk));
  ...
  it.effect("creates fresh local state directories with private permissions", () =>
    Effect.gen(function* () {
      ...
      const homeDir = makeTempHome("cortex-main-private-fresh-");
  ...
  it.effect("repairs permissions for an upgraded local state directory", () =>
    Effect.gen(function* () {
      if (process.platform === "win32") return;
      const homeDir = makeTempHome("cortex-main-private-upgrade-");
      const stateDir = path.join(homeDir, "userdata");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      const envHome = makeTempHome("cortex-main-env-");
      ...
      yield* runCli([], {
        ...
        CORTEX_NO_BROWSER: "true",
        CORTEX_AUTH_TOKEN: "env-token",
        CORTEX_DESKTOP_SHUTDOWN_TOKEN: "shutdown-token",
        CORTEX_MIGRATION_DIVERGENCE_CONSENT: "migration-consent",
      });
  ...
  it.effect("consumes desktop shutdown authority before generic child launches", () =>
    Effect.gen(function* () {
      const canonicalKey = "CORTEX_DESKTOP_SHUTDOWN_TOKEN";
      const mixedCaseKey = "cOrTeX_dEsKtOp_ShUtDoWn_ToKeN";
      ...
      try {
        ...
        resolvedConfig = null;
        yield* runCli([], { CORTEX_DESKTOP_SHUTDOWN_TOKEN: injectedToken });
  ...
  it.effect("consumes migration divergence consent before generic child launches", () =>
    Effect.gen(function* () {
      const environmentKey = "CORTEX_MIGRATION_DIVERGENCE_CONSENT";
  ...
  it.effect("omits both server authority secrets from startup log data", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        CORTEX_AUTH_TOKEN: "browser-secret",
        CORTEX_DESKTOP_SHUTDOWN_TOKEN: "shutdown-secret",
        CORTEX_MIGRATION_DIVERGENCE_CONSENT: "migration-secret",
      });
  ...
  it.effect("prefers --mode over CORTEX_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over CORTEX_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        CORTEX_NO_BROWSER: "false",
      });
    ...
    Effect.gen(function* () {
      yield* runCli(
        ...
        ],
        {
          CORTEX_MODE: "desktop",
          CORTEX_NO_BROWSER: "true",
          CORTEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
          CORTEX_LOG_PROVIDER_EVENTS: "true",
          CORTEX_LOG_WS_EVENTS: "true",
        },
      );
  ...
  it.effect("honors insecure remote opt-in from the environment when the CLI flag is absent", () =>
    Effect.gen(function* () {
      yield* runCli(["--host", "0.0.0.0", "--auth-token", "remote-secret"], {
        CORTEX_ALLOW_INSECURE_REMOTE: "true",
        CORTEX_NO_BROWSER: "true",
      });
  ...
  it.effect("uses the HTTPS public origin for remote startup pairing", () =>
    Effect.gen(function* () {
      ...
      assert.equal(resolvedConfig?.publicUrl?.origin, "https://cortex.example.test");
      assert.equal(openBrowser.mock.calls.length, 1);
      assert.match(
        openBrowser.mock.calls[0]?.[0] ?? "",
        /^https:\/\/cortex\.example\.test\/pair#token=/,
      );
    }),
  );
  ...
  it.effect("supports the HTTPS public origin through environment configuration", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        CORTEX_HOST: "192.168.1.50",
        CORTEX_AUTH_TOKEN: "remote-secret",
  ...
  it.effect("rejects non-root or non-HTTPS public URLs", () =>
    Effect.gen(function* () {
      for (const publicUrl of ["http://cortex.example.test", "https://cortex.example.test/app"]) {
        const error = yield* Effect.flip(
  ...
  it.effect("refuses non-loopback exposure without authentication", () =>
    Effect.gen(function* () {
      ...
      assert.match(String(error), /Refusing to bind Cortex to non-loopback host 0\.0\.0\.0/);
    }),
  );
  ...
  it.effect("supports CLI and env for bootstrap/provider-log/websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        CORTEX_MODE: "desktop",
        CORTEX_LOG_PROVIDER_EVENTS: "true",
        CORTEX_LOG_WS_EVENTS: "false",
        CORTEX_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        CORTEX_NO_BROWSER: "true",
      });
  ...
  it.effect("rejects invalid boolean environment values instead of treating them as absent", () =>
    Effect.gen(function* () {
      const error = yield* Effect.flip(
        runCli([], {
          CORTEX_LOG_PROVIDER_EVENTS: "sometimes",
        }),
      );
