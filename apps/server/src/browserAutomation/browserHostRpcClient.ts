...
import type { BrowserToolName, ProviderKind, ThreadId } from "@cortex/contracts";
...
function assertCompatibleHostInfo(value: unknown, expectedSessionId: string): void {
  ...
  if (
    protocolVersion !== 1 ||
    (info?.type !== undefined && info.type !== "cortex-browser-host") ||
...
export function resolveBrowserHostPipePath(env: NodeJS.ProcessEnv = process.env): string | null {
  const configured =
    env.CORTEX_BROWSER_HOST_PIPE_PATH?.trim() || env.CORTEX_BROWSER_USE_PIPE_PATH?.trim();
...
export function resolveBrowserHostCapability(env: NodeJS.ProcessEnv = process.env): string | null {
  const direct = env.CORTEX_BROWSER_HOST_CAPABILITY?.trim();
  if (direct && Buffer.byteLength(direct, "utf8") >= 32) return direct;

  const rawFd = env.CORTEX_BROWSER_HOST_CAPABILITY_FD?.trim();
