...
describe("resolveAllowedLocalPreviewFile", () => {
  it("allows images inside the current workspace", async () => {
    const workspace = makeTempDir("cortex-image-workspace-");
  ...
  it("allows images inside Codex generated_images without a cwd", async () => {
    const codexHome = makeTempDir("cortex-codex-home-");
  ...
  it("allows images written to the CORTEX_HOME codex-home-overlay generated_images root", async () => {
    // Codex app-server is launched with CODEX_HOME pointing at a Cortex overlay
    // directory (see resolveCortexCodexHomeOverlayPath). Generated images therefore
    // live under <CORTEX_HOME>/codex-home-overlay/generated_images/<thread>/<call>.png,
    ...
    const cortexHome = path.join(fakeRoot, "cortex", "runtime");
  ...
  it("allows PDFs inside the current workspace", async () => {
    const workspace = makeTempDir("cortex-pdf-workspace-");
    writeFileSync(path.join(workspace, ".git"), "gitdir: .git");
  ...
  it("allows PDFs inside a per-thread scratch workspace without a cwd", async () => {
    ...
    // <tmpdir>/cortex-codex-workspaces/<threadId>; files agents create there
    // are workspace-equivalent, so documents must be servable from that root.
    const scratchRoot = path.join(os.tmpdir(), "cortex-codex-workspaces");
  ...
  it("allows PDFs inside the configured private scratch root without a cwd", async () => {
    const privateTempRoot = makeTempDir("cortex-private-scratch-");
    const scratchRoot = path.join(privateTempRoot, "cortex-codex-workspaces");
  ...
  it("rejects PDFs outside the workspace even under the temp-dir image roots", async () => {
    ...
    const tempDir = makeTempDir("cortex-pdf-outside-");
  ...
  it("still allows images under the temp-dir roots without a workspace", async () => {
    const tempDir = makeTempDir("cortex-image-tmp-root-");
