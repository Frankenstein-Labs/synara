...
describe("remote access policy", () => {
  ...
  it("requires authentication when an HTTPS proxy publishes a loopback bind", () => {
    ...
    ).toContain("without CORTEX_AUTH_TOKEN");
  });

  it("rejects invalid public URLs in the shared embedded-server policy", () => {
    for (const publicUrl of [
      new URL("http://cortex.example.test/"),
      new URL("https://cortex.example.test/app"),
    ]) {
      expect(
        remoteAccessPolicyError({
          ...remoteBase,
  ...
  it("accepts only credential-free HTTPS root origins", () => {
    expect(normalizeHttpsPublicOrigin(new URL("https://cortex.example.test/"))?.origin).toBe(
    ...
    for (const value of [
      ...
      "https://cortex.example.test/?query=1",
      "https://cortex.example.test/#fragment",
    ]) {
