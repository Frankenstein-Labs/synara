...
describe("Codex Cortex harness policy", () => {
  it("keeps the same host policy exactly once in default and plan instructions", () => {
    for (const instructions of [
      CODEX_DEFAULT_MODE_DEVELOPER_INSTRUCTIONS,
      CODEX_PLAN_MODE_DEVELOPER_INSTRUCTIONS,
    ]) {
      expect(instructions).toContain(CORTEX_HARNESS_POLICY_MARKER);
      expect(instructions.split(CORTEX_HARNESS_POLICY_MARKER)).toHaveLength(2);
      expect(instructions).toContain("Cortex is the host and harness");
      ...
      expect(instructions).toContain("one exact cortex_create_threads plan");
      expect(instructions).toContain("tools.mcp__cortex__browser_open");
  ...
  it("resolves the gateway endpoint when each session environment is built", async () => {
    const homePath = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-gateway-endpoint-"));
    const previousCortexHome = process.env.CORTEX_HOME;
    process.env.CORTEX_HOME = path.join(homePath, "cortex-home");
    ...
    try {
      ...
      if (previousCortexHome === undefined) {
        delete process.env.CORTEX_HOME;
      } else {
        process.env.CORTEX_HOME = previousCortexHome;
      }
...
describe("codex CLI version gate", () => {
  it("memoizes the version probe per binary and shares concurrent probes", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-version-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("CORTEX_HOME", path.join(dir, "runtime"));
  ...
  it("does not reuse a general-version verdict for the stricter Auto floor", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-version-auto-floor-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("CORTEX_HOME", path.join(dir, "runtime"));
  ...
  it("fails closed for Auto when the Codex CLI version cannot be parsed", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-version-auto-unknown-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("CORTEX_HOME", path.join(dir, "runtime"));
  ...
  it("re-probes when the binary behind an unchanged path is replaced", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-version-swap-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("CORTEX_HOME", path.join(dir, "runtime"));
    ...
    try {
      ...
      await expect(
        assertSupportedCodexCliVersion({ binaryPath, cwd: dir, homePath }),
      ).rejects.toThrow(/too old for Cortex/);
    } finally {
      reset();
      vi.unstubAllEnvs();
      rmSync(dir, { recursive: true, force: true });
  ...
  it("re-probes when a PATH-resolved codex is replaced behind the same bare name", async () => {
    ...
    const dir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-version-path-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("CORTEX_HOME", path.join(dir, "runtime"));
  ...
  it("rejects an unsupported codex version without caching the failure", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-version-old-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("CORTEX_HOME", path.join(dir, "runtime"));
...
describe("buildCodexProcessEnv", () => {
  it("hydrates the active custom provider env_key from the effective CODEX_HOME", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-env-"));
    try {
      writeFileSync(
  ...
  it("keeps the private desktop browser host out of the Codex process", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-private-host-"));
    ...
    try {
      const env = await buildCodexProcessEnv({
        env: {
          ...
          CORTEX_HOME: tempDir,
          CORTEX_BROWSER_HOST_PIPE_PATH: "/tmp/cortex-browser-host.sock",
          CORTEX_BROWSER_USE_PIPE_PATH: "/tmp/legacy-browser-use.sock",
      ...
      expect(env.CORTEX_BROWSER_HOST_PIPE_PATH).toBeUndefined();
      expect(env.CORTEX_BROWSER_USE_PIPE_PATH).toBeUndefined();
      expect(env.CORTEX_BROWSER_HOST_CAPABILITY).toBeUndefined();
      expect(env.CORTEX_BROWSER_HOST_CAPABILITY_FD).toBeUndefined();
  ...
  it("applies durable section suppressions inside Cortex's Codex overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "cortex-runtime-home-"));
    try {
      writeFileSync(
        path.join(tempDir, "config.toml"),
        [
          ...
          "enabled = true",
          "",
          ...CORTEX_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS.flatMap((header) => [
            header,
            "enabled = true",
            "",
          ]),
      ...
      writeFileSync(
        path.join(overlayHome, "cortex-config-suppressions-v1.json"),
        `${JSON.stringify({
          version: 1,
      ...
      for (const header of CORTEX_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS) {
        expect(readFileSync(path.join(codexHome, "config.toml"), "utf8")).toContain(
          `${header}\nenabled = false`,
        );
  ...
  it("seeds markerless suppressions for conflicting local browser plugins", async () => {
    ...
    try {
      ...
      const suppressionMarker = JSON.parse(
        readFileSync(path.join(overlayHome, "cortex-config-suppressions-v1.json"), "utf8"),
  ...
  it("keeps Codex SQLite state out of Cortex's Codex home overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "cortex-runtime-home-"));
    ...
    try {
      ...
      // A regular database file in the overlay is not Cortex's to destroy.
  ...
  it("repairs stale auth.json files in Cortex's Codex home overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "cortex-runtime-home-"));
    try {
  ...
  it("preserves real generated image directories in Cortex's Codex home overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "cortex-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "cortex-runtime-home-"));
    try {
      writeFileSync(path.join(tempDir, "config.toml"), 'model = "gpt-5.5"', "utf8");
...
describe("handleStdoutLine", () => {
  ...
  it("ignores multiline and standalone JSON leaked from command output", () => {
    ...
    for (const line of ["{", "[", '{"scripts": {', "{}", "[]", '{"name":"cortex"}']) {
      handleStdoutLine(context, line);
    }
...
describe("startSession", () => {
  ...
  it("uses an isolated scratch workspace path when no cwd is provided", () => {
    ...
    expect(cwd).toContain(`${path.sep}cortex-codex-workspaces${path.sep}thread-1`);
  });

  it("reports a missing project working directory instead of a missing Codex CLI", () => {
    const missingCwd = path.join(os.tmpdir(), `cortex-missing-cwd-${randomUUID()}`, "old-project");
  ...
  it("accepts an existing project working directory", () => {
    const cwd = mkdtempSync(path.join(os.tmpdir(), "cortex-existing-cwd-"));
    try {
  ...
  it("fails session start with missing-cwd guidance instead of missing Codex CLI", async () => {
    ...
    const missingCwd = path.join(
      os.tmpdir(),
      `cortex-missing-session-cwd-${randomUUID()}`,
      "old-project",
    );
  ...
  it("fails fast with an upgrade message when codex is below the minimum supported version", async () => {
    ...
    const versionCheck = vi
      ...
      .mockImplementation(() => {
        throw new Error(
          "Codex CLI v0.36.0 is too old for Cortex. Upgrade to v0.37.0 or newer and restart Cortex.",
        );
      });
