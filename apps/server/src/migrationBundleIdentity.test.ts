...
import { migrationRuntimeSourceDigest } from "@cortex/shared/migrationRecovery";
...
function sourceCheckout(source: string): string {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "cortex-migration-bundle-"));
  tempDirectories.push(cwd);
  const sourcePath = path.join(cwd, "apps/server/src/persistence/Migrations.ts");
  fs.mkdirSync(path.dirname(sourcePath), { recursive: true });
  fs.writeFileSync(path.join(cwd, "package.json"), '{"name":"@cortex/monorepo"}\n');
...
describe("verifyMigrationRuntimeIdentity", () => {
  ...
  it("checks the source checkout when launched from a nested package", () => {
    ...
    fs.writeFileSync(path.join(nestedCwd, "package.json"), '{"name":"@cortex/cli"}\n');
