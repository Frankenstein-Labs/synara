/**
 * Device pane end-to-end verification against a live Cortex server.
 *
...
import {
  ...
  decodeDeviceFrame,
} from "@cortex/shared/deviceFrame";
import {
  DEVICE_WS_METHODS,
...
/**
 * Reboot the simulator behind Cortex's back, the way `simctl` from a terminal,
...
async function screenshotViaSimctl(udid: string): Promise<Buffer> {
  const path = join(tmpdir(), `cortex-device-e2e-${randomUUID()}.png`);
  try {
...
describeE2e("device pane end-to-end", () => {
  ...
  it("boots the target simulator", async () => {
    ...
    expect(booted?.bootSource).toBe("cortex");
  }, 180_000);
  ...
  it("keeps input working after the simulator is rebooted outside Cortex", async () => {
    if (!target) throw new Error("no target device");
