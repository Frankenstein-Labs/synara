...
id CortexEmptyTranslatorResponse(void);
...
static id CortexMsgSend0(id target, NSString *selectorName) {
...
static NSString *CortexStringAttribute(id element, NSString *selectorName) {
  id value = CortexMsgSend0(element, selectorName);
  return [value isKindOfClass:NSString.class] ? value : nil;
}

static BOOL CortexBoolAttribute(id element, NSString *selectorName) {
  SEL selector = NSSelectorFromString(selectorName);
...
static id CortexJSONValue(id value) {
  if (value == nil) {
    return NSNull.null;
  }
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _callbackQueue = dispatch_queue_create("dev.cortex.device-helper.ax", DISPATCH_QUEUE_CONCURRENT);
    _requestTimeout = 5.0;
  }
  return self;
}
...
- (id)accessibilityTranslationDelegateBridgeCallbackWithToken:(NSString *)token {
  ...
  id (^callback)(id) = ^id(id request) {
    if (device == nil) {
      return CortexEmptyTranslatorResponse();
    }
    ...
    return response ?: CortexEmptyTranslatorResponse();
  };
...
- (nullable NSDictionary *)frontmostTreeWithMaxDepth:(NSInteger)maxDepth error:(NSError **)error {
  if (_translator == nil) {
    if (error) {
      *error = [NSError errorWithDomain:@"dev.cortex.device-helper.ax"
                                   code:1
...
- (NSDictionary *)serializeElement:(id)element depth:(NSInteger)depth maxDepth:(NSInteger)maxDepth {
  ...
  NSString *role = CortexStringAttribute(element, @"accessibilityRole");
  ...
  NSString *label = CortexStringAttribute(element, @"accessibilityLabel");
  if (label.length > 0) {
    node[@"label"] = label;
  }
  id value = CortexJSONValue(CortexMsgSend0(element, @"accessibilityValue"));
  if (value != NSNull.null) {
    node[@"value"] = value;
  }
  NSString *identifier = CortexStringAttribute(element, @"accessibilityIdentifier");
  if (identifier.length > 0) {
    node[@"identifier"] = identifier;
  }
  NSString *title = CortexStringAttribute(element, @"accessibilityTitle");
  if (title.length > 0) {
    node[@"title"] = title;
  }
  ...
  NSString *subrole = CortexStringAttribute(element, @"accessibilitySubrole");
  ...
  node[@"enabled"] = @(CortexBoolAttribute(element, @"isAccessibilityEnabled"));
  ...
  id rawChildren = CortexMsgSend0(element, @"accessibilityChildren");
