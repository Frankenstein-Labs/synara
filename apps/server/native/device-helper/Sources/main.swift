// cortex-device-helper — the native side of Cortex's Device Pane.
//
...
final class HelperSession {
  ...
  private(set) var hid: CortexHIDBridge?
  private(set) var accessibility: CortexAXBridge?
  ...
  func requireHID() throws -> CortexHIDBridge {
  ...
  func requireAccessibility() throws -> CortexAXBridge {
...
///
/// Every injection path in CortexHIDBridge returns silently when it has no
...
private func withHIDDelivery(_ hid: CortexHIDBridge, _ body: () -> Void) throws {
...
func handle(method: String, params: Params, session: HelperSession) throws -> Any {
  ...
  case "button":
    ...
    var button = CortexHardwareButton.home
    guard CortexHardwareButtonFromName(name, &button) else {
      throw RPCError(
        .invalidParams,
...
let requestQueue = DispatchQueue(label: "dev.cortex.device-helper.rpc")
