...
typedef union {
  CortexIndigoTouch touch;
  CortexIndigoButton button;
...
typedef struct {
  ...
  unsigned int field3;
  CortexIndigoEvent event;
} CortexIndigoPayload;

typedef struct {
  CortexIndigoMachHeader header;
  unsigned int innerSize;
  unsigned char eventType;
  CortexIndigoPayload payload;
} CortexIndigoMessage;
...
typedef CortexIndigoMessage *(*CortexIndigoButtonFn)(int keyCode, int op, int target);
typedef CortexIndigoMessage *(*CortexIndigoKeyboardFn)(uint32_t usage, int op);
typedef CortexIndigoMessage *(*CortexIndigoMouseFn)(CGPoint *point0, CGPoint *point1, int target,
                                                    int eventType, BOOL extra);
typedef CortexIndigoMessage *(*CortexIndigoArbitraryFn)(int target, uint32_t page, uint32_t usage,
                                                        int op);

BOOL CortexHardwareButtonFromName(NSString *name, CortexHardwareButton *outButton) {
  NSDictionary<NSString *, NSNumber *> *map = @{
    @"home": @(CortexHardwareButtonHome),
    @"lock": @(CortexHardwareButtonLock),
    @"side": @(CortexHardwareButtonSide),
    @"siri": @(CortexHardwareButtonSiri),
    @"volume-up": @(CortexHardwareButtonVolumeUp),
    @"volume-down": @(CortexHardwareButtonVolumeDown),
  };
  ...
  if (outButton != NULL) {
    *outButton = (CortexHardwareButton)found.integerValue;
  }
  return YES;
}

static int CortexButtonSource(CortexHardwareButton button) {
  switch (button) {
    case CortexHardwareButtonHome: return kButtonSourceHome;
    case CortexHardwareButtonLock: return kButtonSourceLock;
    case CortexHardwareButtonSide: return kButtonSourceSide;
    case CortexHardwareButtonSiri: return kButtonSourceSiri;
    case CortexHardwareButtonVolumeUp:
    case CortexHardwareButtonVolumeDown: return -1;
  }
...
static uint32_t CortexConsumerUsage(CortexHardwareButton button) {
  switch (button) {
    case CortexHardwareButtonVolumeUp: return kHIDUsageVolumeIncrement;
    case CortexHardwareButtonVolumeDown: return kHIDUsageVolumeDecrement;
    default: return 0;
  }
}
...
static uint32_t CortexUsageForCharacter(unichar c, BOOL *outShift) {
  BOOL shift = NO;
  uint32_t usage = 0;
...
@implementation CortexHIDBridge {
  id _client;
  dispatch_queue_t _sendQueue;
  CortexIndigoButtonFn _buttonFn;
  CortexIndigoKeyboardFn _keyboardFn;
  CortexIndigoMouseFn _mouseFn;
  CortexIndigoArbitraryFn _arbitraryFn;
...
- (BOOL)attachToDevice:(id)device error:(NSError **)error {
  NSString *developerDir = NSProcessInfo.processInfo.environment[@"CORTEX_DEVELOPER_DIR"];
  ...
  if (kit == NULL) {
    if (error) {
      *error = [NSError errorWithDomain:@"dev.cortex.device-helper.hid"
                                   code:1
  ...
  _arbitraryFn = (CortexIndigoArbitraryFn)dlsym(kit, "IndigoHIDMessageForHIDArbitrary");
  _buttonFn = (CortexIndigoButtonFn)dlsym(kit, "IndigoHIDMessageForButton");
  _keyboardFn = (CortexIndigoKeyboardFn)dlsym(kit, "IndigoHIDMessageForKeyboardArbitrary");
  _mouseFn = (CortexIndigoMouseFn)dlsym(kit, "IndigoHIDMessageForMouseNSEvent");
  ...
  if (client == nil) {
    if (error) {
      *error = initError ?: [NSError errorWithDomain:@"dev.cortex.device-helper.hid"
                                                code:4
  ...
  _client = client;
  _sendQueue = dispatch_queue_create("dev.cortex.device-helper.hid", DISPATCH_QUEUE_SERIAL);
  return YES;
}

- (void)sendMessage:(CortexIndigoMessage *)message {
  ...
  ((void (*)(id, SEL, CortexIndigoMessage *, BOOL, dispatch_queue_t, void (^)(NSError *)))objc_msgSend)(
...
- (void)sendTouchAtX:(double)x y:(double)y down:(BOOL)down {
  ...
  CortexIndigoMessage *seed = _mouseFn(&point, NULL, kTouchTarget, down ? kButtonOpDown : kButtonOpUp, NO);
  if (seed == NULL) {
    [self noteUndelivered];
    return;
  }
  ...
  size_t stride = sizeof(CortexIndigoPayload);
  CortexIndigoMessage *message = calloc(1, sizeof(CortexIndigoMessage) + stride);
  if (message == NULL) {
    free(seed);
    [self noteUndelivered];
    return;
  ...
  memcpy(&message->payload.event.touch, &seed->payload.event.touch, sizeof(CortexIndigoTouch));
  ...
  CortexIndigoPayload *secondPayload = (CortexIndigoPayload *)second;
...
- (void)sendKeyUsage:(uint32_t)usage down:(BOOL)down {
  ...
  CortexIndigoMessage *message = _keyboardFn(usage, down ? kButtonOpDown : kButtonOpUp);
  [self sendMessage:message];
}
...
- (NSInteger)typeText:(NSString *)text {
  ...
  for (NSUInteger index = 0; index < length; index++) {
    ...
    BOOL needsShift = NO;
    uint32_t usage = CortexUsageForCharacter(character, &needsShift);
    if (usage == 0) {
      skipped++;
      continue;
    }
...
- (void)sendButton:(CortexHardwareButton)button down:(BOOL)down {
  int op = down ? kButtonOpDown : kButtonOpUp;
  uint32_t consumerUsage = CortexConsumerUsage(button);
  ...
  [self sendMessage:_buttonFn(CortexButtonSource(button), op, kButtonTargetHardware)];
}

- (void)tapButton:(CortexHardwareButton)button {
