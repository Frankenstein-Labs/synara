...
//   CORTEX_PERF=1 bunx vitest run perf/engineStreamingThroughput.perf.test.ts
...
const ENABLED = process.env.CORTEX_PERF === "1";
const DELTAS_PER_THREAD = Number(process.env.CORTEX_PERF_DELTAS ?? 1_500);
if (!Number.isInteger(DELTAS_PER_THREAD) || DELTAS_PER_THREAD <= 0) {
  throw new Error("CORTEX_PERF_DELTAS must be a positive integer per thread");
}
...
async function createSystem(dbPath: string) {
  const layer = OrchestrationEngineLive.pipe(
    ...
    Layer.provideMerge(ServerConfig.layerTest(process.cwd(), { prefix: "cortex-engine-perf-" })),
  ...
  it("measures ms per delta for 1/5/10 concurrently streaming threads", async () => {
    ...
    for (let repeat = -1; repeat < 3; repeat += 1) {
      for (const threadCount of repeat % 2 === 0 ? [...THREAD_COUNTS].reverse() : THREAD_COUNTS) {
        const directory = mkdtempSync(join(tmpdir(), "cortex-engine-measure-"));
    ...
    writeFileSync(
      process.env.CORTEX_PERF_OUT ?? "/tmp/cortex-perf/engine-report.json",
