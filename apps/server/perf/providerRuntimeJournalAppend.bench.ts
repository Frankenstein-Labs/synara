...
const EVENT_COUNT = readPositiveInteger("CORTEX_BENCH_EVENTS", 10_000);
const SAMPLE_COUNT = readPositiveInteger("CORTEX_BENCH_SAMPLES", 8);
...
function createDatabase(): { readonly database: Database; readonly dispose: () => void } {
  const directory = mkdtempSync(path.join(os.tmpdir(), "cortex-runtime-journal-bench-"));
