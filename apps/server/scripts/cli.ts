...
const packCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const tarballPath = path.join(outDir, `cortex-server-${version}.tar.gz`);
      yield* runCommand(
  ...
  Command.withDescription("Produce a cortex-server-<version>.tar.gz from the staged package."),
);
...
const cli = Command.make("cli").pipe(
  Command.withDescription("Cortex server build & publish CLI."),
