...
Cortex does not host models or sell a separate model subscription. It operates supported
coding-agent runtimes installed and authenticated on your machine, then presents them through one
consistent workspace.

## Supported providers

| Provider                                                                | What Cortex connects to                                      |
| ----------------------------------------------------------------------- | ------------------------------------------------------------ |
| [Claude Code](https://www.trycortex.com/docs/providers/claude-code)     | Your installed Claude Code runtime and authenticated account |
| [Codex](https://www.trycortex.com/docs/providers/codex)                 | Your installed and authenticated Codex CLI                   |
| [OpenCode](https://www.trycortex.com/docs/providers/opencode)           | Your local OpenCode runtime and configured model providers   |
| [Cursor](https://www.trycortex.com/docs/providers/cursor)               | Your local Cursor agent runtime and account                  |
| [Devin](https://docs.devin.ai)                                          | Your installed and authenticated Devin CLI                   |
| [Antigravity](https://www.trycortex.com/docs/providers/antigravity)     | Your installed and authenticated Antigravity CLI             |
| [Grok Build](https://www.trycortex.com/docs/providers/grok)             | Your configured Grok Build runtime and access                |
| [Pi](https://www.trycortex.com/docs/providers/pi)                       | Pi and the model providers configured through it             |
| [Factory Droid](https://www.trycortex.com/docs/providers/factory-droid) | Your installed and authenticated Droid runtime               |

Provider availability can differ between the current stable release and development builds. Use the
provider settings in your installed Cortex version as the authoritative list for that build.

## What Cortex manages

Cortex provides the shared operating surface around each provider:
...
provider feature is supported through Cortex.

## Connect a provider

1. **Install the official runtime.** Use the provider's official installation instructions.
2. **Authenticate outside Cortex.** Complete the provider's normal sign-in or credential setup.
   Verify the runtime from a fresh terminal.
3. **Open Cortex provider settings.** Confirm that the provider is detected and enabled. When
...
4. **Check model discovery.** Open the model picker and confirm that the expected models and options
   appear. Cortex discovers many provider capabilities at runtime; the result can depend on the
...
Cortex normalizes these choices into the composer where possible without pretending that every
...
A [provider handoff](https://www.trycortex.com/docs/workflows/handoffs) allows another provider to
continue the task and work in the same environment with the context Cortex passes to it.
...
6. Does restarting Cortex refresh the provider status?
7. Does the provider itself report a service or account error?

Continue with the [troubleshooting hub](https://www.trycortex.com/docs/troubleshooting) when the
runtime works independently but remains unavailable in Cortex.

Use the dedicated [provider guides](https://www.trycortex.com/docs/providers) for exact
