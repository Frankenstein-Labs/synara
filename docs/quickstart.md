...
This guide takes you through Cortex's basic loop: open a repository, give one coding agent a
concrete task, supervise the work, and review the resulting diff. You should be up and running in
about five minutes.

> **Before you begin:** install Cortex and authenticate at least one supported provider. In
> shortcuts, `mod` means Command on macOS and Ctrl on Windows or Linux.

## 1. Add a Git project

Open Cortex and add a local repository.
...
[Git worktree](https://www.trycortex.com/docs/workflows/worktrees) when you begin running multiple
...
Cortex uses the provider runtime and account configured on your machine. It does not add a separate
Cortex model plan.
...
That is Cortex's core workflow:
...
- [Core concepts](https://www.trycortex.com/docs/getting-started/core-concepts) — projects, tasks,
  environments, provider sessions, and Git ownership.
- [Your first task](https://www.trycortex.com/docs/getting-started/first-task) — the same workflow
  in more detail, including recovery and review.
- [Best practices](https://www.trycortex.com/docs/workflows/best-practices) — patterns Cortex
