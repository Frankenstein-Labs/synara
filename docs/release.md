...
- Publishes default `latest*.yml` metadata plus byte-identical `cortex*.yml` aliases on every stable release so existing packaged binaries keep working.
...
- Publishes prerelease installers only on their versioned GitHub prerelease; prereleases never replace the stable `cortex` update manifests.
- Publishes the CLI package (`apps/server`, npm package `@cortex/cli`) with OIDC trusted publishing.
...
- Runtime channel: `cortex`. Stable clean-lane releases publish both `latest` and `cortex` metadata; the 0.4.x compatibility release remains available for historical migration.
- Repository slug source:
  - `CORTEX_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Required Cortex release assets for updater:
  - platform installers (`.exe`, `.dmg`, `.AppImage`, plus macOS `.zip` for Squirrel.Mac update payloads)
  - `cortex-mac.yml`, `cortex.yml`, and `cortex-linux.yml` metadata
  - every stable release includes both `cortex-mac.yml`, `cortex.yml`, `cortex-linux.yml` and `latest-mac.yml`, `latest.yml`, `latest-linux.yml`
  - `*.blockmap` files, except the macOS update `.zip.blockmap` removed after zip repack
- Enforced upgrade path:
  - Stable clean Cortex releases are created with `make_latest=true` and carry both six-manifest filenames in the versioned release.
  ...
  - Clean-release publication fails closed if either the default Latest manifests or the dedicated `cortex` aliases are missing.
- Production desktop builds omit web/server/desktop source maps by default to keep update payloads small. Set `CORTEX_WEB_SOURCEMAP=1`, `CORTEX_SERVER_SOURCEMAP=1`, or `CORTEX_DESKTOP_SOURCEMAP=1` only for a diagnostic release that needs them.
- macOS metadata note:
  - The build initially emits `latest-mac.yml` for both Intel and Apple Silicon.
  - The workflow merges the per-arch macOS metadata, then keeps the merged manifest as `latest-mac.yml` and copies it to `cortex-mac.yml` for stable releases.
...
1. Confirm the npm account controls the `@cortex` scope and can publish `@cortex/cli`.
...
- Every stable versioned release must include both the default `latest` updater metadata and the dedicated `cortex` aliases alongside its installers.
- The published release title should read `Cortex vX.Y.Z`.
...
- Optional jobs stay disabled unless repository variables enable them:
  - `CORTEX_PUBLISH_CLI=1`
  - `CORTEX_FINALIZE_RELEASE=1`
...
8. For a stable clean-lane release, confirm the new versioned release is GitHub Latest, contains all three default `latest` manifests plus all three `cortex` aliases, and left the historical compatibility release unchanged.
