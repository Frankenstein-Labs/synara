...
Cortex treats Windows support as a platform boundary, not as provider-specific command handling.
...
Use `@cortex/shared/executable` when code needs to identify the concrete executable that a launch would use. It owns PATH splitting, Windows PATHEXT ordering, manual paths, extensionless command names, and executable validation.
...
Use `prepareProcess` from `@cortex/shared/platformProcess` when a launch plan must be inspected or logged before execution. It owns:
...
Use `@cortex/shared/filesystemPlatform` for platform-sensitive durability and identity operations. It centralizes writable-handle fsync on Windows, POSIX directory fsync/no-follow behavior, and the documented Windows file-identity fallback used by guarded recovery code.
