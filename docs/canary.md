...
Cortex Canary is a frozen local build of a chosen remote Git ref. It is intentionally separate
from both Cortex Stable and the HMR development process.

## Isolation

- App name: `Cortex Canary`
- Bundle ID: `com.emanueledipietro.cortex.canary`
- Desktop origin: `cortex-canary://app`
- Cortex data: `~/.cortex-canary`
- Electron profile: `cortex-canary`
- Managed source: `~/.cache/cortex-canary/source`
- Runtime log: `~/.cortex-canary/canary.log`
...
```bash
bun run canary:setup -- --ref codex/cortex-canary
```
...
```bash
CORTEX_CANARY_HOME=/path/to/data \
CORTEX_CANARY_SOURCE=/path/to/source \
bun run canary:update
```
...
```bash
env -u CORTEX_AUTH_TOKEN \
  CORTEX_DEV_INSTANCE=my-feature \
  bun run dev -- --home-dir ./.cortex/dev-my-feature
```
