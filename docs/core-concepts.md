...
Cortex becomes much easier to use once its ownership model is clear: **each task owns one body of
...
| Concept          | Meaning                                                             |
| ---------------- | ------------------------------------------------------------------- |
| Workspace        | The complete Cortex application and the projects available in it    |
...
A project is the folder Cortex works with.
...
[thread goal](https://www.trycortex.com/docs/features/thread-goals). A goal can continue after a
clean turn, but queued user work, approvals, questions, interruptions, failures, and pause rules
remain in control.

Use a [thread fork](https://www.trycortex.com/docs/workflows/forks) when a new task should inherit
the conversation or split from one exact turn. Use a
[handoff](https://www.trycortex.com/docs/workflows/handoffs) when another provider should continue
...
Read the [Git worktrees guide](https://www.trycortex.com/docs/workflows/worktrees) before starting
several tasks in the same repository.

## Providers, models, and sessions

A provider is the coding-agent runtime Cortex operates, such as Claude Code, Codex, OpenCode, Cursor,
...
Cortex supplies the shared workspace around it:
...
Each task owns a provider session. Available models and controls can differ because Cortex discovers
capabilities from the installed runtime and account.

## Handoffs

A [provider handoff](https://www.trycortex.com/docs/workflows/handoffs) lets another supported
provider continue the same task and working environment using the context Cortex passes to it.
...
Cortex treats Git as the durable review and recovery layer.
...
Cortex's checkpoint and revert controls can help recover task work, but committed Git history remains
...
Read the [parallel agents guide](https://www.trycortex.com/docs/workflows/parallel-agents) before
...
Check the [keyboard reference](https://www.trycortex.com/docs/reference/keyboard-shortcuts) for the
