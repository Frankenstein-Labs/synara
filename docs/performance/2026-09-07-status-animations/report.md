...
During the original installed-app investigation, Cortex's renderer briefly used
35.1% CPU, its graphics helper 11.2%, and Claude 1.2%. These were uncontrolled
snapshots. Later, after the turn finished, Cortex was almost idle. That localized
...
the real built Cortex stylesheet, a copy of the spinner's SVG geometry, static
...
```sh
cd apps/web
node node_modules/vite/bin/vite.js build --config perf/vite.config.ts --outDir /private/tmp/cortex-energy-20260907/baseline-dist
# After applying the status-animation change:
node node_modules/vite/bin/vite.js build --config perf/vite.config.ts --outDir /private/tmp/cortex-energy-20260907/status-dist
cd ../..
# With web and desktop dependencies installed:
node apps/web/perf/status-animation-runner.mjs /private/tmp/cortex-energy-20260907
```
...
and discarded-experiment results remain in `/private/tmp/cortex-energy-20260907`.
...
- Full `bun fmt`, `bun lint` and `bun typecheck` were not run: repository
  ...
  remains outstanding. No installed Cortex app was replaced or restarted.
