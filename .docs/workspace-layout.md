...
Cortex is a Bun/Turbo monorepo. Runtime ownership is split across the app workspaces, while shared schemas and cross-runtime helpers live under `packages`.

- `/apps/server` — The authoritative Cortex backend and published `@cortex/cli` package. Owns orchestration/persistence, provider adapters and health/discovery, Git/worktrees, terminals, automation, workspace files, HTTP/WebSocket RPC, and the bundled web client used outside Vite development.
...
- `/apps/desktop` — Electron host for the shared web client. Supervises a desktop-scoped Cortex server process and provides native window, update, IPC, browser-automation, and other OS/Electron integrations.
...
- `/packages/shared` — Shared runtime utilities consumed by multiple apps/packages, including pure helpers as well as intentionally cross-runtime logging, worker, filesystem/network, and platform-boundary utilities. Uses explicit subpath exports (for example `@cortex/shared/git` and `@cortex/shared/threadWorkspace`) rather than one catch-all barrel.
