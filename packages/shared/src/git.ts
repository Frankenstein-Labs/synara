 ...
 */
export const WORKTREE_BRANCH_PREFIX = "cortex";
...
const CORTEX_BRANCH_FALLBACK = "update";
...
export function buildCortexBranchName(preferredBranch?: string | null): string {
  const preferred = preferredBranch?.trim() ?? "";
  ...
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || CORTEX_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniqueCortexBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildCortexBranchName(preferredBranch);
  return resolveUniqueBranchName(existingBranchNames, resolvedBase);
}
