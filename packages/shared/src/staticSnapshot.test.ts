...
describe("findAsarArchivePath", () => {
  it("resolves the containing archive for a path inside an asar", () => {
    expect(
      findAsarArchivePath(
        "/Applications/Cortex.app/Contents/Resources/app.asar/apps/server/dist/client",
      ),
    ).toBe(path.join("/Applications/Cortex.app/Contents/Resources/app.asar"));
  });
  ...
  it("returns null for plain directories", () => {
    expect(findAsarArchivePath("/Users/me/dev/cortex/apps/web/dist")).toBeNull();
  });
});
