...
/**
 * Who owns the boot. Cortex only auto-shuts down devices it booted itself;
 * anything the user started (pane picker, Simulator.app) outlives the session.
 */
export const DeviceBootSource = Schema.Literals(["cortex", "user"]);
...
export const ThreadDeviceState = Schema.Struct({
  ...
  /** Devices the pane may show: booted devices plus anything Cortex is booting. */
...
/** Devices Cortex itself booted are capped globally; viewing already-booted devices is not. */
export const DEVICE_CORTEX_BOOT_LIMIT = 3;
...
/**
 * Boot is refusable rather than fatal: past DEVICE_CORTEX_BOOT_LIMIT the caller
...
export const DeviceBootResult = Schema.Union([
  ...
  Schema.Struct({
    ...
    cortexBooted: Schema.Array(DeviceDescriptor).check(Schema.isMaxLength(64)),
  }),
]);
...
export const DeviceInstallAppInput = Schema.Struct({
  udid: DeviceUdid,
  /** Absolute path to a built `.app` bundle; Cortex never runs the build itself. */
 ...
 * the bitstream. Runtime encode/decode lives in `@cortex/shared/deviceFrame`;
