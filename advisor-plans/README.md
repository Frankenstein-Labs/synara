# Cortex Roadmap Execution Controller
...
- `P2-ACP-01`: the official TypeScript SDK owns framing, JSON-RPC dispatch/correlation, validation,
  ...
  and workspace package are deleted. `AcpErrors.ts` and `AcpExtensions.ts` retain only Cortex-owned
...
- `PRUNE-09` — `ProviderService` no longer reads `CORTEX_PROVIDER_RUNTIME_IDLE_STOP_MS` twice through
...
cancellation, handler dispatch, and NDJSON encoding. Cortex retains Effect lifecycle, bounded
